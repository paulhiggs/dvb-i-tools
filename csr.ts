...
import type { language_stats } from "./lib/IANA_languages.mjs";
...
if (cluster.isPrimary) {
	...
	cluster.on("message", (worker, msg : IPC_message, /* eslint-disable no-unused-vars*/ handle /* eslint-enable */) => {
		if (msg.topic)
			switch (msg.topic) {
				case RELOAD:
				...
				case INCR_FAILURES:
					...
					break;
				case STATS:
					let kl : language_stats = {};
					knownLanguages.stats(kl);
					console.log(`knownLanguages.length=${kl.numLanguages || "unknown"}`);
					...
					console.log(`application version=${pkg.version || "unknown"}`);
					break;
			}
	});
} else {
	...
	let manualCORS = function (req : express.Request, res : express.Response, next : express.NextFunction) {
		next();
	};
	if (options.CORSmode == CORSlibrary) {
		app.use(cors());
	} else if (options.CORSmode == CORSmanual) {
		manualCORS = function (req : express.Request, res : express.Response, next : express.NextFunction) {
			let opts = res.getHeader("X-Frame-Options");
			if (opts) {
				switch (datatypeIs(opts)) {
					case "string":
						if (opts as string != "SAMEORIGIN") 
							res.setHeader("X-Frame-Options", [opts as string, "SAMEORIGIN"]);
						break;
					case "array":
						let o2 : string[] = opts as string[];
						if (!o2.includes("SAMEORIGIN")) o2.push("SAMEORIGIN");
						res.setHeader("X-Frame-Options", o2);
						break;
				}
			} 
	...
	process.on("message", (msg : IPC_message) => {
		if (msg.topic)
			switch (msg.topic) {
				case UPDATE:
					knownCountries.loadCountries(options.urls ? { url: ISO3166.url } : { file: ISO3166.file });
	...
	let http_server = app.listen(options.port, (error? : Error) => {
		if (error)
	...
	if (https_options.key && https_options.cert) {
		...
		https_server.listen(options.sport, () => {
			console.log(chalk.cyan(`HTTPS listening on port number ${options.port}, PID=${process.pid}`));
		});
	}
}
