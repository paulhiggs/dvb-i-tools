...
if (cluster.isPrimary) {
	...
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	cluster.on("message", (worker, msg : IPCMessageType, handle) => {
		if (msg.topic)
			switch (msg.topic) {
				case RELOAD:
	...
	process.on("message", (msg : IPCMessageType) => {
		if (msg.topic)
			switch (msg.topic) {
				case UPDATE:
					knownCountries.loadCountries(options.urls ? { url: ISO3166.url } : { file: ISO3166.file });
