...
export default class IANAlanguages {
	...
	#loadLanguagesFromFile(languagesFile, purge = false, async = true, verbose = true) {
		verbose && console.log(chalk.yellow(`reading languages from ${languagesFile}`));
		if (purge) this.empty();
	...
	#loadLanguagesFromURL(languagesURL, purge = false, async = true, verbose = true) {
		...
		verbose && console.log(chalk.yellow(`${isHTTPurl ? "" : "--> NOT "}retrieving languages from ${languagesURL} using fetch()`));
		if (!isHTTPurl) return;
	...
	loadLanguages(options, async = true, verbose = true) {
		if (!options) options = {};
		if (!Object.prototype.hasOwnProperty.call(options, "purge")) options.purge = false;

		if (options.file) this.#loadLanguagesFromFile(options.file, options.purge, async, verbose);
		else if (options.url) this.#loadLanguagesFromURL(options.url, options.purge, async, verbose);
	}
