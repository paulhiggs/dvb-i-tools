 ...
 * 
 * Check Extension elements in DVB-I Service Lists and Service List Registry responses
 */
...
export function CheckExtension(extn, extLoc, errs, errCode) {
	...
	if (extn_extensionName) {
		const where = (extension, location) => `${extension} extension only permitted in ${location}`;
		switch (extn_extensionName) {
			case "DVB-HB":
				if (extLoc != EXTENSION_LOCATION_DASH_INSTANCE)
					errs.addError({
						code: `${errCode}-1`,
						message: where("DVB-HB", "DVB DASH service instance"),
						fragment: extn,
			...
			case "vnd.apple.mpegurl":
				if (extLoc != EXTENSION_LOCATION_OTHER_DELIVERY && extLoc != EXTENSION_LOCATION_SERVICE_LIST_REGISTRY)
					errs.addError({
						code: `${errCode}-3`,
						message: where("HLS", "Service List Registry or Service List"),
						fragment: extn,
						key: keys.k_Extensibility,
					});
				break;
