...
const RESULT_WITH_INSTRUCTION = (source) =>`${BREAK}<p><i>Results:</i> ${source}</p>`;
...
const DETAIL_FORM_HEADER = (mode, colour = null) =>
	`${scrollFunc}<table ${colour ? ` style="color:${colour}"` : ""}><tr>${SHOW_LINE_NUMBER ? "<th>line</th>" : ""}<th>code</th><th>${mode}</th></tr>`;
const DESCRIPTION_TABLE_HEADER = () => `<table><tr><th>code</th><th>description</th></tr>`;
const DEBUG_FORM_HEADER = (mode) => `${scrollFunc}<table><tr><th>code</th><th>${mode}</th></tr>`;
const TABLE_FOOTER = `</table>${BREAK}`;

const tabluateMessage = (res, value) => {
	res.write(`<tr>`);
...
const simpleMessage = (res, value) => {
	res.write(`<tr><td>${value.code}</td><td>${value.message}</td></tr>`);
};

const lineCompareFn = (a, b) => a?.line && b?.line ? a.line - b.line : 0;

const ErrorTable = (res, errors, title, colour = null) => {
...
function tabulateResults(source, res, error, errs) {
	...
	if (errs) {
		...
		if (errs.debugs.length > 0) {
			...
			errs.debugs.forEach((d) => simpleMessage(res, d));
			resultsShown = true;
			res.write(TABLE_FOOTER);
		}
	}
	...
	if (errs && errs.markupXML?.length > 0) {
		...
		errs.markupXML.forEach((line) => {
			...
			const qualifier = tip ? ` class="${cla}" title="${tip}"` : "";
			if (SHOW_LINE_NUMBER) {
				lineNum++;
...
export function drawForm(req, res, modes, supportedRequests, motd = null, error = null, errs = null) {
	...
	function redrawForm() {
		...
		document.getElementById("variants").hidden=!document.getElementById("radSL").checked
	}
	</script>
