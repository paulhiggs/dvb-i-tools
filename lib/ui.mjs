...
function tabulateResults(source, res, error, errs) {
	...
	if (errs) {
		...
		let lineCompare = (a, b) => {
			let x = a?.line && b?.line ? a.line - b.line : 0;
			return x;
		};
		if (errs.numFatals() > 0) {
			res.write(DETAIL_FORM_HEADER("fatals (further validation aborted)", "red"));
			if (MESSAGES_IN_ORDER) errs.fatals.sort(lineCompare);
		...
		if (errs.numErrors() > 0) {
			...
			if (MESSAGES_IN_ORDER) errs.errors.sort(lineCompare);
		...
		if (errs.numWarnings() > 0) {
			...
			if (MESSAGES_IN_ORDER) errs.warnings.sort(lineCompare);
		...
		if (errs.numInformationals() > 0) {
			...
			if (MESSAGES_IN_ORDER) errs.informationals.sort(lineCompare);
