...
import ErrorList, { FATAL, APPLICATION, INFORMATION, WARNING, DEBUG } from "./error_list.mts";
...
import { forEachChildElement, forEachNamedChildElement, hasChildren, attrAnyNs, getAnyNs } from "../libxml2-wasm-extensions.mts";
...
export function checkAttributes(checkElement : XmlElement, requiredAttributes : string[], optionalAttributes : string[], definedAttributes : string[], errs : ErrorList, errCode : string) {
	...
	requiredAttributes.forEach((attributeName) => {
		if (!attrAnyNs(checkElement, attributeName))
	...
	definedAttributes.forEach((attribute) => {
		if (!isIn(requiredAttributes, attribute) && !isIn(optionalAttributes, attribute))
			if (attrAnyNs(checkElement, attribute))
				errs.addError({
					type: INFORMATION,
					code: `${errCode}-3`,
 ...
 */
export type ChildElement_type = {
	name: string;
	minOccurs?: number;
	maxOccurs?: number;
}

export function checkTopElementsAndCardinality(parentElement: XmlElement, childElements: ChildElement_type[], definedChildElements: string[], allowOtherElements: boolean, errs: ErrorList, errCode: string) {
	if (!parameterCheck("checkTopElementsAndCardinality", parentElement, null, errs, "TE000")) return false;

	const findElementIn = (elementList : ChildElement_type[], elementName : string) => 
		(datatypeIs(elementList, "array") ? elementList.find((element) => element.name == elementName) : false);

	function getNamedChildElements(node : XmlElement, childElementName : string) : XmlElement[] {
		const res : XmlElement[] = [];
		forEachNamedChildElement(node, childElementName, (child) => {
			res.push(child);
		});
		return res;
	}
	...
	let rv = true;
	const thisElem = `${parentElement.parent.name}.${parentElement.name}`.elementize();
	...
	if (hasChildren(parentElement)) {
		...
		forEachChildElement(parentElement, (child) => {
...
export function SchemaVersionCheck(document : XmlDocument, publication_state : number, errs : ErrorList, errCode : string) {
	const ServiceList = getAnyNs(document.root, dvbi.e_ServiceList);
