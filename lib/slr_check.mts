...
import { LoadGenres, LoadCountries } from "./classification_scheme_loaders.mts";
import { keys, InvalidCountryCode, InvalidURL } from "./common_errors.mts";
import { slVersions, dvbi, dvbiEA, dvbiEC, dvbisld, validApplicationTypes, DvbIApplicationTypes } from "./DVB-I_definitions.mts";
...
import { validServiceListLogo, isA177specification_URN, a177versionFromURN } from "./sl_data_versions.mts";
import { SLR_GetSchema, SLR_SchemaVersion, SLR_SchemaSpecVersion } from "./slr_data_versions.mts";
...
import { documentNamespace, forEachNamedChildElement, attrAnyNs, attrAnyNsValueOrNull, getAnyNs} from "../libxml2-wasm-extensions.mts";
...
export default class ServiceListRegistryCheck {
	...
	stats() {
		return {
			...
			numKnownCountries: this.knownCountries?.count(),
			numAllowedGenres: this.allowedGenres?.count(),
		};
	}

	private doSchemaVerification(ServiceListRegistry : XmlDocument, errs : ErrorList, errCode : string, report_schema_version : boolean = true) {
		const x = SLR_GetSchema(ServiceListRegistry.root.namespaceUri);
		if (x && x.schema) {
			SchemaCheck(ServiceListRegistry, x.schema, x.filename, errs, `${errCode}:${SLR_SchemaVersion(documentNamespace(ServiceListRegistry.root))}`);
	...
	private ValidateGenre(Offering : XmlElement, errs : ErrorList, errCode : string) {
		...
		forEachNamedChildElement(Offering, tva.e_Genre, (Genre) => {
			...
			if (genreValue && !this.allowedGenres.isIn(genreValue))
				errs.addError({
					...
					description: "The genre of the contents available for the service list",
				});
		});
	}
	 ...
	 * @param {String}     errCode           error code prefix to be used in reports
	 */
	private ValidateRelatedMaterial(RelatedMaterial : XmlElement, errs : ErrorList, errCode : string) {
		...
		if (attrAnyNs(HowRelated, tva.a_href) && !validServiceListLogo(HowRelated))
			errs.addError({
				code: `${errCode}-6`,
				fragment: HowRelated,
	...
	private CheckServiceListOffering(Offering : XmlElement, errs : ErrorList, errCode : string) {
		const offeringNamespace = documentNamespace(Offering);
		// ServiceListOfferingType @regulatorListFlag checked by schema

		// check ServiceListOfferingType@lang
		const Offering_lang = attrAnyNsValueOrNull(Offering, tva.a_lang);
		...
		let lowest_version = -10,
			found_versioned_URI = false;
		forEachNamedChildElement(Offering, dvbisld.e_ServiceListURI, (ListURI) => {
			if (attrAnyNs(ListURI, dvbisld.a_standardVersion)) found_versioned_URI = true;
			// check ServiceListURI
			const ListURI_standardVersion = attrAnyNsValueOrNull(ListURI, dvbisld.a_standardVersion);

			if (SLR_SchemaVersion(offeringNamespace) >= slVersions.r7) {
				// check @standardVersion
				if (found_versioned_URI && !attrAnyNs(ListURI, dvbisld.a_standardVersion)) {
					errs.addError({
						code: `${errCode}-31`,
						message: `${dvbisld.e_ServiceListURI} elements without ${dvbisld.a_standardVersion.attribute()} must be listed before versioned URIs`,
						fragment: ListURI,
						key: "improper semantics",
						clause: "A177 Table 12",
						description: "A ServiceListURI without the @standardVersion attribute should be specified first, for compatibility with any client that does not support versioning",
					});
				}
				
				if (ListURI_standardVersion) {
					if (isA177specification_URN(ListURI_standardVersion)) {
						const versionFound = a177versionFromURN(ListURI_standardVersion);
						...
						errs.addError({
							...
							fragment: ListURI,
							key: "invalid specifiction URN",
							clause: "A177 Table 1c",
						});
				}
			}
			const ListURI_contentType = attrAnyNsValueOrNull(ListURI, dvbisld.a_contentType); // its a required attribute so should always exist
			if (ListURI_contentType && ListURI_standardVersion && a177versionFromURN(ListURI_standardVersion) <= slVersions.r6 && ListURI_contentType != "application/vnd.dvb.dvbisl+xml") {
				errs.addError({
					type: WARNING,
					code: `${errCode}-34`,
					...
					clause: "A177 Table 12",
					description:
						"For ServiceListURI elements specified for Version less than or equal to relase 6 (see table 1c) the @contentType attribute shall be application/vnd.dvb.dvbisl+xml otherwise it should be application/xml",
				});
			}
			...
			const URI = getAnyNs(ListURI, dvbisld.e_URI);
			if (URI && !isHTTPURL(URI.content)) errs.addError(InvalidURL(URI.content, URI, dvbisld.e_URI.elementize(), `${errCode}-36`));
		});

		// check <ServiceListOffering><Delivery>
		const Delivery = getAnyNs(Offering, dvbisld.e_Delivery);
		if (Delivery) {
			forEachNamedChildElement(Delivery, dvbisld.e_DVBTDelivery, (DVBTDelivery) => {
				if (DVBTDelivery.attr(dvbisld.a_originalNetworkID) && SLR_SchemaVersion(offeringNamespace) >= slVersions.r6) {
					errs.addError(DeprecatedAttribute(DVBTDelivery.attr(dvbisld.a_originalNetworkID), SLR_SchemaSpecVersion(slVersions.r6), `${errCode}-41`));
				}
			});
			...
			forEachNamedChildElement(Delivery, dvbisld.e_DVBSDelivery, (DVBSDelivery) => {
				if (DVBSDelivery.attr(dvbisld.a_originalNetworkID) && SLR_SchemaVersion(offeringNamespace) >= slVersions.r6) {
					errs.addError(DeprecatedAttribute(DVBSDelivery.attr(dvbisld.a_originalNetworkID), SLR_SchemaSpecVersion(slVersions.r6), `${errCode}-42`));
				}
			});
		...
		forEachNamedChildElement(Offering, dvbisld.e_Language, (Language) => {
			ValidateLanguage(Language.content, errs, `${errCode}l`, Language.line);
		});
		...
		forEachNamedChildElement(Offering, dvbisld.e_TargetCountry, (TargetCountry) => {
		...
		forEachNamedChildElement(Offering, tva.e_RelatedMaterial, (RelatedMaterial) => {
		...
		const ServiceListId = getAnyNs(Offering, dvbisld.e_ServiceListId);
	 ...
	 */
	/*public*/ doValidateServiceListRegistry(SLRtext : string, errs : ErrorList, options : {log_prefix? : string | null, report_schema_version? : boolean}= {}) {
		this.numRequests++;
		...
		if (!this.doSchemaVerification(SLR, errs, "SR005", options.report_schema_version)) {
			errs.addError({
				code: "SR010",
				message: `Unsupported namespace ${documentNamespace(SLR.root).quote()}`,
				key: keys.k_XSDValidation,
			});
			return;
		}
		...
		const slrOptionalAttributes = ["schemaLocation"];
		const SLRversion = SLR_SchemaVersion(documentNamespace(ServiceListRegistry));
		if (SLRversion >= slVersions.r3) slrRequiredAttributes.push(tva.a_lang);
		if (SLRversion >= slVersions.r6) slrOptionalAttributes.push(dvbi.a_version);
		...
		const SLRlang = attrAnyNsValueOrNull(ServiceListRegistry, tva.a_lang);
		if (SLRlang) ValidateLanguage(SLRlang, errs, "SR012", ServiceListRegistry.line);

		//<ServiceListEntryPoints><ServiceListRegistryEntry>
		const RegistryEntity = getAnyNs(ServiceListRegistry, dvbisld.e_ServiceListRegistryEntity);
		if (RegistryEntity) this.CheckOrganisationType(RegistryEntity, errs, "SR051");

		//<ServiceListEntryPoints><ProviderOffering>
		forEachNamedChildElement(ServiceListRegistry, dvbisld.e_ProviderOffering, (ProviderOffering) => {
			const Provider = getAnyNs(ProviderOffering, dvbisld.e_Provider);
			if (Provider) this.CheckOrganisationType(Provider, errs, "SR081");

			forEachNamedChildElement(ProviderOffering, dvbisld.e_ServiceListOffering, (ServiceListOffering) => {
				this.CheckServiceListOffering(ServiceListOffering, errs, "SR082");
			});
		});

		//<ServiceListEntryPoints><Extension>
		forEachNamedChildElement(ServiceListRegistry, dvbi.e_Extension, (Extension) => {
