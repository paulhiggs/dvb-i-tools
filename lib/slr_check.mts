...
import { keys, InvalidCountryCode } from "./common_errors.mts";
...
export default class ServiceListRegistryCheck {
	...
	constructor(useURLs : boolean, opts : slr_options, async : boolean = true) {
		this.#numRequests = 0;
	...
	/*private*/ #doSchemaVerification(ServiceListRegistry : XmlElement, errs : ErrorList, errCode : string, report_schema_version : boolean = true) : boolean {
		...
		if (Delivery) {
			...
			while ((DVBTDelivery = Delivery.getAnyNs(dvbisld.e_DVBTDelivery, ++dt)) != null)
				if (DVBTDelivery.attr(dvbisld.a_originalNetworkID) && SLR_SchemaVersion(offeringNamespace) >= slVersions.r6) {
					errs.addError(ibute(DVBTDelivery.attr(dvbisld.a_originalNetworkID), "A177r6", `${errCode}-41`));
				}
			let ds = 0,
				DVBSDelivery;
