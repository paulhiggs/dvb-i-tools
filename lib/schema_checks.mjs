...
import { parameterCheck, HasProperty } from "./utils.mjs";
...
export function checkTopElementsAndCardinality(parentElement, childElements, definedChildElements, allowOtherElements, errs, errCode) {
	...
	const findElementIn = (elementList, elementName) => (datatypeIs(elementList, "array") ? elementList.find((element) => element.name == elementName) : false);
	...
	childElements.forEach((child) => {
		if (child?.name) {
			const min = HasProperty(child, "minOccurs") ? child.minOccurs : 1;
			const max = HasProperty(child, "maxOccurs") ? child.maxOccurs : 1;
...
export function SchemaVersionCheck(document, publication_state, errs, errCode) {
	...
	if (publication_state & StandardStatus.OLD) {
		const err1 = { code: `${errCode}a`, message: "schema version is out of date", key: "schema version" };
		if (ServiceList) err1.line = ServiceList.line;
		errs.addError(err1);
	}
	if (publication_state & StandardStatus.DRAFT) {
		const err2 = { type: WARNING, code: `${errCode}b`, message: "schema is in draft state", key: "schema version" };
