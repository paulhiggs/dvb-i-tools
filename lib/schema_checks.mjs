...
import { keys, DeprecatedElement, DeprecatedAttribute } from "./common_errors.mjs";
...
export function checkDeprecatedElement(element, deprecatedElement, revision, errs, errCode) {
	const el = element.getAnyNS(deprecatedElement);
	if (el)
		errs.addError(DeprecatedElement(el, revision, errCode));	
}

export function checkDeprecatedAttribute(element, deprecatedAttribute, revision, errs, errCode) {
	const attr = element.attrAnyNs(deprecatedAttribute);
	if (attr)
		errs.addError(DeprecatedAttribute(attr, revision, errCode));
}
