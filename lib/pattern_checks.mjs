...
const variant = `(?<variant>[${e_alpha}]{5,8}|[${e_digit}][${l_alphanum}]{3})`;
const region = `(?<region>[${e_highalpha}]{2}|[${e_digit}]{3})`;
const script = `(?<script>[${e_highalpha}][${e_lowalpha}]{3})`;
const extlang = `(?<extlang>[${e_alpha}]{3}(-[${e_alpha}]{3}){0,2})`;
const language = `(?<language>([${e_lowalpha}]{2,3}(-${extlang})?)|[${e_alpha}]{4}|[${e_alpha}]{5,8})`;
...
export const isRatioType = (ratio) => (datatypeIs(ratio, "string") ? ratioRegex.test(ratio.trim()) : false);
...
export const isUTCDateTime = (time) => (datatypeIs(time, "string") ? UTCregex.test(time.trim()) : false);

import { allowedImageTypes } from "./MIME_checks.mjs";
export function isInlineImage(data) {
	let valid = false;
...
const HTTPprotocolPrefix = new RegExp("^https?://", "i");
 ...
 * see RFC 3986 - https://tools.ietf.org/html/rfc3986
 */
const HTTPSprotocolRegex = new RegExp("^https:$", "i");
const HTTPSprotocolPrefix = new RegExp("^https://", "i");
export function isHTTSPURL(url) {
	url = decodeURIComponent(url);
...
export const isURN = (urn) => (datatypeIs(urn, "string") ? URNregex.test(urn) : false);
 ...
 */
export const isURI = (uri) => isURL(uri) || isURN(uri);
 ...
 */
const dataRegexp = new RegExp(/^data:((?:\w+\/(?:(?!;).)+)?)((?:;[\w=]*[^;])*),(.+)$/, "i");
export const isDataURI = (uri) => (datatypeIs(uri, "string") ? dataRegexp.test(uri) : false);
...
export const isISODuration = (duration) => (datatypeIs(duration, "string") ? isoRegex.test(duration.trim()) : false);
 ...
 */
const locatorRegex = new RegExp(`^dvb://[${e_hex}]+.[${e_hex}]*.[${e_hex}]+;[${e_hex}]+$`);
export const isDVBLocator = (locator) => (datatypeIs(locator, "string") ? locatorRegex.test(locator.trim()) : false);
...
export const isPostcode = (postcode) => (datatypeIs(postcode, "string") ? postcodeRegex.test(postcode.trim()) : false);
...
export const isWildcardPostcode = (postcode) =>
...
export const validExtensionName = (ext) => (datatypeIs(ext, "string") ? ExtensionRegex.test(ext.trim()) : false);
...
export const validFrameRate = (rate) => (datatypeIs(rate, "string") ? FrameRateRegex1.test(rate.trim()) || FrameRateRegex2.test(rate.trim()) : false);
...
export const isDomainName = (domain) => (datatypeIs(domain, "string") ? DomainNameRegex.test(domain.trim()) : false);
...
export const isRTSPURL = (url) => (datatypeIs(url, "string") ? isURL(url) && RTSPRegex.test(url.trim()) : false);
...
export const validServiceDaysList = (daysList) => (datatypeIs(daysList, "string") ? DaysListRegex.test(daysList.trim()) : false);
...
export const validZuluTimeType = (time) => (datatypeIs(time, "string") ? ZuluRegex.test(time.trim()) : false);
...
const languageRegex = new RegExp(languageFormat,"i");
export const  isTVAAudioLanguageType = (languageCode) => datatypeIs(languageCode, "string") ? languageRegex.test(languageCode) : false;
...
export const isASCII = (ascii_str) => (datatypeIs(ascii_str, "string") ? ASCIIregexp.test(ascii_str) : false);
...
const year = "([0-9]{4})", month = "([0-9]{2})", day = "([0-9]{2})";
const date = `${year}(-${month}(-${day})?)?`
const DNScomp = `[${l_alphanum}](([${l_alphanum}]|-)*[${l_alphanum}])?`
const DNSname = `${DNScomp}(.${DNScomp})*`
const emailAddress = `[${l_alphanum}-._]+@${DNSname}`
const authorityName = `(${DNSname}|${emailAddress})`;
const taggingEntity = `${authorityName},${date}`;
//const unreserved = `${e_alpha}0-9-._~`
const pct_encoded = "(%[0-9a-f]{2})"
//const sub_delims = "*\\+!$&',;=\\(\\)"
//const pchar = `${unreserved}:${sub_delims}@|${pct_encoded}`; // from RFC 3986
const specific = `(['a-z0-9-._~!$&()*+,;=:@?/]|${pct_encoded})*`;
const fragment = `(['a-z0-9-._~!$&()*+,;=:@?/]|${pct_encoded})*`;
...
export const isTAGURI = (identifier) => (datatypeIs(identifier, "string") ? TagRegex.test(identifier.trim()) : false);
...
export const isCRIDURI = (value) => (datatypeIs(value, "string") ? CRIDRegex.test(value.trim()) : false);
...
export const hasNonPrintableChars = (value) => (datatypeIs(value, "string") ? !ASCIIPrint.test(value) : false);

//const UnicodePrint = new RegExp(/^[^\p{Cc}\p{Cf}\p{Zl}\p{Zp}]+$/);
//const UnicodePrint = new RegExp(/^\PC+$/);
//export const hasNonPrintableUnicodeChars = (value) => (datatypeIs(value, "string") ? !UnicodePrint.test(value) : false);
...
export const isUUIDformat = (value) => (datatypeIs(value, "string") ? UUIDRegex.test(value) : false);