...
import { CMCD_MODE_REQUEST, CMCD_MODE_RESPONSE, CMCD_MODE_EVENT, dvbiEA } from "./DVB-I_definitions.mjs";
...
function checkCMCDkeys(CMCD, errs, errCode) {
	...
	const version = CMCD.attrAnyNs(dvbi.a_version) ? CMCD.attrAnyNs(dvbi.a_version).value : 1;
	...
	if (version >= 2) {
		...
		if (!isIn(keys, CMCD_keys.version)) errs.addError(missing_mandatory_key(CMCD_keys.version));
	}
}
