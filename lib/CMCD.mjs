...
function check_CMCD(CMCDelem, counts, errs, errCode, props) {
	...
	let rep = 0,
		CMCDreport;
	while ((CMCDreport = CMCDelem.get(xPath(props.prefix, dvbi.e_Report, ++rep), props.schema)) != null) {
...
export function ValidateCMCDinDASH(props, DASHDeliveryParameters, errs, errCode) {
	let cc = 0,
		mode_counts = {}, // counts of each request tyoe
		CMCDelem;
	while ((CMCDelem = DASHDeliveryParameters.get(xPath(props.prefix, dvbi.e_CMCD, ++cc), props.schema)) != null) check_CMCD(CMCDelem, mode_counts, errs, errCode, props);
}
