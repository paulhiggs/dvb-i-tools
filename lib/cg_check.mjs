...
export default class ContentGuideCheck {
	...
	/*private*/ #doSchemaVerification(TVAdoc, errs, errCode, report_schema_version = true) {
		...
		let x = SchemaVersions.find((s) => s.namespace == TVAdoc.root.documentNamespace());
		if (x && x.schema) {
			SchemaCheck(TVAdoc, x.schema, errs, `${errCode}:${SchemaVersion(TVAdoc.root.documentNamespace())}`);
	 ...
	 * @param {ErrorList}  errs           errors found in validaton
	 */
	/* private */ #ValidateAVAttributes(AVAttributes, errs) {
		if (!AVAttributes) {
			errs.addError({
				type: APPLICATION,
				code: "AV000",
		...
		checkTopElementsAndCardinality(
			AVAttributes,
			SchemaVersion(AVAttributes.documentNamespace()) >= cgVersions.r2
				? [
		...
		let CaptioningAttributes = AVAttributes.getAnyNs(tva.e_CaptioningAttributes);
	...
	/* private */ #ValidateRestartRelatedMaterial(RelatedMaterial, errs) {
		...
		let MediaLocator = RelatedMaterial.getAnyNs(tva.e_MediaLocator);
		if (MediaLocator)
	...
	/* private */ #ValidateInstanceDescription(VerifyType, InstanceDescription, isCurrentProgram, errs) {
		...
		switch (VerifyType) {
			case tva.e_OnDemandProgram:
				...
				const allowedODChildren =
					SchemaVersion(InstanceDescription.documentNamespace()) < cgVersions.r2
						? [
		...
		switch (VerifyType) {
			...
			case tva.e_ScheduleEvent:
				/* eslint-disable no-case-declarations */
				let Genre = InstanceDescription.getAnyNs(tva.e_Genre);
				/* eslint-enable */
				if (Genre) {
		...
		// <AVAttributes>
		let AVAttributes = InstanceDescription.getAnyNs(tva.e_AVAttributes);
		if (AVAttributes) this.#ValidateAVAttributes(AVAttributes, errs);
		...
		// <RelatedMaterial>
		let RelatedMaterial = InstanceDescription.getAnyNs(tva.e_RelatedMaterial);
		if (RelatedMaterial) {
	...
	/* private */ #ValidateOnDemandProgram(OnDemandProgram, programCRIDs, plCRIDs, requestType, errs) {
		...
		// <Program>
		let Program = OnDemandProgram.getAnyNs(tva.e_Program);
		if (Program) {
		...
		// <AuxiliaryURL>
		const AuxiliaryURL = OnDemandProgram.getAnyNs(va.e_AuxiliaryURL);
		...
		if ([CG_REQUEST_SCHEDULE_NOWNEXT, CG_REQUEST_SCHEDULE_TIME, CG_REQUEST_SCHEDULE_WINDOW, CG_REQUEST_PROGRAM].includes(requestType)) {
			let DeliveryMode = OnDemandProgram.getAnyNs(tva.e_DeliveryMode);
		...
		// <Free>
		let Free = OnDemandProgram.getAnyNs(tva.e_Free);
	...
	/* private */ #CheckProgramLocation(ProgramDescription, programCRIDs, currentProgramCRID, requestType, errs, o = null) {
		...
		ProgramLocationTable?.childNodes().forEachSubElement((child) => {
			switch (child.name) {
				...
				case tva.e_Schedule:
					/* eslint-disable no-case-declarations */
					let thisServiceIdRef = this.#ValidateSchedule(child, programCRIDs, plCRIDs, currentProgramCRID, requestType, errs);
					/* eslint-enable */
	...
	doValidateContentGuide(CGtext, requestType, errs, options = {}) {
		...
		switch (requestType) {
			...
			case CG_REQUEST_SCHEDULE_NOWNEXT:
				...
				/* eslint-disable no-case-declarations */
				let currentProgramCRIDnn = this.#CheckProgramInformation(ProgramDescription, programCRIDs, groupIds, requestType, errs);
				/* eslint-enable */
			...
			case CG_REQUEST_SCHEDULE_WINDOW:
				...
				if (this.#hasElement(ProgramDescription, tva.e_GroupInformationTable)) this.#CheckGroupInformationNowNext(ProgramDescription, groupIds, requestType, errs);
				/* eslint-disable no-case-declarations */
				let currentProgramCRIDsw = this.#CheckProgramInformation(ProgramDescription, programCRIDs, groupIds, requestType, errs);
				/* eslint-enable */
