...
import { isCRIDURI, isTAGURI } from "./URI_checks.mjs";
import { xPath, isIn, isIni, unEntity, parseISOduration, CountChildElements, DuplicatedValue } from "./utils.mjs";
import { isHTTPURL, isDVBLocator, isUTCDateTime } from "./pattern_checks.mjs";
...
import { checkAttributes, checkTopElementsAndCardinality, hasChild, SchemaCheck, SchemaLoad, SchemaVersionCheck } from "./schema_checks.mjs";
...
import { LoadCredits } from "./role_loader.mjs";
...
export default class ContentGuideCheck {
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	/* private */ #ValidateRelatedMaterial_Pagination(props, BasicDescription, Location, errs) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "VP000",
		...
		while ((RelatedMaterial = BasicDescription.get(xPath(props.prefix, tva.e_RelatedMaterial, ++rm), props.schema)) != null) {
			...
			if (!HowRelated) errs.addError(NoChildElement(tva.e_HowRelated.elementize(), RelatedMaterial, Location, "VP001"));
				...
				const MediaLocator = RelatedMaterial.get(xPath(props.prefix, tva.e_MediaLocator), props.schema);
				const MediaURI = MediaLocator ? MediaLocator.get(xPath(props.prefix, tva.e_MediaUri), props.schema) : null;
				if (MediaURI) {
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	/* private */ #ValidateRelatedMaterial_BoxSetContents(props, BasicDescription, errs) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "RM-BSC000",
	 ...
	 * @param {String}     errCode          error code prefix to be used in reports
	 */
	/* private */ #ValidateTitle(props, containingNode, allowSecondary, TypeIsRequired, errs, errCode) {
		if (!containingNode) {
	 ...
	 * @param {ErrorList}  errs            errors found in validaton
	 */
	/* private */ #ValidateBasicDescription(props, parentElement, requestType, categoryGroup, errs) {
		if (!parentElement) {
			errs.addError({
				type: APPLICATION,
				code: "BD000",
			...
			case tva.e_ProgramInformation:
				switch (requestType) {
					...
					case CG_REQUEST_SCHEDULE_TIME:
						...
						this.#ValidateTitle(props, BasicDescription, true, true, errs, "BD011");
					...
					case CG_REQUEST_PROGRAM: // 6.10.5.3
						...
						this.#ValidateTitle(props, BasicDescription, true, true, errs, "BD021");
					...
					case CG_REQUEST_BS_CONTENTS: // 6.10.5.4
						...
						this.#ValidateTitle(props, BasicDescription, true, true, errs, "BD031");
						...
						this.#ValidateRelatedMaterial_PromotionalStillImage(props, BasicDescription, errs);
						this.#ValidateRelatedMaterial_Pagination(props, BasicDescription, "Box Set Contents", errs);
					...
					case CG_REQUEST_MORE_EPISODES: // 6.10.5.5
						...
						this.#ValidateTitle(props, BasicDescription, true, true, errs, "BD041");
			...
			case tva.e_GroupInformation:
				switch (requestType) {
					...
					case CG_REQUEST_BS_CONTENTS: // clause 6.10.5.6 table 47a
						...
						this.#ValidateTitle(props, BasicDescription, false, false, errs, "BD091");
						this.#ValidateSynopsis(props, BasicDescription, [tva.SYNOPSIS_MEDIUM_LABEL], [], errs, "BD092");
						this.#ValidateKeyword(props, BasicDescription, 0, 20, errs, "BD093");
						this.#ValidateRelatedMaterial_BoxSetContents(props, BasicDescription, errs);
						break;
					case CG_REQUEST_BS_LISTS: // clause 6.10.5.6 table 47
						...
						this.#ValidateTitle(props, BasicDescription, false, false, errs, "BD063");
						if (!isParentGroup) {
					...
					case CG_REQUEST_BS_CATEGORIES: // clause 6.10.5.8
						...
						this.#ValidateTitle(props, BasicDescription, false, false, errs, "BD082");
						...
						this.#ValidateRelatedMaterial_PromotionalStillImage(props, BasicDescription, errs);
						this.#ValidateRelatedMaterial_Pagination(props, BasicDescription, "Box Set Categories", errs);
						break;
					default:
						errs.addError({
							type: APPLICATION,
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	/* private */ #ValidateGroupInformationBoxSets(props, GroupInformation, requestType, categoryGroup, indexes, groupsFound, errs) {
		if (!GroupInformation) {
			errs.addError({
				type: APPLICATION,
				code: "GIB000",
		...
		if (!isParentGroup) {
			...
			if (MemberOf) {
				...
				let crid = MemberOf.attrAnyNsValueOr(tva.a_crid, null);
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	/* private */ #ValidateGroupInformationSchedules(props, GroupInformation, requestType, categoryGroup, errs) {
		if (!GroupInformation) {
			errs.addError({
				type: APPLICATION,
				code: "GIS000",
	 ...
	 * @param {ErrorList}  errs                errors found in validaton
	 */
	/* private */ #ValidateGroupInformationMoreEpisodes(props, GroupInformation, requestType, categoryGroup, groupsFound, errs) {
		if (!GroupInformation) {
			errs.addError({
				type: APPLICATION,
				code: "GIM000",
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	/* private */ #ValidateGroupInformation(props, GroupInformation, requestType, categoryGroup, indexes, groupsFound, errs) {
		if (!GroupInformation) {
			errs.addError({
				type: APPLICATION,
				code: "GI000",
	...
	/* private */ #CheckGroupInformation(props, ProgramDescription, requestType, groupIds, errs, o) {
		...
		gi = 0;
		while ((GroupInformation = GroupInformationTable.get(xPath(props.prefix, tva.e_GroupInformation, ++gi), props.schema)) != null) {
			this.#ValidateGroupInformation(props, GroupInformation, requestType, categoryGroup, indexes, groupIds, errs);
	 ...
	 * @param {ErrorList}  errs                errors found in validaton
	 */
	/* private */ #ValidateGroupInformationNowNext(props, GroupInformation, requestType, numEarlier, numNow, numLater, groupCRIDsFound, errs) {
	...
	/* private */ #ValidateAVAttributes(props, AVAttributes, errs) {
		...
		checkTopElementsAndCardinality(
			AVAttributes,
			SchemaVersion(props.namespace) >= cgVersions.r2
				? [
	...
	/* private */ #ValidateInstanceDescription(props, VerifyType, InstanceDescription, isCurrentProgram, errs) {
		...
		function checkGenre(genre, errs, errcode) {
			...
			let GenreType = genre.attrAnyNsValueOr(tva.a_type, tva.DEFAULT_GENRE_TYPE);
	 ...
	 * @param {ErrorList}  errs                errors found in validaton
	 */
	/* private */ #ValidateEvent(props, Event, programCRIDs, plCRIDs, currentProgramCRID, Schedule, errs) {
		let prefix = "";
