...
export default class ContentGuideCheck {
	...
	/* private */ #checkTAGUri(elem, errs, errCode) {
		const svcID = elem?.attrAnyNsValueOr(tva.a_serviceIDRef, null);
		if (svcID) {
			if (!isTAGURI(svcID))
				errs.addError({
					type: WARNING,
				...
				ParentalGuidance.forEachChildElement((pgChild) => {
					switch (pgChild.name) {
						...
						case tva.e_ExplanatoryText:
							...
							// eslint-disable-next-line no-case-declarations
							const lengthAttr = pgChild.attrAnyNsValueOr(tva.a_length, null);
		...
		ProgramInformation.forEachChildElement((child) => {
			switch (child.name) {
				...
				case tva.e_MemberOf: // <ProgramInformation><MemberOf>
					// eslint-disable-next-line no-case-declarations
					const child_type = child.attrAnyNsValueOr(tva.a_type, null),
						child_crid = child.attrAnyNsValueOr(tva.a_crid, null);
	...
	/* private */ #ValidateGroupInformationBoxSets(GroupInformation, requestType, categoryGroup, indexes, groupsFound, errs) {
		...
		if (!isParentGroup) {
			...
			if (MemberOf) {
				...
				const MemberOf_index = MemberOf.attrAnyNsValueOr(tva.a_index, null);
				if (MemberOf_index) {
	...
	/* private */ #ValidateAVAttributes(AVAttributes, AudioCodecCS, errs) {
		...
		AVAttributes.forEachNamedChildElement(tva.e_AudioAttributes, (AudioAttributes) => {
			...
			if (MixType) {
				...
				MixType_href = MixType.attrAnyNsValueOr(tva.a_href, null);
				if (MixType_href) {
			...
			if (AudioLanguage) {
				...
				const AudioLanguage_purpose = AudioLanguage.attrAnyNsValueOr(tva.a_purpose, null);
	...
	/* private */ #ValidateInstanceDescription(VerifyType, InstanceDescription, isCurrentProgram, errs) {
		...
		switch (VerifyType) {
			...
			case tva.e_ScheduleEvent:
				...
				if (Genre) {
					...
					const Genre_type = Genre.attrAnyNsValueOr(tva.a_type, null);
					if (Genre_href) {
		...
		InstanceDescription.forEachNamedChildElement(tva.e_OtherIdentifier, (OtherIdentifier) => {
			...
			const OtherIdentifier_type = OtherIdentifier.attrAnyNsValueOr(tva.a_type, null);
