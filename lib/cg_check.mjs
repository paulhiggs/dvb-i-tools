...
export default class ContentGuideCheck {
				...
				if (MediaLocator.length != 0)
					MediaLocator.forEach((ml) => {
						...
						ml.childNodes()?.forEachNamedSubElement(tva.e_AuxiliaryURI, (child) => {
							...
							let contentType = child.attrAnyNsValueOr(tva.a_contentType, null);
		...
		ProgramInformation.childNodes()?.forEachSubElement((child) => {
			switch (child.name) {
				...
				case tva.e_MemberOf: // <ProgramInformation><MemberOf>
					...
					let MO_index = child.attrAnyNsValueOr(tva.a_index, null);
					if (MO_index) {
		...
		if (contentsGroup) {
			...
			while ((GroupInformation = GroupInformationTable.get(xPath(props.prefix, tva.e_GroupInformation, ++gi), props.schema)) != null) {
				if (GroupInformation.line != contentsGroup.line) {
					const MemberOf = GroupInformation.get(xPath(props.prefix, tva.e_MemberOf), props.schema);
					if (MemberOf) {
						const MemberOf_crid = emberOf.attrAnyNsValueOr(tva.a_crid, null);
	...
	/* private */ #ValidateGroupInformationNowNext(props, GroupInformation, requestType, errs, numEarlier, numNow, numLater, groupCRIDsFound) {
		...
		if (GroupInformation_groupId) {
			if ((GroupInformation_groupId == dvbi.CRID_EARLIER && numEarlier > 0) || (GroupInformation_groupId == dvbi.CRID_NOW && numNow > 0) || (GroupInformation_groupId == dvbi.CRID_LATER && numLater > 0)) {
