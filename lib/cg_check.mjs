...
import { ValidatePromotionalStillImage, validateAnyContentDigests } from "./related_material_checks.mjs";
 ...
 */
const CG_SchemaVersion = (namespace) => {
				...
				if (MediaLocator.length != 0)
					MediaLocator.forEach((locator) => {
						...
						locator.forEachNamedChildElement(tva.e_AuxiliaryURI, (AuxiliaryURI) => {
							...
							checkAttributes(AuxiliaryURI, [tva.a_contentType], [tva.e_contentDigest], tvaEA.AuxiliaryURI, errs, "TA010");
							...
							validateAnyContentDigests(AuxiliaryURI, errs, "TA012")
						});
						if (!hasAuxiliaryURI) errs.addError(NoChildElement(tva.e_AuxiliaryURI.elementize(), locator, Location, "TA013"));
					});
				else errs.addError(NoChildElement(tva.e_MediaLocator.elementize(), RelatedMaterial, Location, "TA014"));
			}
		}
	}
