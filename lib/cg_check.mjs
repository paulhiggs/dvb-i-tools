...
function LoadCGschemas(opts) {
	...
	opts.verbose && console.log(chalk.yellow.underline("loading content guide schemas..."));
	CG_SchemaVersions.forEach((version) => {
		opts.verbose && process.stdout.write(chalk.yellow(`..loading ${version.version} ${version.namespace} from ${version.filename} `));
		const schema = readFileSync(version.filename);
		if (schema) version.schema = XmlDocument.fromBuffer(schema, { url: version.filename });
		opts.verbose && process.stdout.write(`${version.schema ? chalk.green("OK") : chalk.red.bold("FAIL")}\n`);
	});
}
