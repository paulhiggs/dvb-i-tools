...
const TESTSUITE_DIR = "testsuite";
export const __dirname = process.cwd().endsWith(TESTSUITE_DIR) ? process.cwd().substring(0, process.cwd().length - TESTSUITE_DIR.length) : process.cwd();
...
export const resouces_subDir = "res";
const SLEPR_subDir = "registries",
	schemasPath =  process.cwd().endsWith(TESTSUITE_DIR) ? "../schemas": "schemas";

const subdirDVBI = join(resouces_subDir, "dvbi"),
	subdirISO = join(resouces_subDir, "iso"),
	subdirMPEG7 = join(resouces_subDir, "mpeg7"),
	subdirTVA = join(resouces_subDir, "tva");

const pathDVBCS = join(__dirname, resouces_subDir, "dvb", "cs"),
	pathDVBI = join(__dirname, subdirDVBI),
	pathIANA = join(__dirname, resouces_subDir, "iana"),
...
export const spam_blocker_config = join(__dirname, "sblocker_config.json");
...
const idDVB_LinkApplicationTypeCS = "DVBLinkedApplicationCS-2019.xml";
export const DVBI_LinkedApplicationCS = { file: join(pathDVBI, idDVB_LinkApplicationTypeCS), url: `${DVB_METADATA}${url2019CS}/${idDVB_LinkApplicationTypeCS}` };
...
const idMPEG7_AudioCodingFormatCS = "AudioCodingFormatCS.xml";
export const MPEG7_AudioCodingFormatCS = { file: join(pathMPEG7, idMPEG7_AudioCodingFormatCS), url: `${REPO_RAW}${subdirMPEG7}/${idMPEG7_AudioCodingFormatCS}` };
...
export const DVBI_ServiceListSchema = {
	...
	r8: { file:  `./${schemasPath}/dvbi_v8.0-with-hls-hbbtv-dvbhb.xsd` },
};
