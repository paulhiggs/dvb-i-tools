...
import { attrAnyNs, attrAnyNsValueOrNull, forEachChildElement, forEachNamedChildElement } from "../libxml2-wasm-extensions.mts";
 ...
 */
function validateImageRelatedMaterial(RelatedMaterial : XmlElement, location : string, allowedHowRelated : string[], errs : ErrorList, errCode : string) {
	...
	let HowRelated : XmlElement = null,
		Format : XmlElement = null,
		MediaLocator : XmlElement = null;
	...
	if (Format) {
		...
		forEachNamedChildElement(Format, tva.e_StillPictureFormat, (StillPicture) => {
			...
			const childHref = attrAnyNsValueOrNull(StillPicture, tva.a_href);
			if (childHref)
				switch (childHref) {
	...
	if (attrAnyNs(MediaLocator, dvbi.a_contentLanguage)) 
		checkLanguage(attrAnyNs(MediaLocator, dvbi.a_contentLanguage).value, MediaLocator, errs, `${errCode}-27`);
	if (!hasMediaURI)
		errs.addError({
			code: `${errCode}-26`,
...
export function checkValidLogos(RelatedMaterial : XmlElement, location : string, errs : ErrorList, errCode : string) {
	if (!parameterCheck("checkValidLogos", RelatedMaterial, dvbi.e_RelatedMaterial, errs, `${errCode}-00`)) return;

	const specifiedMediaTypes: Array<string> = [];
	forEachNamedChildElement(RelatedMaterial, tva.e_MediaLocator, (MediaLocator) => {
		...
		const MediaLocator_language = attrAnyNsValueOrNull(MediaLocator, dvbi.a_contentLanguage);
