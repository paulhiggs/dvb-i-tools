...
import { datatypeIs, DefaultProperty } from "./utils.mts"
...
export default class IANAlanguages {
	...
	languageRedundant : number;
	languageNotSpecified : number;
	languageInvalidType : number;
	...
	constructor() {
		...
		this.#languagesList = new Set<string>();
		this.#redundantLanguagesList = [];
		this.#languageRanges = [];
		this.#signLanguagesList = new Set<string>();
		this.#regionsList = new Set<string>();				// we parse regions from the source location but dont use them in DVB-I validation
	}

	/**
	 * Remove all values and reinitialise
	 *
	...
	#processLanguageData(languageData) {
				...
				if (match) {
					...
					for (let a = start1; a <= end1; a++)
						for (let b = start2; b <= end2; b++)
							res.push(`${String.fromCharCode(a)}${String.fromCharCode(b)}`);
				}
			}
			return res
		}
		...
		entries.forEach((entry) => {
			...
			if (items.includes("Type: language") || items.includes("Type: extlang")) {
				for (let i = 0; i < items.length; i++)
					if (items[i].startsWith("Subtag:")) {
						const subtag = items[i].split(":")[1].trim();
						if (items.includes("Scope: private-use")) {
			...
			} else if (items.includes("Type: redundant")) {
				const redund: RedundntLanguageType = {};
				items.forEach((item) => {
			...
			} else if (items.includes("Type: region")) {
				items.forEach((item) => { 
...
const isValidLangFormat = (lang: string): boolean => BCP47langaugetag_exp.test(lang);
