...
export default function CheckAccessibilityAttributes(AccessibilityAttributes : XmlElement, cs, errs : ErrorList, errCode : string) {
	...
	let checkSignLanguage = (elem : XmlElement, childName : string, errNum : number) => {
		elem?.childNodes().forEachNamedSubElement(childName, (child) => {
			...
			if (cs.KnownLanguages.checkSignLanguage(languageCode) != LanguageCheckResponse.languageKnown)
				errs.addError({
					code: `${errCode}-${errNum}`,
					fragment: child,
	...
	let checkLanguageFmt = (elem : XmlElement, childName : string, errNum : number) => {
		elem?.childNodes().forEachNamedSubElement(childName, (child) => {
	...
	let checkLanguagePurpose = (elem : XmlElement, childName : string, errNum : number) => {
	...
	let carriages : Array<string> = [],
		...
		hasAppInformation : boolean = false;
	AccessibilityAttributes?.childNodes().forEachSubElement((child) => {
		switch (child.name) {
			...
			case tva.e_ScreenReaderAttributes:
				...
				let ScreenReaderChildElements = allowedAppChildren.concat([{ name: tva.e_ScreenReaderLanguage, minOccurs: 0, maxOccurs: Infinity }]);
				/* eslint-enable */
