...
export function LoadKnownCustomKeysRegistry(opts) {
	if (opts.verbose) console.log(chalk.yellow.underline("loading known CMCD Custom keys ..."));

	if (opts.useURLs) {
		if (opts.verbose) console.log(chalk.yellow(`retrieving known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.url}`));
		if (opts.async)
			...
			if (resp) {
				...
				else console.log(chalk.red(`error ((${resp.status} ${resp.statusText}) retrieving ${CMCD_CUSTOM_KEYS_REGISTRY.url}`));
			}
		}
	}
	else {
		if (opts.verbose) console.log(chalk.yellow(`reading known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.file}`));
		if (opts.async) 
			readFile(
