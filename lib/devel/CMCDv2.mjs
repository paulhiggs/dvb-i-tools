...
function checkCMCDkeys(Report, version, errs, errCode) {
	if (!Report) {
		errs.addError({ type: APPLICATION, code: `${errCode}-00`, message: "checkCMCDkeys() called with Report=null" });
		return;
	}
	...
	if (reporting_mode == CMCD_MODE_EVENT) {
		if (!isIn(keys, CMCD_keys.timestamp)) errs.addError({
			code: `${errCode}d`,
			message: `the key ${CMCD_keys.timestamp.quote()} is required to be specified for ${reportingMode(reporting_mode)} reporting`,
			fragment: CMCD,
			key: error_key,
		});
		if (!isIn(keys, CMCD_keys.event)) errs.addError({
			code: `${errCode}e`,
			message: `${reportingMode(report_type)} mode report will include "${CMCD_keys.event.quote()}" - it is not specified here`,
			fragment: CMCD,
			key: error_key,
		});
	}
...
export function check_CMCD(CMCDelem, counts, errs, errCode) {
	...
	CMCDDelem.forEachNamedChildElement(dvbi.e_Report, (Report) => {
