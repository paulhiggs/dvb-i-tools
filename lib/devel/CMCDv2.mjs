...
function checkCMCDkeys(CMCD, errs, errCode) {
	...
	const version = parseInt(CMCD.attrAnyNsValueOr(dvbi.a_CMCDversion, 1));
...
export function check_CMCD(CMCDelem, counts, errs, errCode) {
	...
	const CMCDversion = CMCDelem.attrAnyNsValueOr(dvbi.a_CMCDversion, null);
	...
	if (enabledKeys) {
		...
		if (CMCDelem.attrAnyNs(dvbi.a_contentId)) {
			const contentId = CMCDelem.attrAnyNs(dvbi.a_contentId);
