...
let csr : SLEPR | undefined = undefined;
 ...
 *
 * @param {Object} options   Command Line Arguments - see OptionDefinitions in all-in-one.ts
 */
export default function validator(options) {
	...
	let manualCORS = function (req : Request, res : Response, next : NextFunction) {
		next();
	};
	if (options.CORSmode == CORSlibrary) {
		app.use(cors());
	} else if (options.CORSmode == CORSmanual) {
		manualCORS = function (req : Request, res : Response, next : NextFunction) {
			let opts = res.get("X-Frame-Options");
			if (opts) {
	...
	var http_server = app.listen(options.port, (error? : Error) => {
		if (error)
