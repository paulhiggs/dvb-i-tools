...
import { MODE_URL, MODE_FILE, MODE_SL, MODE_CG, MODE_SLR, MODE_UNSPECIFIED } from "./ui.mts";
...
function DVB_I_check(req, res, slcheck, cgcheck, slrcheck, hasSL, hasCG, hasSLR, motd, mode = MODE_UNSPECIFIED, linktype = MODE_UNSPECIFIED) {
	if (!req.session.data) {
		// setup defaults
		req.session.data = {};
	...
	const FormArguments = { cg: MODE_CG, sl: MODE_SL, slr: MODE_SLR, file: MODE_FILE, url: MODE_URL, hasSL: hasSL, hasCG: hasCG, hasSLR: hasSLR };
...
export type ValidatorInitializationOptions = {
	languages?: ReturnType<typeof LoadLanguages>;
	genres?: ReturnType<typeof LoadGenres>;
	countries?: ReturnType<typeof LoadCountries>;
	accessibilities?: ReturnType<typeof LoadAccessibilityPurpose>;
	audiopurps?: ReturnType<typeof LoadAudioPurpose>;
	stcodings?: ReturnType<typeof LoadSubtitleCodings>;
	stcarriage?: ReturnType<typeof LoadSubtitleCarriages>;
	stpurposes?: ReturnType<typeof LoadSubtitlePurposes>;
	videofmts?: ReturnType<typeof LoadVideoCodecCS>;
	audiofmts?: ReturnType<typeof LoadAudioCodecCS>;
	audiopres?: ReturnType<typeof LoadAudioPresentationCS>;
	credits?: ReturnType<typeof LoadCredits>;
	ratings?: ReturnType<typeof LoadRatings>;
};
...
export default function validator(options) {
	if (options.nocsr && options.nosl && options.nocg && options.noslr) {
	...
	app.use(morgan(":remote-addr :protocol :method :url :status :res[content-length] :counts - :response-time ms :agent :parseErr :location :vary :redirect"));
	...
	const knownLanguages = LoadLanguages(options.urls);
	const isoCountries = LoadCountries(options.urls);
	const knownGenres = LoadGenres(options.urls);

	if (!options.nosl || !options.nocg || !options.noslr) {
		...
		if (!options.noslr) slrcheck = new ServiceListRegistryCheck(options.urls, { languages: knownLanguages, genres: knownGenres, countries: isoCountries });
	}
	...
	if (!options.noslr) {
		app.all("/validate_slr", express.text({ type: "application/xml", limit: "10mb" }), (req, res) => {
			validateServiceListRegistry(req, res, slrcheck, motd, false);
		});

		app.all("/validate_slr_json", express.text({ type: "application/xml", limit: "10mb" }), (req, res) => {
	...
	if (!options.nosl || !options.nocg || !options.noslr) {
		app.get("/check", express.text({ type: "application/xml", limit: "10mb" }), (req, res) => {
			DVB_I_check(req, res, slcheck, cgcheck, slrcheck, !options.nosl, !options.nocg, !options.noslr, motd);
		});
	...
	let  tabulate = function (res, group, stats) {
	...
	app.get("/stats", (req, res) => {
		...
		tabulate(res, "CSR", csr ? csr.stats() : { active: "false" });
		tabulate(res, "SL", slcheck ? slcheck.stats() : { active: "false" });
		tabulate(res, "SLR", slrcheck ? slrcheck.stats() : { active: "false" });
		tabulate(res, "CG", cgcheck ? cgcheck.stats() : { active: "false" });
	...
	var http_server = app.listen(options.port, () => {
