...
// from RFC3986
const u_pct_encoded = `%[${e_digit}${e_hexChar}]{2}`,
	u_sub_delims = `!$&amp;&#x22;${e_quot}()*+,;=`,
	u_pchar = `[${e_unreserved}${u_sub_delims}:&amp;]|${u_pct_encoded}`;

// from RFC 4151
const t_year = `${e_digit}[4]`,
	t_month = `${e_digit}[2]`,
	t_day = `${e_digit}[2]`,
	t_date = `${t_year}(\-${t_month}(\-${t_day})?)?`,
	t_alphaNum = `${e_lowalpha}${e_highalpha}${e_digit}`,
	t_DNScomp = `[${t_alphaNum}]([${t_alphaNum}\-]*[${t_alphaNum}])?`,
	t_DNSname = `${t_DNScomp}(\.${t_DNScomp})*`,
	t_emailAddress = `([${t_alphaNum}-._])+@${t_DNSname}`,
	t_authorityName = `(${t_DNSname}|${t_emailAddress})`,
	t_taggingEntity = `${t_authorityName},${t_date}`,
	t_specific = `(${u_pchar}/?)*`,
	t_fragment = `(${u_pchar}/?)*`;
...
export let isRatioType = (ratio : string) : boolean => (datatypeIs(ratio, "string") ? ratioRegex.test(ratio.trim()) : false);
...
export let isUTCDateTime = (time : string) : boolean => (datatypeIs(time, "string") ? UTCregex.test(time.trim()) : false);
...
export function isInlineImage(data : string) : boolean {
	...
	allowedImageTypes.forEach((image_MIME) => {
		if (data.startsWith(`data:${image_MIME};base64,`))
			valid = true;
	});
	return valid;
}
 ...
 * @param {string} url url or urn to check
 */
const URNregex = new RegExp(`^${e_URN}$`, "i");
export let isURL = (url : string) : boolean => {
...
export let isURN = (urn : string) : boolean => (datatypeIs(urn, "string") ? URNregex.test(urn) : false);
...
export let isDataURI = (uri : string) : boolean => (datatypeIs(uri, "string") ? dataRegexp.test(uri) : false);
...
export let isISODuration = (duration : string) : boolean => (datatypeIs(duration, "string") ? isoRegex.test(duration.trim()) : false);
...
export let isDVBLocator = (locator : string) : boolean => (datatypeIs(locator, "string") ? locatorRegex.test(locator.trim()) : false);
...
export let isPostcode = (postcode : string) : boolean => (datatypeIs(postcode, "string") ? postcodeRegex.test(postcode.trim()) : false);
...
export let isWildcardPostcode = (postcode : string) : boolean =>
...
export let validExtensionName = (ext : string) : boolean => (datatypeIs(ext, "string") ? ExtensionRegex.test(ext.trim()) : false);
...
export let validFrameRate = (rate : string) : boolean => (datatypeIs(rate, "string") ? FrameRateRegex1.test(rate.trim()) || FrameRateRegex2.test(rate.trim()) : false);
...
export let isDomainName = (domain : string) : boolean => (datatypeIs(domain, "string") ? DomainNameRegex.test(domain.trim()) : false);
...
export let isRTSPURL = (url : string) : boolean => (datatypeIs(url, "string") ? isURL(url) && RTSPRegex.test(url.trim()) : false);
...
export let validServiceDaysList = (daysList : string) : boolean => (datatypeIs(daysList, "string") ? DaysListRegex.test(daysList.trim()) : false);
...
export let validZuluTimeType = (time : string) : boolean => (datatypeIs(time, "string") ? ZuluRegex.test(time.trim()) : false);
...
export let isASCII = (ascii_str : string) : boolean => (datatypeIs(ascii_str, "string") ? ASCIIregexp.test(ascii_str) : false);
...
const TagRegex_new = new RegExp(`/^tag:${t_taggingEntity}:${t_specific}(#${t_fragment})?$/`, "");
export let isTAGURI = (identifier : string) : boolean => (datatypeIs(identifier, "string") ? TagRegex.test(identifier.trim()) : false);
...
export let isCRIDURI = (value : string) : boolean => (datatypeIs(value, "string") ? CRIDRegex.test(value.trim()) : false);

/**
 * check is the argument only contains printable ascii characters ("space" --> "tilda")
 * @param {string} value  value whose format to check
 * @returns {boolean} true if the value only contains ASCII characters, else false
 */
const ASCIIPrint = new RegExp("/^[ -~]+$/");
export let hasNonPrintableChars = (value : string) : boolean => (datatypeIs(value, "string") ? !ASCIIPrint.test(value) : false);
