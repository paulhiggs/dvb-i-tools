 ...
 *
 * @param {string | undefined} url  The value whose format is to be checked
 ...
 *
 * @param {string | undefined} path  The value whose format is to be checked
 ...
 * @param {string | undefined} urn | url
 * @returns {boolean}
 */
 ...
 *
 * @param {string | undefined} uri  the value to be checked
...
export let isWildcardPostcode = (postcode : string | undefined) : boolean =>
	postcode ? WildcardEndRegex.test(postcode.trim()) || WildcardMiddleRegex.test(postcode.trim()) || WildcardFirstRegex.test(postcode.trim()) : false;

/**
 * check if the argument is in the correct format for an DVB-I extension identifier
 *
 * @param {string | undefined} ext  the signalled extensionName
...
export let validExtensionName = (ext : string | undefined) : boolean => ext ? ExtensionRegex.test(ext.trim()) : false;

/**
 * check if the argument is in the correct format for a TV-Anytime FrameRateType
 *    <pattern value="([0-9]{1,3}(.[0-9]{1,3})?)|([0-9]{1,3}/1.001)"/>
 *
 * @param {string | undefined} ratre  the signalled frameRate
...
export let validFrameRate = (rate : string | undefined) : boolean => rate ? FrameRateRegex1.test(rate.trim()) || FrameRateRegex2.test(rate.trim()) : false;

/**
 * checks of the specified argument matches an domain name (RFC 1034)
 *
 * @param {string | undefined} domain  The value whose format is to be checked
 * @returns {boolean} true if the argument is a domain name
 */
const DomainNameRegex = new RegExp(/^[a-z\d]+([\-\.]{1}[a-z\d]+)*\.[a-z]{2,5}(:[\d]{1,5})?(\/.*)?$/, "i");
export let isDomainName = (domain : string | undefined) : boolean => domain ? DomainNameRegex.test(domain.trim()) : false;
 ...
 *
 * @param {string | undefined} daysList  the value to check, likely from an Interval@days attribute
...
export let validServiceDaysList = (daysList : string | undefined) : boolean => daysList ? DaysListRegex.test(daysList.trim()) : false;

/**
 * check that a values conforms to the ZuluTimeType type
 *
 * @param {string | undefined} time the value to check, likely from an Interval@startTime or @endTime attributes
 * @returns {boolean} true if @time is properly formated
 */
const ZuluRegex = new RegExp(/^(([01]\d|2[0-3]):[0-5]\d:[0-5]\d(\.\d+)?)Z$/);
export let validZuluTimeType = (time : string | undefined) : boolean => time ? ZuluRegex.test(time.trim()) : false;

/**
 * checks that the supplied argument conforms to the pattern for a TVA LanguageType
 * @param {string | undefined} languageCode  the language code to check
...
export function isTVAAudioLanguageType(languageCode :string | undefined, caseSensitive : boolean = true) {
 ...
 * @param {string | undefined} ascii_str   the string to check
...
export let isASCII = (ascii_str : string | undefined) : boolean => ascii_str ? ASCIIregexp.test(ascii_str) : false;
