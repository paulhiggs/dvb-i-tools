...
import { slVersions, dvbi, dvbiEA, dvbisld, validApplicationTypes } from "./DVB-I_definitions.mjs";
...
import { checkAttributes, SchemaCheck, SchemaVersionCheck, SchemaLoad } from "./schema_checks.mjs";
...
export default class ServiceListRegistryCheck {
	...
	/*private*/ #doSchemaVerification(ServiceListRegistry, props, errs, errCode, report_schema_version = true) {
		const x = SLR_GetSchema(props.namespace);
		if (x && x.schema) {
			SchemaCheck(ServiceListRegistry, x.schema, errs, `${errCode}:${SLR_SchemaVersion(props.namespace)}`);
	 ...
	 * @param {String}     errCode           error code prefix to be used in reports
	 */
	/* private */ #ValidateRelatedMaterial(props, RelatedMaterial, errs, errCode) {
		...
		if (HowRelated.attrAnyNs(dvbi.a_href) && !validServiceListLogo(HowRelated, props.namespace))
			errs.addError({
				code: `${errCode}-6`,
				fragment: HowRelated,
	...
	/* private */ #CheckServiceListOffering(props, Offering, errs, errCode) {
		...
		while ((ListURI = Offering.getAnyNs(dvbisld.e_ServiceListURI, ++u)) != null) {
			...
			if (SLR_SchemaVersion(props.namespace) >= slVersions.r7) {
			...
			const ListURI_contentType = ListURI.attrAnyNsValueOr(dvbi.a_contentType, null); // its a required attribute so should always exist
			if (ListURI_contentType && SLR_SchemaVersion(props.namespace) <= slVersions.r6 && ListURI_contentType != "application/vnd.dvb.dvbisl+xml") {
				errs.addError({
					type: WARNING,
					code: `${errCode}-34`,
					message: `${dvbi.a_contentType.attribute()} should be "application/vnd.dvb.dvbisl+xml" for A177r6 and below`,
					fragment: ListURI,
					keys: "invalid value",
					clause: "A177 Table 12",
				});
			}
			if (ListURI_contentType && SLR_SchemaVersion(props.namespace) >= slVersions.r7 && ListURI_contentType != "application/xml") {
				errs.addError({
					code: `${errCode}-35`,
					message: `${dvbi.a_contentType.attribute()} must be "application/xml" for A177r7 and above`,
					fragment: ListURI,
					keys: "invalid value",
					clause: "A177 Table 12",
				});
			...
			let URI = ListURI.getAnyNs(dvbi.e_URI);
			if (URI) {
				if (!isHTTPURL(URI.content))
					errs.addError({
						code: `${errCode}-36`,
						message: `${dvbi.e_URI.elementize()}=${URI.content.quote()} is not a valid HTTP URL`,
		...
		const Delivery = Offering.getAnyNs(dvbi.e_Delivery);
		if (Delivery) {
			let dt = 0,
				DVBTDelivery;
			while ((DVBTDelivery = Delivery.getAnyNs(dvbisld.e_DVBTDelivery, ++dt)) != null)
				if (DVBTDelivery.attr(dvbisld.a_originalNetworkID) && SLR_SchemaVersion(props.namespace) >= slVersions.r6) {
			...
			while ((DVBSDelivery = Delivery.getAnyNs(dvbisld.e_DVBSDelivery, ++ds)) != null)
				if (DVBSDelivery.attr(dvbisld.a_originalNetworkID) && SLR_SchemaVersion(props.namespace) >= slVersions.r6) {
			...
			if (ApplicationDelivery) {
				...
				while ((ApplicationType = ApplicationDelivery.getAnyNs(dvbisld.e_ApplicationType, ++at)) != null) {
					const Application_contentType = ApplicationType.attrAnyNsValueOr(dvbi.a_contentType, null);
					if (Application_contentType) {
						...
						if (Application_contentType == dvbi.XML_AIT_CONTENT_TYPE) {
							...
							if (!xmlAitApplicationType)
								errs.addError({
									code: `${errCode}-48`,
									message: `${dvbisld.a_xmlAitApplicationType.attribute()} must be specified when ${dvbi.a_contentType.attribute()}="${dvbi.XML_AIT_CONTENT_TYPE}"`,
									fragment: ApplicationType,
		...
		let l = 0,
			Language;
		while ((Language = Offering.getAnyNs(dvbi.e_Language, ++l)) != null) ValidateLanguage(Language.content, errs, `${errCode}l`, Language.line);
		...
		let c = 0,
			TargetCountry;
		while ((TargetCountry = Offering.getAnyNs(dvbi.e_TargetCountry, ++c)) != null)
		...
		let rm = 0,
			RelatedMaterial;
		while ((RelatedMaterial = Offering.getAnyNs(dvbi.e_RelatedMaterial, ++rm)) != null) {
			this.#ValidateRelatedMaterial(props, RelatedMaterial, errs, `${errCode}-60`);
		}
	...
	/*public*/ doValidateServiceListRegistry(SLRtext, errs, options = {}) {
		...
		if (SLR_SchemaVersion(props.namespace) >= slVersions.r3) slrRequiredAttributes.push(tva.a_lang);
		if (SLR_SchemaVersion(props.namespace) >= slVersions.r6) slrOptionalAttributes.push(dvbi.a_version);
		...
		while ((ProviderOffering = ServiceListRegistry.getAnyNs(dvbisld.e_ProviderOffering, ++p)) != null) {
			...
			let slo = 0,
				ServiceListOffering;
			while ((ServiceListOffering = ProviderOffering.getAnyNs(dvbisld.e_ServiceListOffering, ++slo)) != null)
				this.#CheckServiceListOffering(props, ServiceListOffering, errs, "SR082");
		}
