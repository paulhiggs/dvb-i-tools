...
import { slVersions, dvbi, dvbiEA, dvbiEC, dvbisld, validApplicationTypes, DvbApplicationType } from "./DVB-I_definitions.mjs";
				...
				while ((ApplicationType = ApplicationDelivery.getAnyNs(dvbisld.e_ApplicationType, ++at)) != null) {
					...
					if (Application_contentType) {
						...
						if (Application_contentType == dvbi.XML_AIT_CONTENT_TYPE) {
							...
							if (xmlAitApplicationType && !DvbApplicationType.includes(xmlAitApplicationType))
								errs.addError({
									code: `${errCode}-49`,
									...
									key: "invalid value",
									clause: "A177 Table 12h",
									description: `If @contentType="${dvbi.XML_AIT_CONTENT_TYPE}", the ${dvbisld.a_xmlAitApplicationType.attribute()} attribute shall be included with a value as defined in ETSI TS 102 809, clauses 5.2.2, 5.4.4.4 and 5.4.4.11, i.e. [${DvbApplicationType.join(",")}]`,
								});
						}
					}
				}
