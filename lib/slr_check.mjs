...
import { parameterCheck, isIn } from "./utils.mjs";
...
export default class ServiceListRegistryCheck {
	...
	/* private */ #CheckServiceListOffering(Offering, defined_policies, errs, errCode) {
		...
		Offering.forEachNamedChildElement(dvbisld.e_ServiceListURI, (ListURI) => {
			...
			const ListURI_verificationPolicy = ListURI.attrAnyNsValueOr(dvbisld.a_verificationPolicy);
			if (ListURI_verificationPolicy && !isIn(defined_policies, ListURI_verificationPolicy))
				errs.addError({
					type: WARNING,
					code: `${errCode}-36`,
					message: `${dvbisld.a_verificationPolicy.attribute()}=${ListURI_verificationPolicy.quote()} does not reference a defined signature policy`,
					fragment: ListURI,
	...
	/*public*/ doValidateServiceListRegistry(SLRtext, errs, options = {}) {
		...
		const signaturePolicyIDs = ValidateSignaturePolicies(ServiceListRegistry, errs, "SR060");
		...
		ServiceListRegistry.forEachNamedChildElement(dvbisld.e_ProviderOffering, (ProviderOffering) => {
			...
			ProviderOffering.forEachNamedChildElement(dvbisld.e_ServiceListOffering, (ServiceListOffering) => {
				this.#CheckServiceListOffering(ServiceListOffering, signaturePolicyIDs,errs, "SR082");
			});
		});
