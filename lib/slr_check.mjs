...
export default class ServiceListRegistryCheck {
	...
	/* private */ #ValidateRelatedMaterial(props, RelatedMaterial, errs, errCode) {
		...
		if (HowRelated.attrAnyNs(tva.a_href) && !validServiceListLogo(HowRelated, props.namespace))
			errs.addError({
				code: `${errCode}-6`,
				fragment: HowRelated,
	...
	/* private */ #CheckOrganisationType(Organisation, errs, errCode) {
		...
		checkTopElementsAndCardinality(Organisation, [], [], false, errs, errCode);
	}
	...
	/* private */ #CheckServiceListOffering(props, Offering, errs, errCode) {
		...
		while ((ListURI = Offering.getAnyNs(dvbisld.e_ServiceListURI, ++u)) != null) {
			...
			let URI = ListURI.getAnyNs(dvbisld.e_URI);
			if (URI) {
