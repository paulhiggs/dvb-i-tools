	...
	/* private */ #CheckServiceListOffering(Offering, errs, errCode) {
		...
		Offering.forEachNamedChildElement(dvbisld.e_ServiceListURI, (ListURI) => {
			...
			if (ListURI_contentType && a177versionFromURN(ListURI_standardVersion) >= slVersions.r7 && ListURI_contentType != dvbi.XML_APP) {
				errs.addError({
					code: `${errCode}-35`,
					message: `${dvbisld.a_contentType.attribute()} must be "${dvbi.XML_APP}" for A177r7 and above`,
					fragment: ListURI,
					key: "invalid value",
					clause: "A177 Table 12",
					description:
						...
						if (Application_contentType == dvbi.XML_AIT_CONTENT_TYPE) {
							...
							if (xmlAitApplicationType) {
								...
								errs.addError({
									...
									description: `If @contentType="${dvbi.XML_AIT_CONTENT_TYPE}", the ${dvbisld.a_xmlAitApplicationType} attribute shall be included ...`,
								});
						}
					}
				});
