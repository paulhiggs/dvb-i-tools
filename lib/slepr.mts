...
let REGISTRY_ERROR = (message : string, document : string) => `<Error><Message><![CDATA[${message}]]></Message>\n<SLRDocument><![CDATA[\n${document}]]>\n</SLRDocument></Error>`;
...
export default class SLEPR {
	...
	#readError : string | NodeJS.ErrnoException | null;

	constructor(useURLs : boolean, SLRmode : string, preloadedLanguageValidator? : IANAlanguages, preloadedCountries? : ISOcountries, preloadedGenres? : ClassificationScheme) {
		this.#numRequests = 0;
	...
	stats() : any{
		let res : any = {
			...
			mumGenres: this.#knownGenres?.count() || "unknown",
			numCountries: this.#knownCountries?.count() || "unknown",
	...
	/* public */ loadDataFiles(useURLs : boolean, preloadedLanguageValidator? : IANAlanguages, preloadedCountries? : ISOcountries, preloadedGenres? : ClassificationScheme) {
	...
	/* public */ loadServiceListRegistry(filename : string) {
		...
		if (isHTTPURL(filename)) {
			...
			readFile(filename, { encoding: "utf-8" }, function (err: NodeJS.ErrnoException | null, data: string) {
	...
	/* private */ #checkQuery(req, params) {
		...
		if (req.query) {
			...
			let checkBoolean = (bool : string) : boolean => ["true", "false"].includes(bool);
			checkIt(params.regulatorListFlag, dvbisld.a_regulatorListFlag, checkBoolean);

			//TargetCountry(s)
			let checkTargetCountry = (country : string)  : boolean => this.#knownCountries.isISO3166code(country, false);
			checkIt(params.TargetCountry, dvbi.e_TargetCountry, checkTargetCountry);

			//Language(s)
			let checkLanguage = (language : string) : boolean => isTVAAudioLanguageType(language, false);
			checkIt(params.Language, dvbi.e_Language, checkLanguage);

			//DeliverySystems(s)
			let checkDelivery = (system : string) : boolean => [DVB_DASH_DELIVERY, DVB_T_DELIVERY, DVB_S_DELIVERY, DVB_C_DELIVERY, DVB_IPTV_DELIVERY, DVB_APPLICATION_DELIVERY].includes(system);
			checkIt(params.Delivery, dvbi.e_Delivery, checkDelivery);

			// Genre(s)
			let checkGenre = (genre : string) : boolean => this.#knownGenres.isIn(genre);
