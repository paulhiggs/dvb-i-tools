...
const allowed_arguments : Array<string> = [dvbi.e_ProviderName, dvbisld.a_regulatorListFlag, dvbi.e_Language, dvbi.e_TargetCountry, dvbi.e_Genre, dvbi.e_Delivery, dvbisld.q_inlineImages];

const DVB_DASH_DELIVERY : string = "dvb-dash",
	DVB_T_DELIVERY : string = "dvb-t",
	DVB_S_DELIVERY : string = "dvb-s",
	DVB_C_DELIVERY : string = "dvb-c",
	DVB_IPTV_DELIVERY : string = "dvb-iptv",
	DVB_APPLICATION_DELIVERY : string = "application";

function GetChild(element : XmlElement, childName : string, index : number) : XmlElement | undefined {
	let rc : XmlElement | undefined = undefined,
		i : number = 0;
...
export default class SLEPR {
	...
	/* public */ processServiceListRequest(req, res) {
		...
		if (queryParams.regulatorListFlag || queryParams.Language || queryParams.TargetCountry || queryParams.Genre) {
			let prov,
				p = 0,
				servicesToRemove : Array<XmlElement> = [];
			while ((prov = slepr.get("//" + xPath(props.prefix, dvbisld.e_ProviderOffering, ++p), props.schema)) != null) {
				let serv,
					s = 0;
				...
				while ((serv = prov.get(xPath(props.prefix, dvbisld.e_ServiceListOffering, ++s), props.schema)) != null) {
					...
					while ((relatedMaterial = serv.get(xPath(props.datatypes_prefix ? props.datatypes_prefix : props.prefix, dvbi.e_RelatedMaterial, ++rm), props.schema)) != null) {
						...
						while ((mediaLocator = GetChild(relatedMaterial, tva.e_MediaLocator, ++ml)) != null) {
							let mediaUri,
								mu = 0;
							let discardURIs : Array<XmlElement> = [];
