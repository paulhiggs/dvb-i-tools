...
import { datatypeIs, isIn } from "./utils.mts";
...
import { attrAnyNs, attrAnyNsValueOrNull, forEachNamedChildElement, getAnyNs, hasChild } from "../libxml2-wasm-extensions.mts";
...
export default class SLEPR {
	...
	/* public */ processServiceListRequest(req, res) {
		...
		let slepr : XmlDocument = undefined;
		try {
		...
		if (queryParams.ProviderName) {
			...
			while ((prov = getAnyNs(slepr.root, dvbisld.e_ProviderOffering, ++p)) != null) {
				const Provider = getAnyNs(prov, dvbisld.e_Provider);
				let provName : XmlElement,
					n = 0,
					matchedProvider = false;
				while (!matchedProvider && Provider && (provName = getAnyNs(Provider, dvbi.e_Name, ++n)) != null)
		...
		if (this.#processingMode == ITALY_PROCESSING) {
			...
			const hasURIfor = (offering : XmlElement, version : number) => {
				let rc = false;
				forEachNamedChildElement(offering, dvbisld.e_ServiceListURI, (uri) => {
					const vers = attrAnyNsValueOrNull(uri, dvbisld.a_standardVersion);
			...
			while ((prov = getAnyNs(slepr.root, dvbisld.e_ProviderOffering, ++p)) != null) {
				let serv : XmlElement,
					s = 0;
				while ((serv = getAnyNs(prov, dvbisld.e_ServiceListOffering, ++s)) != null) {
					let uri : XmlElement,
						u = 0;
					const explicitUri = hasURIfor(serv, requestedVersion);
					while ((uri = getAnyNs(serv, dvbisld.e_ServiceListURI, ++u)) != null) {
						let remove_uri = false;
						// UAS is unspecified, remove version specific URI
						if (requestedVersion == -1 && attrAnyNs(uri, dvbisld.a_standardVersion)) remove_uri = true;
					...
					if (!hasChild(serv, dvbisld.e_ServiceListURI)) {
						serv.remove();
						s--;
					}
				}
				if (!hasChild(prov, dvbisld.e_ServiceListOffering)) {
					prov.remove();
					p--;
				}
			}
		...
		if (queryParams.regulatorListFlag || queryParams.Language || queryParams.TargetCountry || queryParams.Genre || requestedVersion != -1) {
			let prov: XmlElement,
				p = 0;
			const servicesToRemove : XmlElement[] = [];
			while ((prov = getAnyNs(slepr.root, dvbisld.e_ProviderOffering, ++p)) != null) {
				let serv : XmlElement,
					s = 0;
				while ((serv = getAnyNs(prov, dvbisld.e_ServiceListOffering, ++s)) != null) {
					...
					if (queryParams.regulatorListFlag) {
						...
						let flag = attrAnyNs(serv, dvbisld.a_regulatorListFlag) ? attrAnyNs(serv, dvbisld.a_regulatorListFlag).value : "false";
					...
					if (!removeService && this.#processingMode == DEFAULT_PROCESSING && requestedVersion != -1) {
						...
						let ServiceListURI : XmlElement,
							u = 0,
							keepService = false;
						while (!keepService && (ServiceListURI = getAnyNs(serv, dvbisld.e_ServiceListURI, ++u)) != null) {
							const specifiedVersion = attrAnyNs(ServiceListURI, dvbisld.a_standardVersion);
					...
					if (!removeService && queryParams.Language) {
						let lang,
							...
							keepService = false,
							hasLanguage = false;
						while (!keepService && (lang = getAnyNs(serv, dvbi.e_Language, ++l)) != null) {
					...
					if (!removeService && queryParams.TargetCountry) {
						let targetCountry : XmlElement,
							c = 0,
							keepService = false,
							hasCountry = false;
						while (!keepService && (targetCountry = getAnyNs(serv, dvbi.e_TargetCountry, ++c))) {
					...
					if (!removeService && queryParams.Genre) {
						let genre : XmlElement,
							g = 0,
							keepService = false,
							hasGenre = false;
						while (!keepService && (genre = getAnyNs(serv, dvbi.e_Genre, ++g))) {
					...
					if (!removeService && queryParams.Delivery) {
						const delivery = getAnyNs(serv, dvbi.e_Delivery);
		...
		let prov : XmlElement,
			p = 0;
		const providersToRemove : XmlElement[] = [];
		while ((prov = getAnyNs(slepr.root, dvbisld.e_ProviderOffering, ++p)) != null) {
			if (!getAnyNs(prov, dvbisld.e_ServiceListOffering)) providersToRemove.push(prov);
		}
		...
		if (!removeImages) {
			...
			while ((prov = getAnyNs(slepr.root, dvbisld.e_ProviderOffering, ++p)) != null) {
				let serv : XmlElement,
					s = 0;
				while ((serv = getAnyNs(prov, dvbisld.e_ServiceListOffering, ++s)) != null) {
					let relatedMaterial : XmlElement,
						rm = 0;
					const discardRelatedMaterial : XmlElement[] = [];
					while ((relatedMaterial = getAnyNs(serv, dvbi.e_RelatedMaterial, ++rm)) != null) {
						let mediaLocator : XmlElement,
							ml = 0;
						const discardLocators : XmlElement[] = [];
						while ((mediaLocator = getAnyNs(relatedMaterial, tva.e_MediaLocator, ++ml)) != null) {
							let mediaUri : XmlElement,
								mu = 0;
							const discardURIs : XmlElement[] = [];
							while ((mediaUri = getAnyNs(mediaLocator, tva.e_MediaUri, ++mu)) != null)
