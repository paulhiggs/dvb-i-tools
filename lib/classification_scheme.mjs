...
export default class ClassificationScheme {
	...
	#loadFromFile(classificationScheme, async = true, verbose = true) {
		verbose && console.log(chalk.yellow(`reading CS from ${classificationScheme}`));
	 ...
	 */
	loadCS(options, async = true, verbose = true, extra_vals = null) {
		if (!options) options = {};

		if (options.file) this.#loadFromFile(options.file, async, verbose);
		if (options.files) options.files.forEach((file) => this.#loadFromFile(file, async, verbose));
		if (options.url) this.#loadFromURL(options.url, async, verbose);
		if (options.urls) options.urls.forEach((url) => this.#loadFromURL(url, async, verbose));
	 ...
	 * 
	 * @deprecated  Use has() instead - it is better aligned with Set() and Map() methods
	 */
	isIn(value) {
