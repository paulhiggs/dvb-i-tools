...
import { datatypeIs, parameterCheck } from "./utils.mjs";

import { tva, tvaEA, tvaEC, BaseAccessibilityAttributesType } from "./TVA_definitions.mjs";
import { dvbi, cgVersions } from "./DVB-I_definitions.mjs";
...
import { CG_SchemaVersion } from "./cg_check.mjs";
...
const checkPurpose = (elem, mainTerm, cs, errs, errCode, errNum) => {
	elem?.forEachNamedChildElement(tva.e_Purpose, (purpose) => {
		const purposeTerm = purpose.attrAnyNsValueOr(tva.a_href);
		if (purposeTerm) {
			if (!cs.AccessibilityPurposeCS.isLeaf(purposeTerm))
				errs.addError({
...
const checkAppInformation = (elem, errs, errCode, errNum) => {
	const appInfo = elem.getAnyNs(tva.e_AppInformation);
	if (appInfo == null) return false; // AppInformation element is not present
	appInfo?.forEachNamedChildElement([tva.e_RequiredStandardVersion, tva.e_RequiredOptionalFeature], (child) => {
		switch (child.name) {
...
const checkCS = (elem, childName, cs, leafsOnly, errs, errCode, errNum, storage = null) => {
	let rc = true;
...
const checkSignLanguage = (elem, childName, cs, errs, errCode, errNum) => {
	elem?.forEachNamedChildElement(childName, (child) => {
...
const checkLanguageFmt = (elem, childName, errs, errCode, errNum) => {
	elem?.forEachNamedChildElement(childName, (child) => {
		ValidateLanguage(child.content, errs, `${errCode}-${errNum}`, child.line);
	});
};

const checkLanguageAttributes = (elem, childName, errs, errCode, errNum) => {
	elem?.forEachNamedChildElement(childName, (child) => {
...
const checkAudioAttributes = (elem, childName, cs, errs, errCode, errNum, allowPurpose = false, disallowClause = null) => {
	elem?.forEachNamedChildElement(childName, (child) => {
		...
		child?.forEachNamedChildElement([tva.e_Coding, tva.e_MixType], (child2) => {
			...
			switch (child2.name) {
				case tva.e_Coding:
					...
					if (href && !cs.AudioCodecCS.isLeaf(href))
						errs.addError({
				...
				case tva.e_MixType:
					...
					if (href && !cs.AudioPresentationCS.has(href))
						errs.addError({
...
export default function CheckAccessibilityAttributes(AccessibilityAttributes, cs, errs, errCode) {
	...
	AccessibilityAttributes?.forEachChildElement((child) => {
		switch (child.name) {
			case tva.e_MagnificationUIAttributes:
				...
				checkPurpose(child, "1", cs, errs, errCode, 13);
				break;
			case tva.e_HighContrastUIAttributes:
				...
				checkPurpose(child, "2", cs, errs, errCode, 23);
				break;
			case tva.e_ScreenReaderAttributes:
				...
				checkPurpose(child, "3", cs, errs, errCode, 33);
			...
			case tva.e_ResponseToUserActionAttributes:
				...
				checkPurpose(child, "4", cs, errs, errCode, 43);
				break;
			case tva.e_SubtitleAttributes:
				// eslint-disable-next-line no-case-declarations
				const carriages = [],	codings = [];
				checkTopElementsAndCardinality(
					child,
					[
						{ name: tva.e_Carriage },
			...
			case tva.e_AudioDescriptionAttributes:
				...
				checkAudioAttributes(child, tva.e_AudioAttributes, cs, errs, errCode, 63, false, "A177 clause 4.5.2.4");
				break;
			case tva.e_SigningAttributes:
				...
				if (child.hasChild(tva.e_Coding)) {
					...
					if (AVparent.hasChild(tva.e_VideoAttributes) && ClosedSigning == "false") {
						...
						const Video_format = AVparent.getAnyNs(tva.e_VideoAttributes)?.getAnyNs(tva.e_Coding)?.attrAnyNsValueOr(tva.a_href);
						if (Signer_format && Video_format && Signer_format != Video_format) {
							errs.addError({
								...
								fragments: [AVparent.getAnyNs(tva.e_VideoAttributes)?.getAnyNs(tva.e_Coding), child.getAnyNs(tva.e_Coding)],
							});
						}
					}
				}
				checkSignLanguage(child, tva.e_SignLanguage, cs, errs, errCode, 75);
				break;
			case tva.e_DialogueEnhancementAttributes:
				...
				checkAudioAttributes(child, tva.e_AudioAttributes, cs, errs, errCode, 83, false, "A177 clause 4.5.2.5");
				break;
			case tva.e_SpokenSubtitlesAttributes:
				...
				checkAudioAttributes(child, tva.e_AudioAttributes, cs, errs, errCode, 93, false, "A177 clause 4.5.2.6");
				break;
		}
	});
}
