...
export default function CheckAccessibilityAttributes(AccessibilityAttributes, cs, errs, errCode) {
	...
	const checkPurpose = (elem, mainTerm, errNum) => {
	...
	const checkAppInformation = (elem, errNum) => {
	...
	const checkCS = (elem, childName, cs, leafsOnly, errNum, storage = null) => {
		let rc = true;
	...
	const checkSignLanguage = (elem, childName, errNum) => {
		elem?.forEachNamedChildElement(childName, (child) => {
	...
	const checkLanguageFmt = (elem, childName, errNum) => {
		elem?.forEachNamedChildElement(childName, (child) => {
	...
	const checkLanguagePurpose = (elem, childName, errNum) => {
		elem?.forEachNamedChildElement(childName, (child) => {
			...
			checkAttributes(child, [], [tva.a_purpose], tvaEA.AudioLanguage, errs, `${errNum}2`);
		});
	};

	const checkAudioAttributes = (elem, childName, errNum, allowPurpose = false, disallowClause = null) => {
		elem?.forEachNamedChildElement(childName, (child) => {
			checkTopElementsAndCardinality(
				child,
				[
	...
	AccessibilityAttributes?.forEachChildElement((child) => {
		switch (child.name) {
			...
			case tva.e_SubtitleAttributes:
				...
				// eslint-disable-next-line no-case-declarations
				const hasAppInformation = checkAppInformation(child, 52);
			...
			case tva.e_SigningAttributes:
				...
				if (!checkCS(child, tva.e_Coding, cs.VideoCodecCS, true, 73))
