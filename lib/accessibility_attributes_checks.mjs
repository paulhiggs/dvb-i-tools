...
export default function CheckAccessibilityAttributes(AccessibilityAttributes, cs, errs, errCode) {
	...
	let checkPurpose = (elem, mainTerm, errNum) => {
		elem?.childNodes().forEachNamedSubElement(tva.e_Purpose, (e) => {
			const purposeTerm = e.attrAnyNsValueOr(tva.a_href, null)
			if (purposeTerm) {
	...
	let checkCS = (elem, childName, cs, errNum, storage = null) => {
		...
		elem?.childNodes().forEachNamedSubElement(childName, (e) => {
			let href = e.attrAnyNsValueOr(tva.a_href, null);
	...
	let checkLanguagePurpose = (elem, childName, errNum) => {
		...
		elem?.childNodes().forEachNamedSubElement(childName, (e) => {
			const purpose = e.attrAnyNsValueOr(tva.a_purpose, null);
			if (purpose && !cs.AudioPurposeCS.isIn(purpose))
				errs.addError({
					code: errNum,
					fragment: e,
					message: `"${purpose}" is not valid for ${elem.name.elementize()}${e.name.elementize()}`,
					key: ACCESSIBILITY_CHECK_KEY,
				});
		});
	};
	...
	let checkAudioAttributes = (elem, childName, errNum) => {
		elem?.childNodes().forEachNamedSubElement(childName, (e) => {
			...
			e?.childNodes().forEachNAmedSubElement([tva.e_Coding, tva.e_MixType], (e2) => {
				const href = e2.attrAnyNsValueOr(tva.a_href, null);
				switch (e2.name) {
					case tva.e_Coding:
						...
						if (href && !cs.AudioCodecCS.isIn(href))
							errs.addError({
								...
								fragment: e2,
								message: `"${href}" is not valid for ${elem.name.elementize()}${e.name.elementize()}${e2.name.elementize()}`,
								key: ACCESSIBILITY_CHECK_KEY,
							});
						break;
					case tva.e_MixType:
