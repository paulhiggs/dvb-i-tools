...
const checkLanguagePurpose = (elem, childName, errs, errCode, errNum) => {
	elem?.forEachNamedChildElement(childName, (child) => {
		...
		// fake that @purpose is allowed as it is already checked above
		checkAttributes(child, [], [tva.a_purpose], tvaEA.AudioLanguage, errs, `${errCode}-${errNum}2`);
	});
};
...
const checkAudioAttributes = (elem, childName, cs, errs, errCode, errNum, allowPurpose = false, disallowClause = null) => {
	elem?.forEachNamedChildElement(childName, (child) => {
		...
		checkLanguagePurpose(child, tva.e_AudioLanguage, errs, errCode, `${errNum}c`);
	...
	AccessibilityAttributes?.forEachChildElement((child) => {
		switch (child.name) {
			...
			case tva.e_SigningAttributes:
				...
				checkSignLanguage(child, tva.e_SignLanguage, cs, errs, errCode, 74);
				break;
