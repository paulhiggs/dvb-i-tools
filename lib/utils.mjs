...
/* local */
const findInSet = (values, value, caseSensitive) => {
 ...
 */
export const isIn = (values, value, caseSensitive = true) => findInSet(values, value, caseSensitive);
 ...
 * @return {boolean} true if value is in the set of values
 */
export const isIni = (values, value) => findInSet(values, value, false);
 ...
 */
export const unEntity = (string) => string.replace(/(&.+;)/gi, "*");
 ...
 * @param {String[] | String} expectedType  the name of names that the @node needs to be 
 ...
 */
export function parameterCheck(functionName, node, expectedType, errs, errcode) {
	if (!node) {
		errs.addError({
			type: APPLICATION,
			code: `${errcode}-a`,
	...
	if (!(node instanceof XmlElement)) {
		errs.addError({
			type: APPLICATION,
			code: `${errcode}-b`,
			message: `${functionName}() called with type ${node.constructor.name} instead of XmlElement`,
		});
		return false;
	}
	if (expectedType)
...
export const quote = (str) => str.quote();
...
export const elementize = (str) => str.elementize();
...
export const attribute = (attr, elem = "") => attr.attribute(elem);
...
export const HTMLize = (str) => str.HTMLize();
...
export function containsHex(str) {
	if (!str) return false;
	str = str.toLowerCase();
...
export function HexOrDecValue(str) {
	const res = str.match(_HexRegexp);
	return parseInt(res?.groups?.value, res?.groups?.hex_ind == "0x" || containsHex(res?.groups?.value) ? 16 : 10);	
}
...
export const DefaultProperty = (obj, prop, defaultValue) => {
