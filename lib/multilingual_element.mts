...
import { attrAnyNs, forEachNamedChildElement, documentNamespace, countChildElements } from "../libxml2-wasm-extensions.mts";
 ...
 */
export function checkLanguage(lang : string, element : XmlElement, errs : ErrorList, errCode : string) {
...
export function mlLanguage(node : XmlElement) : string {
	...
	const langAttr = attrAnyNs(node, tva.a_lang);
...
export function checkXMLLangs(elementName : string, elementLocation : string, node : XmlElement, errs : ErrorList, errCode : string) {
	...
	if (SL_SchemaVersion(documentNamespace(node)) >= slVersions.r5 && countChildElements(node, elementName) > 1)
		forEachNamedChildElement(node, elementName, (child) => {
			if (!attrAnyNs(child, tva.a_lang))
				errs.addError({
					code: `${errCode}-1`,
	...
	forEachNamedChildElement(node, elementName, (child) => {
		...
		if (attrAnyNs(child, tva.a_lang) && lang != NO_DOCUMENT_LANGUAGE) checkLanguage(lang, child, errs, `${errCode}-4`);
	});
}
...
export function GetNodeLanguage(node : XmlElement, isRequired : boolean, errs : ErrorList, errCode : string) : string {
	...
	if (isRequired && !attrAnyNs(node, tva.a_lang))
	...
	if (attrAnyNs(node, tva.a_lang) && localLang != NO_DOCUMENT_LANGUAGE) checkLanguage(localLang, node, errs, `${errCode}-2`);
	return localLang;
}
