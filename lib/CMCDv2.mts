...
const supported_CMCD_versions: number[] = [1, 2];

const CMCD_keys: Record<string, string> = {
...
const CMCD_EVENT_TYPES: string[] = ["abs","abe","ae","as","b","bc","c","ce","e","h","m","pc","pe","pr","ps","rr","sk","t","um"];
const CMCD_OBJECT_TYPES: string[] = ["m", "a", "v", "av", "i", "c", "tt", "k", "o"];

const request_mode_only: string[] = [CMCD_MODE_REQUEST],
	event_mode_only: string[] = [CMCD_MODE_EVENT],
	request_and_event_modes: string[] = [CMCD_MODE_REQUEST, CMCD_MODE_EVENT];
...
const isCustomKey = (key: string) => key.includes("-");
const reportingMode = (mode: string) => (mode.indexOf(":") != -1 ? mode.substring(mode.lastIndexOf(":") + 1) : "***");
...
function checkCMCDkeys(Report: XmlElement, version: number, errs: ErrorList, errCode: string) {
	...
	if (configured_keys && configured_keys.length > 0) {
		...
		configured_keys.split(" ").forEach((key) => {
			...
			if (reserved_key) {
				if (!reserved_key.allow_modes.includes(reporting_mode))
					errs.addError({
						code: `${errCode}-01`,
	...
	if (configured_keys_array && reporting_mode == CMCD_MODE_EVENT) {
		...
		if (!configured_keys_array.includes(CMCD_keys.event)) 
			errs.addError({
				code: `${errCode}-22`,
...
function ValidateKeys(Report: XmlElement, CMCDversion: number, errs: ErrorList, errCode: string) {
	...
	checkCMCDkeys(Report, CMCDversion, errs, `${errCode}b`);
}
...
function ValidateAttributes(Report: XmlElement, CMCDversion: number, counts: CountsAccumulator, errs: ErrorList, errCode: string) {
	...
	if (objectTypes) {
		...
		objectTypes_array.forEach((objectType) => {
			if (objectType.length != 0 && !CMCD_OBJECT_TYPES.includes(objectType))
				errs.addError({
					code: `${errCode}a`,
	...
	if (eventTypes) {
		...
		eventTypes_array.forEach((eventType) => {
			if (eventType.length != 0 && !CMCD_EVENT_TYPES.includes(eventType))
				errs.addError({
					code: `${errCode}b`,
	...
	if (reporting_mode && supported_CMCD_versions.includes(CMCDversion) && HasProperty(counts, reportingMode(reporting_mode))) {
...
function check_CMCD(CMCDelem: XmlElement, counts: CountsAccumulator, errs: ErrorList) {
	...
	switch (CMCDversion) {
		case 1:
			ValidateCMCDv1Element(CMCDelem, counts, errs, "CD011");
			break;
		case 2:
			ValidateCMCDv2Element(CMCDelem, counts, errs, "CD012");
			break;
	}
