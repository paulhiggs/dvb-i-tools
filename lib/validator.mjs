...
import { drawForm, PAGE_TOP, PAGE_BOTTOM, drawResults, LINE } from "./ui.mjs";
...
import PlaylistCheck from "./playlist_check.mjs";
...
import { MODE_URL, MODE_FILE, MODE_SL, MODE_PL, MODE_CG, MODE_SLR, MODE_UNSPECIFIED } from "./ui.mjs";
...
function DVB_I_check(req, res, slcheck, plcheck, cgcheck, slrcheck, hasSL, hasPL, hasCG, hasSLR, motd, mode = MODE_UNSPECIFIED, linktype = MODE_UNSPECIFIED) {
	...
	const FormArguments = { cg: MODE_CG, sl: MODE_SL, pl: MODE_PL, slr: MODE_SLR, file: MODE_FILE, url: MODE_URL, hasSL: hasSL, hasPL: hasPL, hasCG: hasCG, hasSLR: hasSLR };
 ...
 * @param {Express request}  req           The Express request that triggered the validation
 * @param {Express response} res           The Express response to be written to the requester
 * @param {ServiceListCheck} plcheck       Initialised play List validator
 * @param {String}           motd          HTML text for the Message Of The Day
 * @param {boolean}          jsonResponse  Flag indicating that the response should ne JSON format rather than HTML
 */
...
export default function validator(options) {
	...
	if (!options.nosl || !options.nopl || !options.nocg || !options.noslr) {
		...
		if (!options.nopl)
			plcheck = new PlaylistCheck(options.urls, {});
		...
		if (!options.noslr) slrcheck = new ServiceListRegistryCheck(options.urls, { 
			countries: isoCountries, 
			genres: knownGenres, 
			languages: knownLanguages, 
			appfmts: linkedApplicationTypes,
	...
	if (!options.nopl) {
		app.all("/validate_pl", express.text({ type: "application/xml", limit: "10mb" }), (req, res) => {
			validatePlaylist(req, res, plcheck, motd, false);
		});

		app.all("/validate_pl_json", express.text({ type: "application/xml", limit: "10mb" }), (req, res) => {
	...
	app.get("/stats", (req, res) => {
		...
		tabulate(res, "CSR", csr || InactiveFeatureStats);
		tabulate(res, "SL", slcheck || InactiveFeatureStats);
		tabulate(res, "PL", plcheck || InactiveFeatureStats);
		tabulate(res, "SLR", slrcheck || InactiveFeatureStats);
		tabulate(res, "CG", cgcheck || InactiveFeatureStats);
