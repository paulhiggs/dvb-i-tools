...
export default function validator(options) {
	...
	token("vary", (req, res) => {
		let unique = res.varyOn.filter((value, index, array) => array.indexOf(value) === index);
	...
	app.use(morgan(":remote-addr :protocol :method :url :status :res[content-length] :counts - :response-time ms :agent :vary :parseErr :location"));
