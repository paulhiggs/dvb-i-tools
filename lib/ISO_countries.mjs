...
export default class ISOcountries {
	 ...
	 */
	#loadCountriesFromFile(countriesFile, purge = false, async = true, verbose = true) {
		verbose && console.log(chalk.yellow(`reading countries from ${countriesFile}`));
		if (purge) this.reset();
		if (async)
			readFile(
				countriesFile,
	 ...
	 * @param {boolean} purge         erase the existing values before loading new
	 */
	#loadCountriesFromURL(countriesURL, purge = false, async = true, verbose = true) {
		const isHTTPurl = isHTTPURL(countriesURL);
		verbose && console.log(chalk.yellow(`${isHTTPurl ? "" : "--> NOT "}retrieving countries from ${countriesURL} using fetch()`));
		if (!isHTTPurl) return;
	...
	loadCountries(options, async = true, verbose = true) {
		if (!options) options = {};
		if (!options.purge) options.purge = true;

		if (options.file) this.#loadCountriesFromFile(options.file, options.purge, async, verbose);
		else if (options.url) this.#loadCountriesFromURL(options.url, options.purge, async, verbose);
	}
