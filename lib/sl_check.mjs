...
import SL_helpers from "./sl_check_helpers.mjs"

import { tva, tvaEA } from "./TVA_definitions.mjs";
import { sats } from "./DVB_definitions.mjs";
import { slVersions, dvbi, dvbisld, dvbiEC, dvbiEA, XMLdocumentType} from "./DVB-I_definitions.mjs";

import ErrorList, { WARNING, APPLICATION } from "./error_list.mjs";
import { isIn, unEntity, DuplicatedValue, parameterCheck } from "./utils.mjs";
import { isPostcode, isASCII, isHTTPURL, isHTTPPathURL, isRTSPURL, isTAGURI, isUUIDformat } from "./pattern_checks.mjs";
import { checkValidLogos } from "./related_material_checks.mjs";
import { sl_InvalidHrefValue, InvalidURL, DeprecatedElement, keys, InvalidCountryCode } from "./common_errors.mjs";
...
import { DASH_IF_Content_Protection_List, UUID_type, CA_SYSTEM_ID_REGISTRY, KnownDRMScheme, KnownCASystemID} from "./identifiers.mjs";
import { CheckDelivery } from "./slr_check.mjs";
import { LoadSLschemas, SL_GetSchema, SL_SchemaVersion, SL_SchemaSpecVersion, isA177specification_URN, isContentFinishedBanner, isOutScheduleHours } from "./sl_data_versions.mjs";
...
import { ValidateCMCDinDASH } from "./CMCDv2.mjs";
import { CMCD_custom_stats, LoadKnownCustomKeysRegistry } from "./CMCD_custom_keys.mjs";
...
import { ValidateAnyContentDigests, ValidateAnySignaturePolicy } from "./related_material_checks.mjs";
import { ValidateSignaturePolicies } from "./signature_policies.mjs";
...
export default class ServiceListCheck {
	...
	constructor(useURLs, opts) {
		...
		if (!opts) opts = {};
		if (!Object.prototype.hasOwnProperty.call(opts, "async")) opts.async=true;
		if (!Object.prototype.hasOwnProperty.call(opts, "verbose")) opts.verbose=true;

		LoadSLschemas(useURLs, opts.async, opts.verbose);

		this.#numRequests = 0;

		this.#knownCountries = opts?.countries || LoadCountries(useURLs, opts.async, opts.verbose);
		this.#knownLanguages = opts?.languages || LoadLanguages(useURLs, opts.async, opts.verbose);

		opts.verbose && console.log(chalk.yellow.underline("SL: loading classification schemes..."));
		this.#accessibilityPurposes = opts?.accessibilities || LoadAccessibilityPurpose(useURLs, opts.async, opts.verbose);
		this.#allowedAudioSchemes = opts?.audiofmts || LoadAudioCodecCS(useURLs, opts.async, opts.verbose);
		this.#audioPresentations = opts?.audiopres || LoadAudioPresentationCS(useURLs, opts.async, opts.verbose);
		this.#audioPurposes = opts?.audiopurps || LoadAudioPurpose(useURLs, opts.async, opts.verbose);
		this.#allowedGenres = opts?.genres || LoadGenres(useURLs, opts.async, opts.verbose);
		this.#subtitleCarriages = opts?.stcarriage || LoadSubtitleCarriages(useURLs, opts.async, opts.verbose);
		this.#subtitleCodings = opts?.stcodings || LoadSubtitleCodings(useURLs, opts.async, opts.verbose);
		this.#subtitlePurposes = opts?.stpurposes || LoadSubtitlePurposes(useURLs, opts.async, opts.verbose);
		this.#allowedVideoSchemes = opts?.videofmts || LoadVideoCodecCS(useURLs, opts.async, opts.verbose);
		this.#allowedApplicationTypes = opts?.appfmts || LoadLinkedApplicationCS(useURLs, opts.async, opts.verbose);

		this.#allowedPictureFormats = LoadPictureFormatCS(useURLs, opts.async, opts.verbose);
		this.#allowedColorimetry = LoadColorimetryCS(useURLs, opts.async, opts.verbose);
		this.#allowedServiceTypes = LoadServiceTypeCS(useURLs, opts.async, opts.verbose);

		this.#allowedAudioConformancePoints = LoadAudioConformanceCS(useURLs, opts.async, opts.verbose);
		this.#allowedVideoConformancePoints = LoadVideoConformanceCS(useURLs, opts.async, opts.verbose);
		this.#RecordingInfoCSvalues = LoadRecordingInfoCS(useURLs, opts.async, opts.verbose);

		LoadKnownCustomKeysRegistry(useURLs, opts.async, opts.verbose)
	}
	...
	stats() {
		...
		res.numAccessibilityPurposes = this.#accessibilityPurposes?.count();
		...
		res.numSubtitleCariages = this.#subtitleCarriages?.count();
		res.numSubtitleCodings = this.#subtitleCodings?.count();
		res.numSubtitlePurposes = this.#subtitlePurposes?.count();
		res.numAllowedVideoSchemes = this.#allowedVideoSchemes?.count();
		res.numAllowedApplicationTypes = this.#allowedApplicationTypes?.count();
	...
	/*private*/ #addRegion(Region, depth, documentInfo, countries, errs) {
			...
			if (countriesSpecified) {
				...
				countriesSpecified.forEach((country) => {
					if (DuplicatedValue(countriesList, country))
						errs.addError({
							code: "AR034",
							message: `country code ${country.quote()} specified more than once for region ${displayRegionID}`,
							key: "duplicated country code",
							line: Region.line,
						})
					else if (!this.#knownCountries.isISO3166code(country))
						errs.addError({
							code: "AR033",
		...
		if (schemaVersion >= slVersions.r4) {
			...
			if (regionID) {
				if (documentInfo.knownRegionIDs.find((r) => r.region == regionID) != undefined)
					...
					documentInfo.knownRegionIDs.push({
			...
			if (regionID) {
				if (documentInfo.knownRegionIDs.find((r) => r.region == regionID) != undefined)
					...
					documentInfo.knownRegionIDs.push({
						countries: countriesSpecified,
						region: regionID,
						selectable: true,		// in A177r3 and earlier, all regions are selectable
						used: false,
						line: Region.line,
					});
			} else
		...
		// <Region><Postcode>
		Region.forEachNamedChildElement(dvbi.e_Postcode, (Postcode) => {
		...
		Region.forEachNamedChildElement(dvbi.e_Region, (RegionChild) => this.#addRegion(RegionChild, depth + 1, documentInfo, countriesSpecified, errs));
	}
	 ...
	 * @param  {} documentInfo
	 *              signaturePolicyIDs   Signature Verification policies that are defined in the service list
	 * @param {ErrorList}  errs          The class where errors and warnings relating to the service list processing are stored
	 */
	...
	/*private*/ #validServiceApplication(HowRelated, schemaVersion) {
		...
		const val = HowRelated.attrAnyNsValueOr(dvbi.a_href);
			...
			switch (LocationType) {
				case SERVICE_LIST_RM:
					if (validServiceListLogo(HowRelated)) {
						rc = HowRelated_href;
						checkValidLogos(RelatedMaterial, Location, documentInfo, errs, `${errCode}-10`);
					} else if (validServiceAgreementApp(HowRelated)) {
						rc = HowRelated_href;
						MediaLocator.forEach((locator) => this.#checkSignalledApplication(locator, Location, rc, documentInfo,errs));
					} else {
						errs.addError(sl_InvalidHrefValue(HowRelated_href, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-11`));
						errs.errorDescription(SL_helpers.RMErrorDescription(`${errCode}-11`, dvbi.e_ServiceList, 14));
					}
					break;
				...
				case SERVICE_RM:
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNamespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-21`,
							message: `${HowRelated_href.quote()} not permitted for ${documentNamespace.quote()} in ${Location}`,
					...
					if (validOutScheduleHours(HowRelated) || validContentFinishedBanner(HowRelated) || validServiceLogo(HowRelated) || validServiceBanner(HowRelated)) {
						rc = HowRelated_href;
						checkValidLogos(RelatedMaterial, Location, documentInfo, errs, `${errCode}-22`);
						...
						errs.errorDescription(SL_helpers.RMErrorDescription(`${errCode}-24`, dvbi.e_Service, 15));
					}
					break;
				...
				case SERVICE_INSTANCE_RM:
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNamespace) == slVersions.r0)
						...
						rc = HowRelated_href;
						checkValidLogos(RelatedMaterial, Location, documentInfo, errs, `${errCode}-32`);
						...
						errs.errorDescription(SL_helpers.RMErrorDescription(`${errCode}-34`, dvbi.e_ServiceInstance, 16));
					}
					break;
				...
				case CONTENT_GUIDE_RM:
					if (validContentGuideSourceLogo(HowRelated)) {
						rc = HowRelated_href;
						checkValidLogos(RelatedMaterial, Location, documentInfo, errs, `${errCode}-41`);
					} else {
						errs.addError(sl_InvalidHrefValue(HowRelated_href, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-42`));
						errs.errorDescription(SL_helpers.RMErrorDescription(`${errCode}-42`, dvbi.e_ContentGuideSource, 20));
					}
					break;
			}
		}
	...
	/*private*/ #validateAContentGuideSource(source, loc, documentInfo, errs, errCode) {
		...
		source.forEachNamedChildElement(tva.e_RelatedMaterial, (RelatedMaterial) => 
			this.#validateRelatedMaterial(RelatedMaterial, loc, CONTENT_GUIDE_RM, documentInfo, errs, `${errCode}-3`)
		);
	...
	/*private*/ #ValidateSynopsisType(Element, ElementName, requiredLengths, optionalLengths, parentLanguage, errs, errCode) {
		...
		Element.forEachNamedChildElement(ElementName, (ste) => {
			...
			const synopsisLength = ste.attrAnyNsValueOr(tva.a_length);
					...
					switch (synopsisLength) {
						case tva.SYNOPSIS_BRIEF_LABEL:
							if (cleanSynopsisLength > tva.SYNOPSIS_BRIEF_LENGTH)
								errs.addError({
									code: `${errCode}-10`,
									message: SL_helpers.synopsisLengthError(ElementName, tva.SYNOPSIS_BRIEF_LABEL, tva.SYNOPSIS_BRIEF_LENGTH),
									fragment: ste,
									key: ERROR_KEY,
								});
							hasBrief = true;
						...
						case tva.SYNOPSIS_SHORT_LABEL:
							if (cleanSynopsisLength > tva.SYNOPSIS_SHORT_LENGTH)
								errs.addError({
									code: `${errCode}-11`,
									message: SL_helpers.synopsisLengthError(ElementName, tva.SYNOPSIS_SHORT_LABEL, tva.SYNOPSIS_SHORT_LENGTH),
									fragment: ste,
									key: ERROR_KEY,
								});
							hasShort = true;
						...
						case tva.SYNOPSIS_MEDIUM_LABEL:
							if (cleanSynopsisLength > tva.SYNOPSIS_MEDIUM_LENGTH)
								errs.addError({
									code: `${errCode}-12`,
									message: SL_helpers.synopsisLengthError(ElementName,tva.SYNOPSIS_MEDIUM_LABEL, tva.SYNOPSIS_MEDIUM_LENGTH),
									fragment: ste,
									key: ERROR_KEY,
								});
							hasMedium = true;
						...
						case tva.SYNOPSIS_LONG_LABEL:
							if (cleanSynopsisLength > tva.SYNOPSIS_LONG_LENGTH)
								errs.addError({
									code: `${errCode}-13`,
									message: SL_helpers.synopsisLengthError(ElementName, tva.SYNOPSIS_LONG_LABEL, tva.SYNOPSIS_LONG_LENGTH),
									fragment: ste,
									key: ERROR_KEY,
								});
							hasLong = true;
						...
						case tva.SYNOPSIS_EXTENDED_LABEL:
							if (cleanSynopsisLength < tva.SYNOPSIS_EXTENDED_MIN_LENGTH)
								errs.addError({
									code: `${errCode}-14`,
									message: SL_helpers.synopsisToShortError(ElementName, tva.SYNOPSIS_EXTENDED_LABEL, tva.SYNOPSIS_EXTENDED_MIN_LENGTH),
									fragment: ste,
									key: ERROR_KEY,
								});
							hasExtended = true;
			...
			if (synopsisLang && synopsisLength)
				switch (synopsisLength) {
					case tva.SYNOPSIS_BRIEF_LABEL:
						if (DuplicatedValue(briefLangs, synopsisLang))
							errs.addError({
								code: `${errCode}-21`,
								message: SL_helpers.singleLengthLangError(ElementName, synopsisLength, synopsisLang),
								fragment: ste,
								key: ERROR_KEY,
							});
						break;
					case tva.SYNOPSIS_SHORT_LABEL:
						if (DuplicatedValue(shortLangs, synopsisLang))
							errs.addError({
								code: `${errCode}-22`,
								message: SL_helpers.singleLengthLangError(ElementName, synopsisLength, synopsisLang),
								fragment: ste,
						...
						break;
					case tva.SYNOPSIS_MEDIUM_LABEL:
						if (DuplicatedValue(mediumLangs, synopsisLang))
							errs.addError({
								code: `${errCode}-23`,
								message: SL_helpers.singleLengthLangError(ElementName, synopsisLength, synopsisLang),
								fragment: ste,
						...
						break;
					case tva.SYNOPSIS_LONG_LABEL:
						if (DuplicatedValue(longLangs, synopsisLang))
							errs.addError({
								code: `${errCode}-24`,
								message: SL_helpers.singleLengthLangError(ElementName, synopsisLength, synopsisLang),
								fragment: ste,
						...
						break;
					case tva.SYNOPSIS_EXTENDED_LABEL:
						if (DuplicatedValue(extendedLangs, synopsisLang))
							errs.addError({
								code: `${errCode}-25`,
								message: SL_helpers.singleLengthLangError(ElementName, synopsisLength, synopsisLang),
								fragment: ste,
		...
		if (isIn(requiredLengths, tva.SYNOPSIS_BRIEF_LABEL) && !hasBrief)
			errs.addError({
				code: `${errCode}-31`,
				message: SL_helpers.requiredSynopsisError(ElementName, tva.SYNOPSIS_BRIEF_LABEL),
				fragment: Element,
				key: ERROR_KEY,
			});
		if (isIn(requiredLengths, tva.SYNOPSIS_SHORT_LABEL) && !hasShort)
			errs.addError({
				code: `${errCode}-32`,
				message: SL_helpers.requiredSynopsisError(ElementName, tva.SYNOPSIS_SHORT_LABEL),
				fragment: Element,
				key: ERROR_KEY,
			});
		if (isIn(requiredLengths, tva.SYNOPSIS_MEDIUM_LABEL) && !hasMedium)
			errs.addError({
				code: `${errCode}-33`,
				message: SL_helpers.requiredSynopsisError(ElementName, tva.SYNOPSIS_MEDIUM_LABEL),
				fragment: Element,
				key: ERROR_KEY,
			});
		if (isIn(requiredLengths, tva.SYNOPSIS_LONG_LABEL) && !hasLong)
			errs.addError({
				code: `${errCode}-34`,
				message: SL_helpers.requiredSynopsisError(ElementName, tva.SYNOPSIS_LONG_LABEL),
				fragment: Element,
				key: ERROR_KEY,
			});
		if (isIn(requiredLengths, tva.SYNOPSIS_EXTENDED_LABEL) && !hasExtended)
			errs.addError({
				code: `${errCode}-35`,
				message: SL_helpers.requiredSynopsisError(ElementName, tva.SYNOPSIS_EXTENDED_LABEL),
				fragment: Element,
				key: ERROR_KEY,
			});
	}
	...
	/*private*/ #hasServiceApplication(node, href, contentType) {
		...
		while (!rc && (RelatedMaterial = node.getAnyNs(tva.e_RelatedMaterial, ++rm)) != null) {
			...
			if (HowRelated && HowRelated.attrAnyNsValueOr(tva.a_href) == href) {
				let ml = 0,
					MediaLocator;
				while (!rc && (MediaLocator = RelatedMaterial.getAnyNs(tva.e_MediaLocator, ++ml)) != null) {
					...
					if (MediaUri && MediaUri.attrAnyNsValueOr(tva.a_contentType) == contentType) rc = true;
				}
			}
		}
		return rc;
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, documentInfo, errs) {
		...
		if (SL_SchemaVersion(documentNamespace) <= slVersions.r4) {
			const ServiceInstance_prior = ServiceInstance.attrAnyNs(dvbi.a_priority);
		...
		//<ServiceInstance@id>
		const ServiceInstance_id = ServiceInstance.attrAnyNsValueOr(dvbi.a_id);
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_RelatedMaterial, (RelatedMaterial) => {
			...
			if (foundHref == dvbi.APP_IN_CONTROL) {
				...
				const foundDeliveryParams = SL_helpers.deliveryParameters(ServiceInstance);
			...
			if ([dvbi.APP_IN_PARALLEL, dvbi.APP_IN_CONTROL, dvbi.APP_IN_SERIES].includes(foundHref)) {
				...
				RelatedMaterial.forEachNamedChildElement(tva.e_MediaLocator, (MediaLocator) => {
					...
					if (MediaUri) {
						...
						ValidateAnyContentDigests(MediaUri, errs, "SI025");
						ValidateAnySignaturePolicy(MediaUri, documentInfo, errs, "SI026");
					}
				});
			}
		});
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_ContentProtection, (ContentProtection) => {
			checkTopElementsAndCardinality(
				ContentProtection,
				[
			...
			ContentProtection.forEachNamedChildElement(dvbi.e_CASystemId, (CASystemID) => {
			...
			ContentProtection.forEachNamedChildElement(dvbi.e_DRMSystemId, (DRMSystemID) => {
				const DRM_attributes = [dvbi.a_encryptionScheme, dvbi.a_cpsIndex]
								.concat(SL_SchemaVersion(documentNamespace) >= slVersions.r8 ? [dvbi.a_LAURL, dvbi.a_CertificateURL] : []);
				checkAttributes(DRMSystemID, [], DRM_attributes, DRM_attributes, errs, "SI041");
				...
				if (DRMSystemID_value && DRMSystemID_value.length != 0) {
					const systemID = (DRMSystemID_value.startsWith(UUID_type) ? DRMSystemID_value.substring(UUID_type.length) : DRMSystemID_value);
					if (!isUUIDformat(systemID))
						errs.addError({
							code: "SI043",
					...
					if (!KnownDRMScheme(DRMSystemID_value)) 
						errs.addError({
							code: "SI042",
					...
					if (DRMSystemID_LAurl && !isHTTPURL(DRMSystemID_LAurl))
						errs.addError({
							code: "SI044",
							message: `${dvbi.a_LAURL.attribute(dvbi.e_DRMSystemId)} value (${DRMSystemID_LAurl}) is not a valid URL`,
							fragment: DRMSystemID,
							key: "invalid license acquisition URL",
						});
					...
					if (DRMSystemID_Certificateurl && !DRMSystemID_LAurl)
						errs.addError({
							type: WARNING,
							code: "SI046",
		...
		if (ContentAttributes) {
			...
			ContentAttributes.forEachNamedChildElement(tva.e_AudioAttributes, (audio_attrs) => {
				audio_attrs.forEachChildElement((child) => {
					...
					switch (child.name) {
						...
						case tva.e_MixType:
							...
							if (child_href && !this.#audioPresentations.has(child_href))
								errs.addError({
									code: "SI055",
						...
						case tva.e_AudioLanguage:
							...
							const AudioLanguage_purpose = child.attrAnyNsValueOr(tva.a_purpose);
			...
			ContentAttributes.forEachNamedChildElement(dvbi.e_AudioConformancePoint, (conf) => {
				if (SL_SchemaVersion(documentNamespace) > slVersions.r4) 
					errs.addError(DeprecatedElement(conf, SL_SchemaSpecVersion(slVersions.r4), "SI062"));
				const conf_href = conf.attrAnyNsValueOr(dvbi.a_href);
			...
			ContentAttributes.forEachNamedChildElement(tva.e_VideoAttributes, (conf) => {
				conf.forEachChildElement((child) => {
					const child_href = child.attrAnyNsValueOr(dvbi.a_href);
					switch (child.name) {
						case tva.e_Coding:
							if (child_href && !this.#allowedVideoSchemes.has(child_href))
								errs.addError({
									code: "SI072",
						...
						case tva.e_PictureFormat:
							if (child_href && !this.#allowedPictureFormats.has(child_href))
								errs.addError({
									code: "SI082",
			...
			let codec_count = 0;
			const conf_points = new Set();
			ContentAttributes.forEachNamedChildElement(dvbi.e_VideoConformancePoint, (conf) => {
				const conformanceVal = conf.attrAnyNsValueOr(dvbi.a_href);
				if (conformanceVal) {
					...
					else if (!SL_helpers.isHDRDMISystem(conformanceVal)) {
						codec_count++;
						if (codec_count > 1)
							errs.addError({
								code: "SI092",
					...
					if (DuplicatedValue(conf_points, conformanceVal))
						errs.addError({
							code: "SI093",
			...
			ContentAttributes.forEachNamedChildElement(tva.e_CaptionLanguage, (conf) => {
				checkLanguage(conf.content, conf, errs, "SI101");
			});
			// Check ContentAttributes/SignLanguage
			ContentAttributes.forEachNamedChildElement(tva.e_SignLanguage, (conf) => {
		...
		const packages_found = new Set();
		ServiceInstance.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
			const pkg_lang = mlLanguage(SubscriptionPackage);
			const pkg = SL_helpers.localizedSubscriptionPackage(SubscriptionPackage, pkg_lang);
			if (SL_SchemaVersion(documentNamespace) >= slVersions.r3) {
				if (!documentInfo.declaredSubscriptionPackages.has(pkg))
					errs.addError({
						code: "SI130",
			...
			if (DuplicatedValue(packages_found, pkg))
				errs.addError({
					type: WARNING,
					code: "SI131",
		...
		if (SourceType) {
			...
			switch (SourceType.content) {
				case dvbi.DVBT_SOURCE_TYPE:
					if (!ServiceInstance.hasChild(dvbi.e_DVBTDeliveryParameters)) 
						errs.addError(SL_helpers.NoDeliveryParams("DVB-T", thisServiceId, SourceType, "SI151"));
					v1Params = true;
					break;
				case dvbi.DVBS_SOURCE_TYPE:
					if (!ServiceInstance.hasChild(dvbi.e_DVBSDeliveryParameters)) 
						errs.addError(SL_helpers.NoDeliveryParams("DVB-S", thisServiceId, SourceType, "SI152"));
					v1Params = true;
					break;
				case dvbi.DVBC_SOURCE_TYPE:
					if (!ServiceInstance.hasChild(dvbi.e_DVBCDeliveryParameters)) 
						errs.addError(SL_helpers.NoDeliveryParams("DVB-C", thisServiceId, SourceType, "SI153"));
					v1Params = true;
					break;
				case dvbi.DVBDASH_SOURCE_TYPE:
					if (!ServiceInstance.hasChild(dvbi.e_DASHDeliveryParameters)) 
						errs.addError(SL_helpers.NoDeliveryParams("DVB-DASH", thisServiceId, SourceType, "SI154"));
					v1Params = true;
					break;
				case dvbi.DVBIPTV_SOURCE_TYPE:
					if (!ServiceInstance.hasChild(dvbi.e_MulticastTSDeliveryParameters) && !ServiceInstance.hasChild(dvbi.e_RTSPDeliveryParameters))
						errs.addError(SL_helpers.NoDeliveryParams("Multicast or RTSP", thisServiceId, SourceType, "SI155"));
					v1Params = true;
					break;
				case dvbi.DVBAPPLICATION_SOURCE_TYPE:
					...
					// eslint-disable-next-line no-case-declarations
					const foundDeliveryParameters = SL_helpers.deliveryParameters(ServiceInstance);
			...
			if (v1Params && SL_SchemaVersion(documentNamespace) >= slVersions.r1) 
				errs.addError(DeprecatedElement(SourceType, SL_SchemaSpecVersion(slVersions.r1), "SI160"));
		} else {
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_AltServiceName, (altSN) => {
		...
		if (DASHDeliveryParameters) {
			...
			if (URIBasedLocation) {
				const uriContentType = URIBasedLocation.attrAnyNsValueOr(dvbi.a_contentType);
				if (uriContentType && !validDASHcontentType(uriContentType, SL_SchemaVersion(documentNamespace)))
					errs.addError({
						code: "SI173",
				...
				if (uri) {
					...
					ValidateAnyContentDigests(uri, errs, "SI176");
					ValidateAnySignaturePolicy(uri, documentInfo, errs, "SI177");
				}
			}

			// <DASHDeliveryParameters><CMCD>
			ValidateCMCDinDASH(DASHDeliveryParameters, errs);

			// <DASHDeliveryParameters><Extension>
			DASHDeliveryParameters.forEachNamedChildElement(dvbi.e_Extension, (Extension) => {
		...
		if (DVBTDeliveryParameters) {
			...
			if (DVBTtargetCountry) {
				...
				if (SL_SchemaVersion(documentNamespace) >= slVersions.r6) 
					errs.addError(DeprecatedElement(DVBTtargetCountry, SL_SchemaSpecVersion(slVersions.r6), "SI183"));
			}
		}
		...
		if (DVBCDeliveryParameters) {
			...
			if (DVBCtargetCountry) {
				...
				if (SL_SchemaVersion(documentNamespace) >= slVersions.r6) 
					errs.addError(DeprecatedElement(DVBCtargetCountry, SL_SchemaSpecVersion(slVersions.r6), "SI192"));
			}
			if (SL_SchemaVersion(documentNamespace) >= slVersions.r7) {
				...
				DVBCDeliveryParameters.forEachNamedChildElement(dvbi.e_NetworkID, (NetworkID) => {
					const nid = NetworkID.content;
					if (DuplicatedValue(knownIDs, nid))
						errs.addError({
							code: "SI193",
							type: WARNING,
			...
			if (ModulationSystem) {
				switch (ModulationSystem.content) {
					case sats.MODULATION_S:
						SL_helpers.checkElement(RollOff, dvbi.e_RollOff, sats.S_RollOff, sats.MODULATION_S, ERROR_KEY, errs, "SI201a");
						SL_helpers.checkElement(ModulationType, dvbi.e_ModulationType, sats.S_Modulation, sats.MODULATION_S, ERROR_KEY, errs, "SI202a");
						SL_helpers.checkElement(FEC, dvbi.e_FEC, sats.S_FEC, sats.MODULATION_S, ERROR_KEY, errs, "SI203a");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_ModcodMode, sats.MODULATION_S, ERROR_KEY, errs, "a");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_InputStreamIdentifier, sats.MODULATION_S, ERROR_KEY, errs, "b");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_ChannelBonding, sats.MODULATION_S, ERROR_KEY, errs, "c");
						break;
					case sats.MODULATION_S2:
						SL_helpers.checkElement(RollOff, dvbi.e_RollOff, sats.S2_RollOff, sats.MODULATION_S2, ERROR_KEY, errs, "SI201b");
						SL_helpers.checkElement(ModulationType, dvbi.e_ModulationType, sats.S2_Modulation, sats.MODULATION_S2, ERROR_KEY, errs, "SI202b");
						SL_helpers.checkElement(FEC, dvbi.e_FEC, sats.S2_FEC, sats.MODULATION_S2, ERROR_KEY, errs, "SI203b");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_ModcodMode, sats.MODULATION_S2, ERROR_KEY, errs, "k");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_InputStreamIdentifier, sats.MODULATION_S2, ERROR_KEY, errs, "l");
						SL_helpers.DisallowedElement(DVBSDeliveryParameters, dvbi.e_ChannelBonding, sats.MODULATION_S2, ERROR_KEY, errs, "m");
						break;
					case sats.MODULATION_S2X:
						SL_helpers.checkElement(RollOff, dvbi.e_RollOff, sats.S2X_RollOff, sats.MODULATION_S2X, ERROR_KEY, errs, "SI201c");
						SL_helpers.checkElement(ModulationType, dvbi.e_ModulationType, sats.S2X_Modulation, sats.MODULATION_S2X, ERROR_KEY, errs, "SI202c");
						SL_helpers.checkElement(FEC, dvbi.e_FEC, sats.S2X_FEC, sats.MODULATION_S2X, ERROR_KEY, errs, "SI203c");
						...
						if (ChannelBonding) {
							...
							ChannelBonding.forEachNamedChildElement(dvbi.e_Frequency, (Frequency) => {
								if (DuplicatedValue(freqs, Frequency.content))
									errs.addError({
										code: "SI205",
										key: ERROR_KEY,
								...
								const Frequency_primary = Frequency.attrAnyNsValueOr(dvbi.a_primary);
		...
		if (MulticastTSDeliveryParameters) SL_helpers.checkMulticastDeliveryParams(MulticastTSDeliveryParameters, errs, "SI235");
	 ...
	 * @param {String}     thisServiceId                 the identifier of the service
	 * @param {} documentInfo
	 *                     knownServices                 services found and checked thus far
	 *                     knownRegionIDs                regions identifiers from the RegionList
	 *                     signaturePolicyIDs            Signature Verification policies defined in this service list
	 *                     declaredSubscriptionPackages  subscription packages that are declared in the service list
	...
	/*private*/ #validateService(service, thisServiceId, documentInfo, errs) {
		...
		if (uID) {
			...
			if (!SL_helpers.validServiceIdentifier(thisServiceId))
				errs.addError({
					code: "SL110",
			...
			if (!SL_helpers.uniqueServiceIdentifier(thisServiceId, documentInfo.knownServices))
				errs.addError({
					code: "SL111",
			...
			documentInfo.knownServices.push(thisServiceId);
		}

		//check <ServiceInstance>
		service.forEachNamedChildElement(dvbi.e_ServiceInstance, (ServiceInstance) => 
			this.#validateServiceInstance(ServiceInstance, thisServiceId, documentInfo, errs)
		);

		//check <TargetRegion>
		const rBuf = new Set();
		service.forEachNamedChildElement(dvbi.e_TargetRegion, (TargetRegion) => {
			const found = documentInfo.knownRegionIDs.find((r) => r.region == TargetRegion.content);
			if (found == undefined) 
				errs.addError(SL_helpers.UnspecifiedTargetRegion(TargetRegion.content, `service ${thisServiceId.quote()}`, "SL130", TargetRegion));
			else {
				if (!found.selectable)
					errs.addError({
						code: "SL132",
						message: `target region ${TargetRegion.content.quote()} for service ${thisServiceId.quote()} is not selectable`,
						fragment: TargetRegion,
						key: "unselectable target region",
					});
				found.used = true;
			}
			if (DuplicatedValue(rBuf, TargetRegion.content)) {
				errs.addError({
					...
					code: "SL131",
					key: "duplicate value",
					message: `duplicate value ${TargetRegion.content.quote()} specified for ${dvbi.e_TargetRegion.elementize()}`,
					fragment: TargetRegion,
				});
			}
		});
		...
		service.forEachNamedChildElement(tva.e_RelatedMaterial, (RelatedMaterial) => 
			this.#validateRelatedMaterial(RelatedMaterial, `service ${thisServiceId.quote()}`, SERVICE_RM, documentInfo, errs, "SL150")
		);
		//check <ServiceGenre>
		service.forEachNamedChildElement(dvbi.e_ServiceGenre, (ServiceGenre) => {
			checkAttributes(ServiceGenre, [tva.a_href], [tva.a_type], tvaEA.Genre, errs, "SL160");
			const Genre_type = ServiceGenre.attrAnyNsValueOr(tva.a_type);
			...
			const Genre_value = ServiceGenre.attrAnyNsValueOr(dvbi.a_href);
			if (Genre_value && !this.#allowedGenres.has(Genre_value))
				errs.addError({
					code: "SL162",
		...
		if (ServiceType) {
			const ServiceType_href = ServiceType.attrAnyNsValueOr(dvbi.a_href);
			if (ServiceType_href && !this.#allowedServiceTypes.has(ServiceType_href))
				errs.addError({
					code: "SL164",
		...
		if (RecordingInfo) {
			const RI_href = RecordingInfo.attrAnyNsValueOr(dvbi.a_href);
			if (RI_href && !this.#RecordingInfoCSvalues.has(RI_href))
			errs.addError({
				code: "SL180",
		...
		if (sCG) 
			this.#validateAContentGuideSource(
				sCG, 
				`${dvbi.e_ContentGuideSource.elementize()} in service ${thisServiceId}`, 
				documentInfo, errs, "SL190");

		//check <ContentGuideSourceRef>
		const sCGref = service.getAnyNs(dvbi.e_ContentGuideSourceRef);
		if (sCGref && !documentInfo.ContentGuideSourceIDs.has(sCGref.content))
			errs.addError({
				code: "SL200",
		...
		if (NVOD) {
			const NVOD_mode = NVOD.attrAnyNsValueOr(dvbi.a_mode);
				...
				if (NVOD_reference) {
					...
					if (!referredService)
						...
						if (!refNVOD)
							...
							const refNVOD_mode = refNVOD.attrAnyNsValueOr(dvbi.a_mode);
		...
		if (ProminenceList) {
			...
			ProminenceList.forEachNamedChildElement(dvbi.e_Prominence, (PE) => {
				...
				if (PE_region) {
					const found = documentInfo.knownRegionIDs.find((r) => r.region == PE_region);
					if (found === undefined)
						errs.addError({
							code: "SL230",
				...
				const PE_country = PE.attrAnyNsValueOr(dvbi.a_country);
				...
				// for exact match
				const PE_ranking = PE.attrAnyNsValueOr(dvbi.a_ranking);
		...
		if (ParentalRating) {
			...
			ParentalRating.forEachNamedChildElement(dvbi.e_MinimumAge, (MinimumAge) => {
				const MinimumAge_countryCodes = MinimumAge.attrAnyNsValueOr(dvbi.a_countryCodes);
				if (MinimumAge_countryCodes) {
					MinimumAge_countryCodes.toUpperCase().split(",")
						.forEach((country) => {
							...
							if (DuplicatedValue(foundCountries, country))
								errs.addError({
									code: "SL252",
	...
	/*private*/ #ValidateRequirements(Requires, errs, errCode) {
		...
		if (Delivery)
			CheckDelivery(Delivery, SL_SchemaVersion(Requires.documentNamespace()), this.#allowedApplicationTypes, errs, `${errCode}-10`);

		const extraCaps = Requires.attrAnyNsValueOr(dvbi.a_extraCapabilities);
		if (extraCaps)
			extraCaps.split(" ").forEach((capability) => {
				if (!this.#allowedAudioSchemes.has(capability) && !this.#allowedVideoSchemes.has(capability) && !KnownDRMScheme(capability)) {
					errs.addError({
						type: WARNING,
						code: `${errCode}-30`,
						line: Requires.line,
	...
	/*private*/ #doSchemaVerification(ServiceList, errs, errCode, report_schema_version = true) {
		const x = SL_GetSchema(ServiceList.root.namespaceUri);
		if (x && x.schema) {
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		const ServiceList_lang = ServiceList.attrAnyNsValueOr(tva.a_lang);
		if (ServiceList_lang)
		...
		documentInfo.thisServiceListId = ServiceList.attrAnyNsValueOr(dvbi.a_id);
		if (documentInfo.thisServiceListId && !SL_helpers.validServiceListIdentifier(documentInfo.thisServiceListId))
			errs.addError({
				code: "SL016",
				message: `${documentInfo.thisServiceListId.quote()} is not a valid service list identifier`,
				key: keys.k_InvalidTag,
		...
		ServiceList.forEachNamedChildElement(dvbi.e_StandardVersion, (StandardVersion) => {
		...
		ServiceList.forEachNamedChildElement(dvbi.e_Requires, (Requires) => {
			this.#ValidateRequirements(Requires, errs, "SL025");
		});

		//chck <ServiceList><SignaturePolicies>
		...
		if (LanguageList) {
			LanguageList.forEachNamedChildElement(tva.e_Language, (Language) => {
				...
				const found = documentInfo.declaredAudioLanguages.find((el) => (el.language == lang_lower));
				if (found != undefined)
					errs.addError({
						code: "SL032",
				...
				else documentInfo.declaredAudioLanguages.push({ language: lang_lower, used: false, fragment: Language });
			});
		}

		//check <ServiceList><RelatedMaterial>
		ServiceList.forEachNamedChildElement(tva.e_RelatedMaterial, (RelatedMaterial) => {
			this.#validateRelatedMaterial(RelatedMaterial, "service list", SERVICE_LIST_RM, documentInfo, errs, "SL040");
		});
		...
		if (RegionList) {
			...
			RegionList.forEachNamedChildElement(dvbi.e_Region, (Region) => this.#addRegion(Region, 0, documentInfo, null, errs));
		}

		//check <ServiceList><TargetRegion>
		const	rBuf = new Set();
		ServiceList.forEachNamedChildElement(dvbi.e_TargetRegion, (TargetRegion) => {
			const found = documentInfo.knownRegionIDs.find((r) => r.region == TargetRegion.content);
			if (found == undefined) 
				errs.addError(SL_helpers.UnspecifiedTargetRegion(TargetRegion.content, "service list", "SL051", TargetRegion));
		...
		if (SubscriptionPackageList) {
			SubscriptionPackageList.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
				const pkg = SL_helpers.localizedSubscriptionPackage(SubscriptionPackage);

				if (DuplicatedValue(documentInfo.declaredSubscriptionPackages, pkg))
					errs.addError({
						code: "SL063",
		...
		if (CGSourceList) {
			let cgs = 0;
			CGSourceList.forEachNamedChildElement(dvbi.e_ContentGuideSource, (CGSource) => {
				this.#validateAContentGuideSource(CGSource, 
					`${dvbi.e_ServiceList}.${dvbi.e_ContentGuideSourceList}.${dvbi.e_ContentGuideSource}[${++cgs}]`, 
					documentInfo, errs, "SL070");

				const CGSource_CGSID = CGSource.attrAnyNsValueOr(dvbi.a_CGSID);
				if (CGSource_CGSID) {
					if (DuplicatedValue(documentInfo.ContentGuideSourceIDs, CGSource_CGSID))
						errs.addError({
							code: "SL071",
		...
		if (slGCS) 
			this.#validateAContentGuideSource(slGCS, 
				`${dvbi.e_ServiceList}.${dvbi.e_ContentGuideSource}`, 
				documentInfo, errs, "SL080");

		errs.setW("num services", 0);

		// check <Service>
		let s = 0;
		documentInfo.knownServices = [];
		...
		if (SL_SchemaVersion(ns) >= slVersions.r5) {
			...
			// check <TestService>
			let ts = 0;
			ServiceList.forEachNamedChildElement(dvbi.e_TestService, (testService) => {
				ts++;
		...
		if (LCNtableList) {
			...
			LCNtableList.forEachNamedChildElement(dvbi.e_LCNTable, (LCNTable) => {
				...
				LCNTable.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
					...
					const SubscriptionPackage_lang = SubscriptionPackage.attrAnyNsValueOr(tva.a_lang);
					...
					const localSubscriptionPackage = SL_helpers.localizedSubscriptionPackage(SubscriptionPackage, packageLanguage);
					...
					if (SL_SchemaVersion(ns) >= slVersions.r3)
						if (!documentInfo.declaredSubscriptionPackages.has(localSubscriptionPackage))
							errs.addError({
								code: "SL248",
				...
				// <LCNTable><LCN>
				const LCNNumbers = new Set();
				LCNTable.forEachNamedChildElement(dvbi.e_LCN, (LCN) => {
					// LCN@channelNumber
					const LCN_channelNumber = LCN.attrAnyNsValueOr(dvbi.a_channelNumber)
					if (LCN_channelNumber) {
						if (LCNNumbers.has(LCN_channelNumber))
							errs.addError({
								code: "SL262",
						...
						else LCNNumbers.add(LCN_channelNumber);
						const chanNumV = parseInt(LCN_channelNumber, 10);
						if (chanNumV < dvbi.MIN_LCN || chanNumV > dvbi.MAX_LCN) {
							errs.addError(OutOfRangeChannelNumber("SL264", LCN, chanNumV));
						}
					}

					// LCN@serviceRef
					const LCN_serviceRef = LCN.attrAnyNsValueOr(dvbi.a_serviceRef);
					if (LCN_serviceRef && !isIn(documentInfo.knownServices, LCN_serviceRef)) {
						errs.addError({
							code: "SL263",
							...
							fragment: LCN,
							clause: "A177 table 23",
							description: `The value of ${dvbi.a_serviceRef.attribute(dvbi.e_LCN)} needs to refer to the ${dvbi.e_UniqueIdentifier.elementize()} of a ${dvbi.e_Service.elementize()}`,
						});
					}
				});

				// <LCNTable><LCNRange>
				LCNTable.forEachNamedChildElement(dvbi.e_LCNRange, (LCNRange) => {
					...
					const Range_start = LCNRange.attrAnyNsValueOr(dvbi.a_start);
					const Range_end = LCNRange.attrAnyNsValueOr(dvbi.a_end);
					if (Range_start) {
						const startNum = parseInt(Range_start, 10);
						if (startNum < dvbi.MIN_LCN || startNum > dvbi.MAX_LCN)
							errs.addError(OutOfRangeChannelNumber("SL266", LCNRange, startNum));
					}
					if (Range_end) {
						const endNum = parseInt(Range_end, 10);
						if (endNum < dvbi.MIN_LCN || endNum > dvbi.MAX_LCN)
							errs.addError(OutOfRangeChannelNumber("SL267", LCNRange, endNum));
					}

					const serviceType = LCNRange.attrAnyNsValueOr(dvbi.a_serviceType);
					if (serviceType) {
						if (!this.#allowedServiceTypes.has(serviceType)) 
							errs.addError({
								code: "SL265",
					...
					//<LCNRange@serviceGenre>
					const serviceGenre = LCNRange.attrAnyNsValueOr(dvbi.a_serviceGenre);
					if (serviceGenre && !this.#allowedGenres.has(serviceGenre))
							errs.addError({
								code: "SL268",
		...
		if (SL_SchemaVersion(ns) >= slVersions.r4) {
			documentInfo.knownRegionIDs.forEach((region) => {
		...
		documentInfo.declaredAudioLanguages.forEach((language) => {
			if (!language.used)
				errs.addError({
					code: "SL282",
					type: WARNING,
