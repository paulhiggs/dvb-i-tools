...
import { DASH_IF_Content_Protection_List, UUID_type, CA_SYSTEM_ID_REGISTRY, CASystemIDs, KnownDRMScheme} from "./identifiers.mjs";
...
export default class ServiceListCheck {
	...
	/*private*/ #checkSignalledApplication(MediaLocator, Location, AppType, errs) {
		...
		if (!MediaLocator)
			...
			MediaLocator.forEachNamedChildElement(tva.e_MediaUri, (MediaUri) => {
				hasMediaURI = true;
				if (MediaUri.attrAnyNs(tva.a_contentType) && SL_helpers.isValidApplicationType(MediaUri.attrAnyNs(tva.a_contentType).value))
					errs.addError({
						code: "SA003",
		...
		if (HowRelated.attrAnyNs(dvbi.a_href)) {
			...
			switch (LocationType) {
				...
				case SERVICE_RM:
					...
					const HowRelated1_href = HowRelated.attrAnyNsValueOr(dvbi.a_href, null);
				...
				case SERVICE_INSTANCE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated2_href = HowRelated.attrAnyNsValueOr(dvbi.a_href, null);
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNamespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-31`,
				...
				case CONTENT_GUIDE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated3_href = HowRelated.attrAnyNsValueOr(dvbi.a_href, null);
	...
	/*private*/ #validateAContentGuideSource(source, loc, errs, errCode) {
		...
		const CheckEndpoint = (elementName, suffix, MustEndWithSlash = false) => {
			...
			if (ep) {
				...
				const ep_contentType = ep.attrAnyNsValueOr(dvbi.a_contentType, null);
	...
	/* private */ #GetLanguage(node, parentLang, isRequired, errs, errCode) {
		...
		const localLang = node.attrAnyNsValueOr(tva.a_lang, null);
	...
	/*private*/ #ValidateSynopsisType(Element, ElementName, requiredLengths, optionalLengths, parentLanguage, errs, errCode) {
		...
		Element.forEachNamedChildElement(ElementName, (ste) => {
			...
			const synopsisLength = ste.attrAnyNsValueOr(tva.a_length, null);
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs) {		if (!parameterCheck("validateServiceInstance", ServiceInstance, dvbi.e_ServiceInstance, errs, "SI000a")) return;
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_ContentProtection, (ContentProtection) => {
			...
			ContentProtection.forEachNamedChildElement(dvbi.e_DRMSystemId, (DRMSystemID) => {
				...
				if (DRMSystemID_value && DRMSystemID_value.length == 0) {
					...
					const DRMSystemID_LAurl = DRMSystemID.attrAnyNsValueOr(dvbi.a_LAURL, null);
					const DRMSystemID_Certificateurl = DRMSystemID.attrAnyNsValueOr(dvbi.a_CertificateURL, null);
		...
		if (ContentAttributes) {
			...
			ContentAttributes.forEachNamedChildElement(tva.e_AudioAttributes, (conf) => {
				conf.forEachChildElement((child) => {
					const child_href = child.attrAnyNsValueOr(dvbi.a_href, null);
					switch (child.name) {
						case tva.e_Coding:
		...
		if (DASHDeliveryParameters) {
			...
			if (URIBasedLocation) {
				const uriContentType = URIBasedLocation.attrAnyNsValueOr(dvbi.a_contentType, null);
	...
	/*private*/ #validateService(service, thisServiceId, knownServices, knownRegionIDs, declaredSubscriptionPackages, declaredAudioLanguages, ContentGuideSourceIDs, errs) {
		...
		service.forEachNamedChildElement(dvbi.e_ServiceGenre, (ServiceGenre) => {
			...
			const Genre_value = ServiceGenre.attrAnyNsValueOr(dvbi.a_href, null);
				...
				if (NVOD_reference) {
					...
					if (!referredService)
						...
						if (!refNVOD)
							...
							const refNVOD_mode = refNVOD.attrAnyNsValueOr(dvbi.a_mode, null);
	...
	/*private*/ #ValidateRequirements(Requires, errs, errCode) {
		...
		const extraCaps = Requires.attrAnyNsValueOr(dvbi.a_extraCapabilities, null);
		if (extraCaps)
		...
		if (LCNtableList) {
			...
			LCNtableList.forEachNamedChildElement(dvbi.e_LCNTable, (LCNTable) => {
				...
				LCNTable.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
					...
					const SubscriptionPackage_lang = SubscriptionPackage.attrAnyNsValueOr(tva.a_lang, null);
				...
				LCNTable.forEachNamedChildElement(dvbi.e_LCN, (LCN) => {
					// LCN@channelNumber
					const LCN_channelNumber = LCN.attrAnyNsValueOr(dvbi.a_channelNumber, null)
