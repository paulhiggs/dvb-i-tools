...
import { checkAttributes, checkTopElementsAndCardinality, checkDeprecatedElement, SchemaCheck, SchemaVersionCheck, SchemaLoad } from "./schema_checks.mjs";
...
export default class ServiceListCheck {
	...
	/*private*/ #validateAContentGuideSource(source, loc, errs, errCode) {
		...
		let CheckEndpoint = (elementName, suffix, MustEndWithSlash = false) => {
			...
			if (ep) {
				...
				if (epURL) {
					if (!MustEndWithSlash && !isHTTPURL(epURL.content)) errs.addError(InvalidURL(epURL.content, ep, elementNameelementize(), `${errCode}-${suffix}a`));
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs) {
		...
		if (ContentAttributes) {
			...
			while ((conf = ContentAttributes.getAnyNs(dvbi.e_AudioConformancePoint, ++cp)) != null) {
				if (SL_SchemaVersion(documentNamespace) > slVersions.r4) errs.addError(DeprecatedElement(conf, SchemaSpecVersion(documentNamespace), "SI062"));
		...
		while ((SubscriptionPackage = ServiceInstance.getAnyNs(dvbi.e_SubscriptionPackage, ++sp)) != null) {
			...
			if (isIn(packages_found, pkg)) {
				errs.addError({
					type: WARNING,
					code: "SI131",
					message: `${dvbi.e_SubscriptionPackage.elementize()}="${pkg}" is already defined in this service instance`,
					fragment: SubscriptionPackage,
		...
		if (SourceType) {
			...
			if (v1Params && SL_SchemaVersion(documentNamespace) >= slVersions.r1) errs.addError(DeprecatedElement(SourceType, SchemaSpecVersion(documentNamespace), "SI160"));
		} else {
		...
		if (DVBTDeliveryParameters) {
			...
			if (DVBTtargetCountry) {
				...
				if (SL_SchemaVersion(documentNamespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBTtargetCountry, SchemaSpecVersion(documentNamespace), "SI183"));
			}
		}
		...
		if (DVBCDeliveryParameters) {
			...
			if (DVBCtargetCountry) {
				...
				if (SL_SchemaVersion(documentNamespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBCtargetCountry, SchemaSpecVersion(documentNamespace), "SI192"));
			}
	...
	/*private*/ #validateService(service, thisServiceId, knownServices, knownRegionIDs, declaredSubscriptionPackages, declaredAudioLanguages, ContentGuideSourceIDs, errs) {
		...
		if (ServiceType) {
			...
			const schemaVersion = SL_SchemaVersion(service.documentNamespace());
			if (schemaVersion >= slVersions.r8)
				errs.addError(DeprecatedElement(ServiceType,  SchemaSpecVersion(slVersions.r8), "SL165"));
		}
			...
			while ((LCNTable = LCNtableList.getAnyNs(dvbi.e_LCNTable, ++l)) != null) {
				...
				while ((LCNRange = LCNTable.getAnyNs(dvbi.e_LCNRange, ++r)) != null) {
					...
					if (serviceType) {
						...
						if (schemaVersion >= slVersions.r8)
							errs.addError(DeprecatedAttribute(serviceType,  SchemaSpecVersion(slVersions.r8), "SL266"));
					}
