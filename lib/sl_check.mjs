...
import { slVersions, dvbi, dvbisld, dvbiEC, dvbiEA, XMLdocumentType, validApplicationTypes, DvbIApplicationTypes } from "./DVB-I_definitions.mjs";
// import { dashif } from "./DASH-IF_definitions.mjs";
...
import { isPostcode, isASCII, isHTTPURL, isHTTPPathURL, isDomainName, isRTSPURL, isTAGURI, hasNonPrintableChars, isUUIDformat } from "./pattern_checks.mjs";
		...
		if (Availability) {
			Availability.forEachNamedChildElement(dvbi.e_Period, (Period) => {
				if (Period.attrAnyNs(dvbi.a_validFrom) && Period.attrAnyNs(dvbi.a_validTo)) {
					...
					let ZuluIfNeeded = (dt) => (dt.endsWith("Z") || dt.lastIndexOf("+") > 12 || dt.lastIndexOf("-") > 12) ? dt : `${dt}Z`; 
					try {
						const fr = Temporal.Instant.from(ZuluIfNeeded(Period.attrAnyNs(dvbi.a_validFrom).value)),
							to = Temporal.Instant.from(ZuluIfNeeded(Period.attrAnyNs(dvbi.a_validTo).value));
				...
				Period.forEachNamedChildElement(dvbi.e_Interval, (Interval) => {
					let unzone = (time) => time.includes("Z") ? time.substring(0, time.indexOf("Z")) : time;
					const startTime = unzone(Interval.attrAnyNsValueOr(dvbi.a_startTime, "00:00:00Z")),
						endTime = unzone(Interval.attrAnyNsValueOr(dvbi.a_endTime, "23:59:59.999Z"));
					try {
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		if (LCNtableList) {
			...
			LCNtableList.forEachNamedChildElement(dvbi.e_LCNTable, (LCNTable) => {
				...
				let SubscriptionPackages = new Set();
