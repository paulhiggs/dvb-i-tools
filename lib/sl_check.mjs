...
export default class ServiceListCheck {
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, documentInfo, errs) {
		...
		if (ContentAttributes) {
			...
			ContentAttributes.forEachNamedChildElement(tva.e_AudioAttributes, (conf) => {
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
			...
			if (SL_SchemaVersion(documentNamespace) >= slVersions.r3) {
				if (!documentInfo.declaredSubscriptionPackages.includes(pkg))
					errs.addError({
						code: "SI130",
	...
	/*private*/ #validateService(service, thisServiceId, documentInfo, errs) {
		...
		if (sCGref && !isIn(documentInfo.ContentGuideSourceIDs, sCGref.content))
			errs.addError({
				code: "SL200",
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		documentInfo.signaturePolicyIDs = ValidateSignaturePolicies(ServiceList, errs, "SR060");
		...
		ServiceList.forEachNamedChildElement(tva.e_RelatedMaterial, (RelatedMaterial) => {
			const foundHref = this.#validateRelatedMaterial(RelatedMaterial, "service list", SERVICE_LIST_RM, documentInfo, errs, "SL040");
		...
		documentInfo.declaredSubscriptionPackages = [];
		...
		if (SubscriptionPackageList) {
			SubscriptionPackageList.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
				...
				else documentInfo.declaredSubscriptionPackages.push(pkg);
			});
		}
		...
		documentInfo.ContentGuideSourceIDs = [];
		...
		if (CGSourceList) {
			...
			CGSourceList.forEachNamedChildElement(dvbi.e_ContentGuideSource, (CGSource) => {
				...
				if (CGSource_CGSID) {
					if (isIn(documentInfo.ContentGuideSourceIDs, CGSource_CGSID))
						errs.addError({
							code: "SL071",
					...
					else documentInfo.ContentGuideSourceIDs.push(CGSource_CGSID);
				}
			});
		}
			...
			LCNtableList.forEachNamedChildElement(dvbi.e_LCNTable, (LCNTable) => {
				...
				LCNTable.forEachNamedChildElement(dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
					...
					if (SL_SchemaVersion(ns) >= slVersions.r3)
						if (!documentInfo.declaredSubscriptionPackages.includes(localSubscriptionPackage))
							errs.addError({
								code: "SL248",
