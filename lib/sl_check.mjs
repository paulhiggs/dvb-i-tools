...
import { DASH_IF_Content_Protection_List, UUID_type, ContentProtectionIDs, CA_SYSTEM_ID_REGISTRY, CASystemIDs } from "./identifiers.mjs";
					...
					switch (child.name) {
						case tva.e_Coding:
							if (child_href && !this.#allowedAudioSchemes.isIn(child_href))
								errs.addError({
									code: "SI052",
									message: `invalid ${dvbi.a_href.attribute(child.name)} value for (${child_href}) ${this.#allowedAudioSchemes.valuesRange()}`,
									fragment: child,
									key: "audio codec",
						...
						case tva.e_MixType:
							...
							if (child_href && !this.#audioPresentations.isIn(child_href))
								errs.addError({
									code: "SI055",
									message: `invalid ${dvbi.a_href.attribute(child.name)} value for (${child_href}) ${this.#audioPresentations.valuesRange()}`,
									fragment: child,
									key: "audio codec",
								});
							break;
					...
					switch (child.name) {
						case tva.e_Coding:
							if (child_href && !this.#allowedVideoSchemes.isIn(child_href))
								errs.addError({
									code: "SI072",
									message: `invalid ${dvbi.a_href.attribute(tva.e_Coding)} (${child_href}) ${this.#allowedVideoSchemes.valuesRange()}`,
									fragment: child,
									key: "video codec",
								});
							break;
						case tva.e_PictureFormat:
							if (child_href && !this.#allowedPictureFormats.isIn(child_href))
								errs.addError({
									code: "SI082",
									message: `invalid ${dvbi.a_href.attribute(tva.e_PictureFormat)} value (${child_href}) ${this.#allowedPictureFormats.valuesRange()}`,
									fragment: child,
									key: tva.e_PictureFormat,
								});
							break;
						case dvbi.e_Colorimetry:
							if (child_href && !this.#allowedColorimetry.isIn(child_href))
								errs.addError({
									code: "SI084",
									message: `invalid ${dvbi.a_href.attribute(dvbi.e_Colorimetry)} value (${child_href}) ${this.#allowedColorimetry.valuesRange()}`,
									fragment: child,
