...
export default class ServiceListCheck {
	 ...
	 * @param {String[]}   knownPolicies Signature Verification policies that are defined in the service list
	 * @param {ErrorList}  errs          Errors found in validaton
	 * @param {String}     errCode       Error code prefix to be used in reports
	 */
	/*private*/ #validateAContentGuideSource(source, loc, knownPolicies, errs, errCode) {
		...
		const CheckEndpoint = (elementName, suffix, MustEndWithSlash = false) => {
			...
			if (ep) {
				...
				if (epURL) {
					...
					if (validationPolicy && !isIn(knownPolicies, validationPolicy))
						errs.addError({
							type: WARNING,
	 ...
	 * @param {String[]}   knownPolicies                 Signature Verification policies defined in this service list
	 * @param {Array}      declaredSubscriptionPackages  subscription packages that are declared in the service list
	 ...
	 * @param {ErrorList}  errs                          errors found in validaton
	 */
	/*private*/ #validateService(service, thisServiceId, knownServices, knownRegionIDs, knownPolicies, declaredSubscriptionPackages, declaredAudioLanguages, ContentGuideSourceIDs, errs) {
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		const signaturePolicyIDs = ValidateSignaturePolicies(ServiceList, errs, "SR060");
		...
		if (CGSourceList) {
			...
			CGSourceList.forEachNamedChildElement(dvbi.e_ContentGuideSource, (CGSource) => {
				this.#validateAContentGuideSource(CGSource, `${dvbi.e_ServiceList}.${dvbi.e_ContentGuideSourceList}.${dvbi.e_ContentGuideSource}[${++cgs}]`, signaturePolicyIDs, errs, "SL070");
		...
		if (slGCS) this.#validateAContentGuideSource(slGCS, `${dvbi.e_ServiceList}.${dvbi.e_ContentGuideSource}`, signaturePolicyIDs, errs, "SL080");
