...
export default class ServiceListCheck {
	...
	constructor(useURLs, opts, async = true) {
		...
		this.#knownCountries = opts?.countries || LoadCountries(useURLs, async);
		this.#knownLanguages = opts?.languages || LoadLanguages(useURLs, async);

		console.log(chalk.yellow.underline("SL: loading classification schemes..."));
		this.#accessibilityPurposes = opts?.accessibilities || LoadAccessibilityPurpose(useURLs, async);
		this.#allowedAudioSchemes = opts?.audiofmts || LoadAudioCodecCS(useURLs, async);
		this.#audioPresentations = opts?.audiopres || LoadAudioPresentationCS(useURLs, async);
		this.#audioPurposes = opts?.audiopurps || LoadAudioPurpose(useURLs, async);
		this.#allowedGenres = opts?.genres || LoadGenres(useURLs, async);
		this.#subtitleCarriages = opts?.stcarriage || LoadSubtitleCarriages(useURLs, async);
		this.#subtitleCodings = opts?.stcodings || LoadSubtitleCodings(useURLs, async);
		this.#subtitlePurposes = opts?.stpurposes || LoadSubtitlePurposes(useURLs, async);
		this.#allowedVideoSchemes = opts?.videofmts || LoadVideoCodecCS(useURLs, async);
		this.#allowedApplicationTypes = opts?.appfmts || LoadLinkedApplicationCS(useURLs, async);
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		if (documentInfo.thisServiceListId) {
			if (!SL_helpers.validServiceListIdentifier(documentInfo.thisServiceListId))
				errs.addError({
					code: "SL016",
