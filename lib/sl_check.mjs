...
import { isIn, unEntity, /* getElementByTagName, */ DuplicatedValue } from "./utils.mjs";
import { isPostcode, isASCII, isHTTPURL, isHTTPPathURL, isDomainName, isRTSPURL, isTAGURI } from "./pattern_checks.mjs";
...
import { GetSchema, SchemaVersion, SchemaSpecVersion, isA177specification_URN, isContentFinishedBanner, isOutScheduleHours } from "./sl_data_versions.mjs";
import {
...
export default class ServiceListCheck {
	...
	/*private*/ #addRegion(Region, depth, knownRegionIDs, countries, errs) {
		...
		const schemaVersion = SchemaVersion(Region.documentNamespace());
	...
	/*private*/ #validateRelatedMaterial(RelatedMaterial, Location, LocationType, errs, errCode) {
		...
		if (HowRelated.attrAnyNs(dvbi.a_href)) {
			...
			switch (LocationType) {
				...
				case SERVICE_RM:
					if (isContentFinishedBanner(HowRelated) && SchemaVersion(documentNamespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-21`,
					...
					} else if (this.#validServiceApplication(HowRelated, SchemaVersion(documentNamespace))) {
						rc = HowRelated.attrAnyNs(dvbi.a_href).value;
						MediaLocator.forEach((locator) => this.#checkSignalledApplication(locator, Location, rc, errs));
					} else {
				...
				case SERVICE_INSTANCE_RM:
					...
					} else if (isOutScheduleHours(HowRelated) && SchemaVersion(documentNamespace) >= slVersions.r6)
						errs.addError({
							code: `${errCode}-35`,
	...
	/*private*/ #hasSignalledApplication(node) {
		if (node) {
			...
			while ((elem = node.getAnyNs(tva.e_RelatedMaterial, ++i)) != null) {
				...
				if (hr && this.#validServiceApplication(hr, SchemaVersion(node.documentNamespace()))) return true;
			}
		}
		return false;
	}
	...
	/*private*/ #validateAContentGuideSource(source, loc, errs, errCode) {
		...
		let CheckEndpoint = (elementName, suffix, MustEndWithSlash = false) => {
			...
			if (ep) {
				...
				if (epURL) {
					if (!MustEndWithSlash && !isHTTPURL(epURL.content)) errs.addError(InvalidURL(epURL.content, ep, elementName, `${errCode}-${suffix}a`));
		...
		CheckEndpoint(dvbi.e_GroupInfoEndpoint, 18, SchemaVersion(source.documentNamespace()) >= slVersions.r5);
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs) {
		...
		checkTopElementsAndCardinality(
			...
			[
				...
				{ name: dvbi.e_SubscriptionPackage, minOccurs: 0, mmaxOccursax: Infinity },
				...
				{ name: dvbi.e_IdentifierBasedDeliveryParameters, minOccurs: 0 },
				{ name: dvbi.e_SATIPDeliveryParameters, minOccurs: 0, maxOccurs: Infinity },
			],
			tvaEC.ServiceInstanceType,
			false,
			errs,
			"SI005"
		);
		...
		if (SchemaVersion(documentNamespace) <= slVersions.r4) {
		...
		while ((RelatedMaterial = ServiceInstance.getAnyNs(tva.e_RelatedMaterial, ++rm)) != null) {
			...
			if (foundHref != "" && validServiceInstanceControlApplication(foundHref, SchemaVersion(documentNamespace))) controlApps.push(RelatedMaterial);
			if (foundHref == dvbi.APP_IN_CONTROL) {
				// Application controlling playback SHOULD NOT have any service delivery parameters
				if (SchemaVersion(documentNamespace) >= slVersions.r5 && hasDeliveryParameters(ServiceInstance))
					errs.addError({
						type: WARNING,
						code: "SI022",
		...
		while ((ContentProtection = ServiceInstance.getAnyNs(dvbi.e_ContentProtection, ++cp)) != null) {
			...
			checkTopElementsAndCardinality(
				ContentProtection,
				[
					{ name: dvbi.e_CASystemId, nimOccurs: 0, maxoccurs: Infinity },
					{ name: dvbi.e_CASystemId, nimOccurs: 0, maxoccurs: Infinity },
					{ name: dvbi.e_DRMSystemId, nimOccurs: 0, maxoccurs: Infinity },
				],
			...
			while ((CASystemID = ContentProtection.getAnyNs(dvbi.e_CASystemId, ++ca)) != null) {
				...
				if (SchemaVersion(documentNamespace) <= slVersions.r1) {
		...
		if (ContentAttributes) {
			...
			while ((conf = ContentAttributes.getAnyNs(dvbi.e_AudioConformancePoint, ++cp)) != null) {
				if (SchemaVersion(documentNamespace) > slVersions.r4) errs.addError(DeprecatedElement(conf, SchemaSpecVersion(documentNamespace), "SI062"));
		...
		while ((SubscriptionPackage = ServiceInstance.getAnyNs(dvbi.e_SubscriptionPackage, ++sp)) != null) {
			if (SchemaVersion(documentNamespacee) >= slVersions.r3) {
		...
		if (SourceType) {
			...
			switch (SourceType.content) {
				...
				case dvbi.DVBAPPLICATION_SOURCE_TYPE:
					...
					if (hasDeliveryParameters(ServiceInstance)) {
						errs.addError({
							code: "SI156",
					...
					break;
				default:
					switch (SchemaVersion(documentNamespace)) {
						case slVersions.r0:
							errs.addError({
								code: "SI158",
			...
			if (v1Params && SchemaVersion(documentNamespace) >= slVersions.r1) errs.addError(DeprecatedElement(SourceType, SchemaSpecVersion(documentNamespace), "SI160"));
		} else {
			if (SchemaVersion(documentNamespace) == slVersions.r0)
				errs.addError({
					code: "SI161",
		...
		if (DVBTDeliveryParameters) {
			...
			if (DVBTtargetCountry) {
				...
				if (SchemaVersion(documentNamespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBTtargetCountry, SchemaSpecVersion(documentNamespace), "SI183"));
			}
		}
		...
		if (DVBCDeliveryParameters) {
			...
			if (DVBCtargetCountry) {
				...
				if (SchemaVersion(documentNamespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBCtargetCountry, SchemaSpecVersion(documentNamespace), "SI192"));
			}
			if (SchemaVersion(documentNamespace) >= slVersions.r7) {
	...
	/*private*/ #doSchemaVerification(ServiceList, errs, errCode, report_schema_version = true) {
		...
		if (x && x.schema) {
			SchemaCheck(ServiceList, x.schema, errs, `${errCode}:${SchemaVersion(ServiceList.root.namespaceUri)}`);
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		if (SchemaVersion(ns) >= slVersions.r3) slRequiredAttributes.push(tva.a_lang);
		if (SchemaVersion(ns) >= slVersions.r6) slRequiredAttributes.push(dvbi.a_id);
		...
		while ((RelatedMaterial = ServiceList.getAnyNs(tva.e_RelatedMaterial, ++rm)) != null) {
			...
			if (foundHref != "" && validServiceControlApplication(foundHref, SchemaVersion(ns))) countControlApps++;
		}
		...
		if (SchemaVersion(ns) >= slVersions.r5) {
		...
		if (LCNtableList) {
			...
			while ((LCNTable = LCNtableList.getAnyNs(dvbi.e_LCNTable, ++l)) != null) {
				...
				while ((SubscriptionPackage = LCNTable.getAnyNs(dvbi.e_SubscriptionPackage, ++sp)) != null) {
					...
					if (SchemaVersion(ns) >= slVersions.r5) {
						errs.addError(DeprecatedElement(SubscriptionPackage, SchemaSpecVersion(ns), "SL264"));
					}
					...
					} else if (SchemaVersion(ns) >= slVersions.r3) {
					...
					if (SchemaVersion(ns) >= slVersions.r3)
		...
		if (SchemaVersion(ns) >= slVersions.r4) {
