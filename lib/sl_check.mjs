...
import { isIni, unEntity, DuplicatedValue, parameterCheck, HexOrDecValue } from "./utils.mjs";
...
import { ValidateAnyContentDigests } from "./digest_validation.mjs";
...
export default class ServiceListCheck {
	...
	constructor(opts) {
		...
		if (!opts) opts = {};
		if (!Object.prototype.hasOwnProperty.call(opts, "useURLs")) opts.useURLs = false;
		...
		this.#knownCountries = opts?.countries || LoadCountries(opts);
		this.#knownLanguages = opts?.languages || LoadLanguages(opts);
		...
		this.#subtitleCarriages = opts?.stcarriage || LoadSubtitleCarriages(opts);
		this.#subtitleCodings = opts?.stcodings || LoadSubtitleCodings(opts);
		this.#subtitlePurposes = opts?.stpurposes || LoadSubtitlePurposes(opts);
		this.#allowedVideoSchemes = opts?.videofmts || LoadVideoCodecCS(opts);
		this.#allowedApplicationTypes = opts?.appfmts || LoadLinkedApplicationCS(opts);

		this.#allowedPictureFormats = LoadPictureFormatCS(opts);
		this.#allowedColorimetry = LoadColorimetryCS(opts);
		this.#allowedServiceTypes = LoadServiceTypeCS(opts);

		this.#allowedAudioConformancePoints = LoadAudioConformanceCS(opts);
		this.#allowedVideoConformancePoints = LoadVideoConformanceCS(opts);
	...
	/*private*/ #ValidateSynopsisType(Element, ElementName, requiredLengths, optionalLengths, parentLanguage, errs, errCode) {
		...
		Element.forEachNamedChildElement(ElementName, (ste) => {
			...
			if (synopsisLength) {
				...
				if (requiredLengths.includes(synopsisLength) || optionalLengths.includes(synopsisLength)) {
		...
		if (requiredLengths.includes(tva.SYNOPSIS_BRIEF_LABEL) && !hasBrief)
			errs.addError({
				code: `${errCode}-31`,
		...
		if (requiredLengths.includes(tva.SYNOPSIS_SHORT_LABEL) && !hasShort)
			errs.addError({
				code: `${errCode}-32`,
		...
		if (requiredLengths.includes(tva.SYNOPSIS_MEDIUM_LABEL) && !hasMedium)
			errs.addError({
				code: `${errCode}-33`,
		...
		if (requiredLengths.includes(tva.SYNOPSIS_LONG_LABEL) && !hasLong)
			errs.addError({
				code: `${errCode}-34`,
		...
		if (requiredLengths.includes(tva.SYNOPSIS_EXTENDED_LABEL) && !hasExtended)
			errs.addError({
				code: `${errCode}-35`,
	...
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, documentInfo, errs) {
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_ContentProtection, (ContentProtection) => {
			...
			ContentProtection.forEachNamedChildElement(dvbi.e_CASystemId, (CASystemID) => {
				...
				if (CASystemID_value) {
					const CASid_value = HexOrDecValue(CASystemID_value);
					...
					const CASid_Regexp = /^0x[0-9a-fA-F]{4}$/;
					if (!CASid_Regexp.test(CASystemID_value.trim()))
						errs.addError({
							code: "SI034",
							type: WARNING,
							message: `format of ${dvbi.e_CASystemId.elementize()} should be "0xHHHH"`,
							fragment: CASystemID,
						})
				}
			});
		...
		if (DVBCDeliveryParameters) {
			...
			if (SL_SchemaVersion(documentNamespace) >= slVersions.r7) {
				...
				DVBCDeliveryParameters.forEachNamedChildElement(dvbi.e_NetworkID, (NetworkID) => {
					const nid = HexOrDecValue(NetworkID.content);
					if (!Number.isNaN(nid) && DuplicatedValue(knownIDs, nid))
						errs.addError({
							code: "SI193",
							type: WARNING,
							key: keys.k_DuplicateValue,
							fragment: NetworkID,
							message: `duplicated Network ID value "${NetworkID.content}"`,
						});
				});
			}
		}
					...
					case sats.MODULATION_S2X:
						...
						if (ChannelBonding) {
							...
							ChannelBonding.forEachNamedChildElement(dvbi.e_Frequency, (Frequency) => {
								...
								if (Frequency_primary && isIni(["true"], Frequency_primary)) {
									if (primarySpecified)
										errs.addError({
											code: "SI206",
											key: ERROR_KEY,
		...
		// <ServiceInstance><IdentifierDeliveryParameters>
		const IdentifierBasedDeliveryParameters = ServiceInstance.getAnyNs(dvbi.e_IdentifierBasedDeliveryParameters);
	...
	/*private*/ #validateService(service, thisServiceId, documentInfo, errs) {
		...
		if (uID) {
			...
			if (DuplicatedValue(documentInfo.knownServices, thisServiceId))
				errs.addError({
					code: "SL111",
		...
		service.forEachNamedChildElement(dvbi.e_ServiceGenre, (ServiceGenre) => {
			...
			if (Genre_type && !tva.ALL_GENRE_TYPES.includes(Genre_type))
				errs.addError({
					code: "SL161",
		...
		if (ProminenceList) {
			...
			ProminenceList.forEachNamedChildElement(dvbi.e_Prominence, (PE) => {
				...
				if (!known.includes(hash1)) known.push(hash1);
				else {
				...
				if (PE_ranking) {
					...
					if (!known.includes(hash2)) known.push(hash2);
					else {
						const country = `${PE_country ? `country:${PE_country}` : ""}`,
							region = `${PE_region ? `region:${PE_region}` : ""}`;
						errs.addError({
	 ...
	 *                                             GERMAN_A177r6_VARIANT  use the German derived schema for A177r6
	 */
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		if (!Object.prototype.hasOwnProperty.call(options, "variants")) options.variants = 0;
		...
		if (LCNtableList) {
			...
			LCNtableList.forEachNamedChildElement(dvbi.e_LCNTable, (LCNTable) => {
				...
				LCNTable.forEachNamedChildElement(dvbi.e_LCN, (LCN) => {
					...
					if (LCN_serviceRef && !documentInfo.knownServices.has(LCN_serviceRef)) {
						errs.addError({
							code: "SL263",
