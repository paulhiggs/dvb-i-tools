...
import { validateAnyContentDigests } from "./related_material_checks.mjs";
		...
		const CheckEndpoint = (elementName, suffix, MustEndWithSlash = false) => {
			...
			if (ep) {
				...
				if (epURL) {
					...
					const validationPolicy = epURL.attrAnyNsValueOr(dvbi.a_verificationPolicy);
					if (validationPolicy && !isIn(documentInfo.signaturePolicyIDs, validationPolicy))
						errs.addError({
							type: WARNING,
							code: `${errCode}-${suffix}c`,
							message: `${validationPolicy.quote()} is not defined in this service list`,
							fragment: epURL,
