	...
	/*private*/ #checkSignalledApplication(MediaLocator, Location, AppType, errs) {
		...
		if (!MediaLocator)
			...
			MediaLocator.childNodes().forEachNamedSubElement(tva.e_MediaUri, (MediaUri) => {
				...
				const MediaUri_contentType = MediaQueryList.attrAnyNsValueOr(tva.a_contentType, null);
				if (AppType == dvbi.APP_SERVICE_PROVIDER && MediaUri_contentType && MediaUri_contentType != dvbi.XML_AIT_CONTENT_TYPE)
					errs.addError({
						code: "SA006",
						message: `invalid application type ${MediaUri.attrAnyNs(tva.a_contentType).value.quote()} for Serivce Provider Application (only XMLAIT allowed)`,
						fragment: MediaUri,
