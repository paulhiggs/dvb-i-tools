...
import { GetSchema, SchemaVersion, SchemaSpecVersion, ANY_NAMESPACE, isA177specification_URN } from "./sl_data_versions.mjs";
import {
...
export default class ServiceListCheck {
	...
	/*private*/ #validateRelatedMaterial(props, RelatedMaterial, Location, LocationType, errs, errCode) {
		...
		if (HowRelated.attrAnyNs(dvbi.a_href)) {
			...
			switch (LocationType) {
				...
				case SERVICE_RM:
					if (validContentFinishedBanner(HowRelated, ANY_NAMESPACE) && SchemaVersion(props.namespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-21`,
				...
				case SERVICE_INSTANCE_RM:
					...
					} else if (validOutScheduleHours(HowRelated, ANY_NAMESPACE) && SchemaVersion(props.namespace) >= slVersions.r6)
						errs.addError({
							code: `${errCode}-35`,
	...
	/*private*/ #validateAContentGuideSource(props, source, loc, errs, errCode) {
		...
		CheckEndpoint(dvbi.e_GroupInfoEndpoint, 18, SchemaVersion(props.namespace) >= slVersions.r5);
	...
	/*private*/ #validateServiceInstance(props, ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs) {
		...
		if (SchemaVersion(props.namespace) <= slVersions.r4) {
		...
		while ((RelatedMaterial = ServiceInstance.get(xPath(props.prefix, tva.e_RelatedMaterial, ++rm), props.schema)) != null) {
			...
			if (foundHref == dvbi.APP_IN_CONTROL) {
				...
				if (SchemaVersion(props.namespace) >= slVersions.r5 && hasDeliveryParameters(ServiceInstance, props.prefix, props.schema))
					errs.addError({
						type: WARNING,
						code: "SI022",
		...
		while ((ContentProtection = ServiceInstance.get(xPath(props.prefix, dvbi.e_ContentProtection, ++cp), props.schema)) != null) {
			...
			while ((CASystemID = ContentProtection.get(xPath(props.prefix, dvbi.e_CASystemId, ++ca), props.schema)) != null) {
				...
				if (SchemaVersion(props.namespace) <= slVersions.r1) {
		...
		if (ContentAttributes) {
			...
			while ((conf = ContentAttributes.get(xPath(props.prefix, dvbi.e_AudioConformancePoint, ++cp), props.schema)) != null) {
				if (SchemaVersion(props.namespace) > slVersions.r4) errs.addError(DeprecatedElement(conf, SchemaSpecVersion(props.namespace), "SI062"));
		...
		while ((SubscriptionPackage = ServiceInstance.get(xPath(props.prefix, dvbi.e_SubscriptionPackage, ++sp), props.schema)) != null) {
			if (SchemaVersion(props.namespace) >= slVersions.r3) {
		...
		if (SourceType) {
			...
			if (v1Params && SchemaVersion(props.namespace) >= slVersions.r1) errs.addError(DeprecatedElement(SourceType, SchemaSpecVersion(props.namespace), "SI160"));
		} else {
			if (SchemaVersion(props.namespace) == slVersions.r0)
				errs.addError({
					code: "SI161",
		...
		if (DVBTDeliveryParameters) {
			...
			if (DVBTtargetCountry) {
				...
				if (SchemaVersion(props.namespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBTtargetCountry, SchemaSpecVersion(props.namespace), "SI183"));
			}
		}
		...
		if (DVBCDeliveryParameters) {
			...
			if (DVBCtargetCountry) {
				...
				if (SchemaVersion(props.namespace) >= slVersions.r6) errs.addError(DeprecatedElement(DVBCtargetCountry, SchemaSpecVersion(props.namespace), "SI192"));
			}
			if (SchemaVersion(props.namespace) >= slVersions.r7) {
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		if (SchemaVersion(props.namespace) >= slVersions.r3) slRequiredAttributes.push(tva.a_lang);
		if (SchemaVersion(props.namespace) >= slVersions.r6) slRequiredAttributes.push(dvbi.a_id);
		...
		if (SchemaVersion(props.namespace) >= slVersions.r5) {
		...
		if (LCNtableList) {
			...
			while ((LCNTable = LCNtableList.get(xPath(props.prefix, dvbi.e_LCNTable, ++l), props.schema)) != null) {
				...
				while ((SubscriptionPackage = LCNTable.get(xPath(props.prefix, dvbi.e_SubscriptionPackage, ++sp), props.schema)) != null) {
					...
					} else if (SchemaVersion(props.namespace) >= slVersions.r3) {
