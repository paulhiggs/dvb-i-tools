...
import { slVersions, dvbi, dvbisld, dvbiEC, dvbiEA, XMLdocumentType, DvbIApplicationTypes } from "./DVB-I_definitions.mjs";
...
import { isMIME } from "./MIME_checks.mjs";
...
import { sl_InvalidHrefValue, InvalidURL, DeprecatedAttribute, DeprecatedElement, keys, InvalidCountryCode } from "./common_errors.mjs";
...
export default class ServiceListCheck {
	...
	/*private*/ #addRegion(Region, depth, knownRegionIDs, countries, errs) {
		...
		Region.forEachNamedChildElement(dvbi.e_Region, (RegionChild) => this.#addRegion(RegionChild, depth + 1, knownRegionIDs, countriesSpecified, errs));
	}
		...
		if (HowRelated.attrAnyNs(dvbi.a_href)) {
			...
			switch (LocationType) {
				...
				case SERVICE_RM:
					...
					const HowRelated1_href = HowRelated.attrAnyNsValueOr(dvbi.a_href);
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNamespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-21`,
							message: `${HowRelated1_href.quote()} not permitted for ${documentNamespace.quote()} in ${Location}`,
				...
				case SERVICE_INSTANCE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated2_href = HowRelated.attrAnyNsValueOr(dvbi.a_href);
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNamespace) == slVersions.r0)
						errs.addError({
							code: `${errCode}-31`,
							message: `${HowRelated2_href.quote()} not permitted for ${documentNamespace.quote()} in ${Location}`,
							key: "invalid CS value",
							fragment: HowRelated,
						});
				...
				case CONTENT_GUIDE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated3_href = HowRelated.attrAnyNsValueOr(dvbi.a_href);
	 ...
	 * @param {ErrorList}  errs                          errors found in validaton
	 */
	/*private*/ #validateServiceInstance(ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs) {		if (!parameterCheck("validateServiceInstance", ServiceInstance, dvbi.e_ServiceInstance, errs, "SI000a")) return;
		...
		ServiceInstance.forEachNamedChildElement(dvbi.e_ContentProtection, (ContentProtection) => {
			...
			ContentProtection.forEachNamedChildElement(dvbi.e_DRMSystemId, (DRMSystemID) => {
				...
				if (DRMSystemID_value && DRMSystemID_value.length == 0) {
					if (DRMSystemID_value.startsWith(UUID_type) && !isUUIDformat(DRMSystemID_value.substring(UUID_type.length))) {
						errs.addError({
							code: "SI043",
	...
	/*private*/ #validateService(service, thisServiceId, knownServices, knownRegionIDs, declaredSubscriptionPackages, declaredAudioLanguages, ContentGuideSourceIDs, errs) {
		...
		if (uID) {
			...
			if (!SL_helpers.uniqueServiceIdentifier(thisServiceId, knownServices))
				errs.addError({
					code: "SL111",
	...
	/*private*/ #ValidateRequirements(Requires, errs, errCode) {
		...
		const _documentNamespace = Requires.documentNamespace();
		if (Delivery) {
			Delivery.forEachNamedChildElement(dvbisld.e_DVBTDelivery, (DVBTDelivery) => {
				if (DVBTDelivery.attr(dvbisld.a_originalNetworkID) && SL_SchemaVersion(_documentNamespace) >= slVersions.r6) {
			...
			Delivery.forEachNamedChildElement(dvbisld.e_DVBSDelivery, (DVBSDelivery) => {
				if (DVBSDelivery.attr(dvbisld.a_originalNetworkID) && SL_SchemaVersion(_documentNamespace) >= slVersions.r6) {
			...
			if (ApplicationDelivery) {
				ApplicationDelivery.forEachNamedChildElement(dvbisld.e_ApplicationType, (ApplicationType) => {
					ApplicationType.content.replace(/\t/g, " ").split(" ").forEach((appType) => {
						const type_clean = appType.trim();
						if (type_clean.length > 0 && !this.#allowedApplicationTypes.isLeaf(type_clean)) {
							errs.addError({
								code: `${errCode}-43`,
								message: `${type_clean.quote()} is not a valid application type`,
								fragment: ApplicationType,
								key: `invalid application type`,
								clause: "A177 Table 12h",
								description: "Values shall be taken from the LinkedApplicationCS.",
							});
						}
					});
					const Application_contentType = ApplicationType.attrAnyNsValueOr(dvbisld.a_contentType);
					if (Application_contentType) {
						if (!SL_helpers.isValidApplicationType(Application_contentType)) {
							errs.addError({
								code: `${errCode}-45`,
						...
						if (Application_contentType == dvbi.XML_AIT_CONTENT_TYPE) {
							const xmlAitApplicationType = ApplicationType.attrAnyNsValueOr(dvbisld.a_xmlAitApplicationType);
	...
	/*public*/ doValidateServiceList(SLtext, errs, options = {}) {
		...
		const thisServiceListId = ServiceList.attrAnyNsValueOr(dvbi.a_id);
		if (thisServiceListId) {
			if (!SL_helpers.validServiceListIdentifier(thisServiceListId))
				errs.addError({
					code: "SL016",
		...
		if (RegionList) {
			...
			RegionList.forEachNamedChildElement(dvbi.e_Region, (Region) => this.#addRegion(Region, 0, knownRegionIDs, null, errs));
		}
