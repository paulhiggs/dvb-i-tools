...
export function LoadKnownCustomKeysRegistry(opts) {
	opts.verbose && console.log(chalk.yellow.underline("loading known CMCD Custom keys ..."));

	if (opts.useURLs) {
		opts.verbose && console.log(chalk.yellow(`retrieving known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.url}`));
		if (opts.async)
		...
		opts.verbose && console.log(chalk.yellow(`reading known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.file}`));
		if (opts.async) 
			readFile(
