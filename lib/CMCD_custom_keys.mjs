...
export function LoadKnownCustomKeysRegistry(useURL, async = true, verbose = true) {
	verbose && console.log(chalk.yellow.underline("loading known CMCD Custom keys ..."));

	if (useURL) {
		verbose && console.log(chalk.yellow(`retrieving known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.url}`));
		if (async)
		...
		verbose && console.log(chalk.yellow(`reading known keys from ${CMCD_CUSTOM_KEYS_REGISTRY.file}`));
		if (async) 
			readFile(
