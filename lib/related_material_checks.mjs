...
import {  INFORMATION, WARNING } from "./error_list.mjs";
 ...
 *  
 * @param {Xmlelement} element  The XML element possibly containing the @contentDigest attribute
...
export function ValidateAnyContentDigests(element, errs, errCode) {
	...
	const contentDigests = element.attrAnyNsValueOr(tva.a_contentDigest, null);
	if (contentDigests) {
		contentDigests.split(",").forEach( (contentDigest) => {
			const digest = contentDigest.split("="); // algorithm in digest[0], digest in value[1]
			if (!isIn(dvbi.ALLOWED_DIGESTS, digest[0]))
				errs.addError({
					code: errCode,
					message: `invalid digest algorithm specified (${digest[0]})`,
					fragment: element,
					key: `invalid ${tva.a_contentDigest}`,
			})
		})
	}
}
...
export function ValidateAnySignaturePolicy(element, documentInfo, errs, errCode) {
	...
	if (!Object.prototype.hasOwnProperty.call(documentInfo, "signaturePolicyIDs")) {
		errs.addError({
			type: INFORMATION,
			code: `${errCode}-1`,
	...
	if (policy && !isIn(documentInfo.signaturePolicyIDs, policy))
		errs.addError({
			type: WARNING,
			code: `${errCode}-2`,
...
function validateImageRelatedMaterial(RelatedMaterial, location, allowedHowRelated, documentInfo, errs, errCode) {
	...
	MediaLocator.forEachNamedChildElement(tva.e_MediaUri, (MediaUri) => {
		hasMediaURI = true;
		checkAttributes(MediaUri, [tva.a_contentType], [tva.a_contentDigest], tvaEA.MediaUri, errs, `${errCode}-22`);
	...
	if (MediaLocator.attrAnyNs(dvbi.a_contentLanguage)) 
		checkLanguage(MediaLocator.attrAnyNs(dvbi.a_contentLanguage).value, MediaLocator, errs, `${errCode}-28`);
	if (!hasMediaURI)
		errs.addError({
			code: `${errCode}-29`,
