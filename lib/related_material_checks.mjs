...
function validateImageRelatedMaterial(RelatedMaterial, errs, errCode, location, allowedHowRelated) {
	...
	const hrHref = HowRelated.attrAnyNs(tva.a_href);
	...
	if (Format) {
		...
		Format.childNodes().forEachNamedSubElement(tva.e_StillPictureFormat, (child) => {
			...
			const childHref = child.attrAnyNs(tva.a_href);
			if (childHref)
				switch (childHref.value) {
					...
					case mpeg7.PNG_IMAGE_CS_VALUE:
						...
						break;
					default:
						cg_InvalidHrefValue(childHref.value, child, `${RelatedMaterial.name}.${tva.e_Format}.${tva.e_StillPictureFormat}`, location, errs, `${errCode}-13`);
				}
		});
	}
...
export function checkValidLogos(RelatedMaterial, errs, errCode, location) {
	...
	RelatedMaterial.childNodes().forEachNamedSubElement(tva.e_MediaLocator, (MediaLocator) => {
		...
		MediaLocator.childNodes().forEachNamedSubElement(tva.e_MediaUri, (MediaUri) => {
			...
			const attrCT = MediaUri.attrAnyNs(tva.a_contentType);
			if (attrCT) {
				const contentType = attrCT.value;
