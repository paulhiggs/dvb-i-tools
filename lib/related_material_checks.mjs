...
import { cgVersions } from "./DVB-I_definitions.mjs";
...
export function ValidateAnyContentDigests(element, errs, errCode) {
	...
	const contentDigests = element.attrAnyNsValueOr(tva.a_integrity, null);
	if (contentDigests) {
...
export function ValidateAnySignaturePolicy(element, documentInfo, errs, errCode) {
	...
	if (!isIn(documentInfo.signaturePolicyIDs, policy))
		errs.addError({
			type: WARNING,
			code: `${errCode}-2`,
