...
export function validateAnyContentDigests(element, errs, errCode) {
	if (!element) {
		errs.addError({ type: APPLICATION, code: "VD000", message: "ValidateDigests() called with element==null" });
		return;
	}
...
function validateImageRelatedMaterial(RelatedMaterial, location, allowedHowRelated, errs, errCode) {
	...
	if (MediaLocator.attrAnyNs(dvbi.a_contentLanguage)) 
		checkLanguage(MediaLocator.attrAnyNs(dvbi.a_contentLanguage).value, MediaLocator, errs, `${errCode}-27`);
	if (!hasMediaURI)
		errs.addError({
			code: `${errCode}-27`,
