...
export const DeprecatedElement = (what: XmlElement, since: string, errCode: string) => ({
	...
	code: errCode,
	fragment: what,
	message: `Element ${what.name.elementize()} in ${what.parent?.name.elementize() || "__unknown__"} is deprecated since ${since}`,
 ...
 * @param {string} loc      The location of the element
 */
export const InvalidCountryCode = (value: string, src: string, loc: string) : string => 
