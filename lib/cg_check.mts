...
import { CURRENT, OLD } from "./globals.mts";
import {
	LoadGenres,
	LoadRatings,
	LoadVideoCodecCS,
 ...
 */
let CG_SchemaVersion = (namespace : string) : number => {
 ...
 */
function AllowedValue(elem : XmlElement, attrName : string, allowed : Array<string>, errs : ErrorList, errCode : string, isRequired : boolean = true) {
	if (!elem) {
		errs.addError({ type: APPLICATION, code: `${errCode}-0`, message: "AllowedValue() called with elem==null" });
		return;
	}

	const attributeValue = elem.attrAnyNsValueOrNull(attrName);
	const location = `${elem.parent?.name ? `${elem.parent.name}.` : ""}${elem.name}`
	if (attributeValue) {
		if (!isIn(allowed, attributeValue))
			errs.addError({
				code: `${errCode}-1`,
				message: `${attrName.attribute(location)} must be ${allowed.join(" or ")}`,
				fragment: elem,
			});
	} else if (isRequired)
		errs.addError({
			code: `${errCode}-2`,
			message: `${attrName.attribute()} must be specified for ${location}`,
			fragment: elem,
		});
}
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
let BooleanValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => AllowedValue(elem, attrName, ["true", "false"], errs, errCode, isRequired);
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
let TrueValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => AllowedValue(elem, attrName, ["true"], errs, errCode, isRequired);
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
let FalseValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => AllowedValue(elem, attrName, ["false"], errs, errCode, isRequired);
...
export default class ContentGuideCheck {
  ...
  supportedRequests;
	...
	/*private*/ #doSchemaVerification(TVAdoc : XmlDocument, errs : ErrorList, errCode : string, report_schema_version : boolean = true) : boolean {
		...
		const x = SchemaVersions.find((s) => s.namespace == (TVAdoc.root as XmlElement).documentNamespace());
		if (x && x.schema) {
			SchemaCheck(TVAdoc, x.schema, x.filename, errs, `${errCode}:${CG_SchemaVersion((TVAdoc.root as XmlElement).documentNamespace())}`);
	...
	/* private */ #checkTAGUri(elem : XmlElement, errs : ErrorList, errCode : string ) : string {
		if (elem) {
			const svcID = elem.attrAnyNsValueOrNull(tva.a_serviceIDRef);
			if (svcID && !isTAGURI(svcID))
				errs.addError({
					type: WARNING,
					code: errCode,
	 ...
	 * @param {String}     errCode             error code prefix to be used in reports
	 */
	/* private */ #ValidateSynopsis(BasicDescription : XmlElement, requiredLengths : Array<string>, optionalLengths : Array<string>, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "SY000",
		...
		let synopsisLengthError = (label : string, length : number, actual : number) => `length of ${tva.a_length.attribute(tva.e_Synopsis)}=${label.quote()} exceeds ${length} characters, measured(${actual})`;
		let singleLengthLangError = (length : string, lang : string) => `only a single ${tva.e_Synopsis.elementize()} is permitted per length (${length}) and language (${lang})`;
		let requiredSynopsisError = (length : string) => `a ${tva.e_Synopsis.elementize()} with ${tva.a_length.attribute()}=${length.quote()} is required`;
		...
		while ((Synopsis = BasicDescription.getAnyNs(tva.e_Synopsis, ++s)) != null) {
			...
			const synopsisLength = Synopsis.attrAnyNsValueOrNull(tva.a_length);
