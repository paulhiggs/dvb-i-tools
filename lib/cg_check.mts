...
import { cgVersions, slVersions, dvbi } from "./DVB-I_definitions.mts";

import { TVAschema } from "./data_locations.mts";
import { documentNamespace, attrAnyNsValueOr, attrAnyNsValueOrNull, forEachChildElement, forEachNamedChildElement, getAnyNs, attrAnyNs, countChildElements, hasChild } from "../libxml2-wasm-extensions.mts";

import ErrorList, { WARNING, APPLICATION } from "./error_list.mts";
import { isIn, isIni, unEntity, DuplicatedValue, parameterCheck } from "./utils.mts";
...
import { LoadCredits } from "./credits_loader.mts";
...
type KnownRequestType = {
	value: string;
	label: string;
};
const supportedRequests : KnownRequestType[]= [
...
const SchemaVersions : SchemaVersion_type[] = [
	{
		namespace: TVAschema.v2026.namespace,
		version: cgVersions.r3,
		filename: TVAschema.v2026.file,
		schema: null,
 ...
 */
const CG_SchemaVersion = (namespace : string) : number => {
 ...
 */
const valUnsignedInt = (str : string) : number => {
	const intRegex = /[\d]+/;
	const s = str.match(intRegex);
	return s && s[0] === str ? parseInt(str, 10) : 0;
};
 ...
 */
function AllowedValue(elem : XmlElement, attrName : string, allowed : string[], errs : ErrorList, errCode : string, isRequired : boolean = true) {
	if (!parameterCheck("AllowedValue", elem, undefined, errs, "AV000")) return;

	const attr_value = attrAnyNsValueOrNull(elem, attrName);
	if (attr_value) {
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
const BooleanValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => 
	AllowedValue(elem, attrName, ["true", "false"], errs, errCode, isRequired);
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
const TrueValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => 
	AllowedValue(elem, attrName, ["true"], errs, errCode, isRequired);
 ...
 * @param {boolean}    isRequired true if the specified attribute is required to be specified for the element
 */
const FalseValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean = true) => 
 ...
 */
export const isRestartAvailability = (genre : string) : boolean => 
...
export default class ContentGuideCheck {
	...
	private audioPresentationCSvalues : ClassificationScheme;
	private allowedCreditItemRoles : ClassificationScheme;
	private allowedRatings : ClassificationScheme;
	...
	private supportedRequests : KnownRequestType[] = supportedRequests;
	...
	constructor(useURLs : boolean, opts : ValidatorInitializationOptions, async : boolean = true) {
		...
		SchemaVersions.forEach((version) => {
			...
			const schema = readFileSync(version.filename);
	...
	private doSchemaVerification(TVAdoc : XmlDocument, errs : ErrorList, errCode : string, report_schema_version : boolean = true) {
		let _rc = true;

		const x = SchemaVersions.find((s) => s.namespace == documentNamespace(TVAdoc.root));
		if (x && x.schema) {
			SchemaCheck(TVAdoc, x.schema, x.filename, errs, `${errCode}:${CG_SchemaVersion(documentNamespace(TVAdoc.root))}`);
	 ...
	 */
	private checkTAGUri(elem : XmlElement, errs : ErrorList, errCode : string) : string {
		const svcID = attrAnyNsValueOrNull(elem, tva.a_serviceIDRef);
		if (svcID) {
			if (!isTAGURI(svcID))
				errs.addError({
					type: WARNING,
	...
	private ValidateSynopsis(BasicDescription : XmlElement, requiredLengths : string[], optionalLengths : string[], errs : ErrorList, errCode : string) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_Synopsis, (Synopsis) => {
			...
			const synopsisLength = attrAnyNsValueOrNull(Synopsis, tva.a_length);
	...
	private ValidateKeyword(BasicDescription : XmlElement, minKeywords : number, maxKeywords : number, errs : ErrorList, errCode : string) {
		...
		const counts = [];
		forEachNamedChildElement(BasicDescription, tva.e_Keyword, (Keyword) => {
			checkAttributes(Keyword, [], [tva.a_lang, tva.a_type], tvaEA.Keyword, errs, `${errCode}-1`);

			const keywordType = attrAnyNsValueOr(Keyword, tva.a_type, tva.DEFAULT_KEYWORD_TYPE);
	...
	private ValidateGenre(BasicDescription : XmlElement, errs : ErrorList, errCode : string) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_Genre, (Genre) => {
			const genreType = attrAnyNsValueOr(Genre, tva.a_type, tva.DEFAULT_GENRE_TYPE);
			...
			const genreValue = attrAnyNsValueOrNull(Genre, tva.a_href);
	...
	private ValidateParentalGuidance(BasicDescription : XmlElement, errs : ErrorList, errCode : string) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_ParentalGuidance, (ParentalGuidance) => {
			let pgCountries = UNSPECIFIED_COUNTRY;
			if (hasChild(ParentalGuidance, tva.e_CountryCodes)) 
				pgCountries = getAnyNs(ParentalGuidance, tva.e_CountryCodes).content;
			...
			pgCountriesList.forEach((pgCountry) => {
				...
				if (pgCountry != UNSPECIFIED_COUNTRY && !this.knownCountries.isISO3166code(pgCountry))
					errs.addError({
						...
						fragment: getAnyNs(ParentalGuidance, tva.e_CountryCodes),
					});

				// first <ParentalGuidance> element must contain an <mpeg7:MinimumAge> element
				forEachChildElement(ParentalGuidance, (pgChild) => {
					switch (pgChild.name) {
						...
						case tva.e_ParentalRating:
							...
							// eslint-disable-next-line no-case-declarations
							const rating = attrAnyNsValueOrNull(pgChild, tva.a_href);
							if (rating) {
						...
						case tva.e_ExplanatoryText:
							...
							// eslint-disable-next-line no-case-declarations
							const lengthAttr = attrAnyNsValueOrNull(pgChild, tva.a_length);
							if (lengthAttr && lengthAttr != tva.v_lengthLong)
								errs.addError({
									code: `${errCode}-31`,
									message: `${tva.a_length.attribute()}=${lengthAttr.quote()} is not allowed for ${tva.e_ExplanatoryText.elementize()}`,
									fragment: pgChild,
									key: keys.k_LengthError,
								});
	...
	private ValidateCreditsList(BasicDescription : XmlElement, errs : ErrorList, errCode : string) {
		...
		function ValidateName(elem : XmlElement, errs : ErrorList, errCode : string) {
			if (!parameterCheck("ValidateName", elem, [tva.e_PersonName, tva.e_Character], errs, "VN000")) return;

			const checkNamePart = (elem : XmlElement, errs : ErrorList, errCode : string) => {
			...
			let givenNameCount = 0;

			forEachChildElement(elem, (child : XmlElement) => {
				switch (child.name) {
		...
		const CreditsList = getAnyNs(BasicDescription, tva.e_CreditsList);
		if (CreditsList) {
			let numCreditsItems = 0;
			forEachNamedChildElement(CreditsList, tva.e_CreditsItem, (CreditsItem : XmlElement) => {
				numCreditsItems++;
				checkAttributes(CreditsItem, [tva.a_role], [], tvaEA.CreditsItem, errs, `${errCode}-1`);
				const CreditsItemRole = attrAnyNsValueOrNull(CreditsItem, tva.a_role);
				if (CreditsItemRole) {
				...
				const foundPersonName = [],
					foundCharacter = [],
					foundOrganizationName = [];

				checkTopElementsAndCardinality(CreditsItem, 
					[ { name: tva.e_PersonName, minOccurs: 0 },
						{ name: tva.e_Character, minOccurs: 0 },
						{ name: tva.e_OrganizationName, minOccurs: 0 }], 
					tvaEC.CreditsItem, false, errs, `${errCode}-10`);

				forEachChildElement(CreditsItem, (child : XmlElement) => {
					switch (child.name) {
				...
				const singleElementError = (elementName : string, parentElementName : string) => 
	...
	private ValidateRelatedMaterial_PromotionalStillImage(BasicDescription : XmlElement, errs : ErrorList) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_RelatedMaterial, (RelatedMaterial) => 
			ValidatePromotionalStillImage(RelatedMaterial, `${BasicDescription.name.elementize()} in ${BasicDescription.parent.name.elementize()}`, errs, "RMPSI001")
		);
	}
	...
	private ValidateRelatedMaterial_Pagination(BasicDescription : XmlElement, Location : string, errs : ErrorList) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_RelatedMaterial, (RelatedMaterial) => {
			...
			if (!HowRelated) errs.addError(NoChildElement(tva.e_HowRelated.elementize(), RelatedMaterial, Location, "VP001"));
				...
				const HowRelated_href = attrAnyNsValueOrNull(HowRelated, tva.a_href);
				...
				const MediaLocator = getAnyNs(RelatedMaterial, tva.e_MediaLocator);
				const MediaURI = MediaLocator ? getAnyNs(MediaLocator, tva.e_MediaUri) : null;
				if (MediaURI) {
	...
	private ValidateRelatedMaterial_MoreEpisodes(BasicDescription : XmlElement, errs : ErrorList) {
		...
		switch (BasicDescription.parent.name) {
			case tva.e_ProgramInformation:
				forEachNamedChildElement(BasicDescription, tva.e_RelatedMaterial, (RelatedMaterial) =>
					ValidatePromotionalStillImage(RelatedMaterial, "More Episodes", errs, "RMME001")
				);
				break;
	...
	private ValidateTemplateAIT(RelatedMaterial : XmlElement, Location : string, errs : ErrorList) {
		...
		let HowRelated : XmlElement = null;
		const MediaLocator : XmlElement[] = [];

		forEachChildElement(RelatedMaterial, (child : XmlElement) => {
			switch (child.name) {
			...
			if (HowRelated_href != dvbi.TEMPLATE_AIT_URI)
				...
				if (MediaLocator.length != 0)
					MediaLocator.forEach((locator) => {
						...
						forEachNamedChildElement(locator, tva.e_AuxiliaryURI, (AuxiliaryURI) => {
							hasAuxiliaryURI = true;
							checkAttributes(AuxiliaryURI, [tva.a_contentType], [], tvaEA.AuxiliaryURI, errs, "TA010");
							const contentType = attrAnyNsValueOrNull(AuxiliaryURI, tva.a_contentType);
	...
	private ValidateRelatedMaterial_BoxSetList(BasicDescription : XmlElement, errs : ErrorList) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_RelatedMaterial, (RelatedMaterial) => {
			...
			if (!HowRelated) errs.addError(NoChildElement(tva.e_HowRelated.elementize(), RelatedMaterial, null, "MB009"));
				...
				const hrHref = attrAnyNsValueOrNull(HowRelated, tva.a_href);
				switch (hrHref) {
					case null:
						break;
					...
					case tva.cs_PromotionalStillImage:
						...
						ValidatePromotionalStillImage(RelatedMaterial, "Box Set List", errs, "MB012");
						break;
					default:
		...
		forEachNamedChildElement(BasicDescription, tva.e_RelatedMaterial, (RelatedMaterial) => {
			...
			if (!HowRelated) errs.addError(NoChildElement(tva.e_HowRelated.elementize(), RelatedMaterial, null, "MC009"));
				...
				switch (hrHref) {
					...
					case tva.cs_PromotionalStillImage:
						countImage.push(HowRelated);
						ValidatePromotionalStillImage(RelatedMaterial, "Box Set Contents", errs, "MC012");
						break;
					default:
	...
	private ValidateTitle(BasicDescription : XmlElement, allowSecondary : boolean, TypeIsRequired : boolean, errs : ErrorList, errCode : string) {
		...
		forEachNamedChildElement(BasicDescription, tva.e_Title, (Title) => {
			checkAttributes(Title, requiredAttributes, optionalAttributes, tvaEA.Title, errs, `${errCode}-1`);

			const titleType = attrAnyNsValueOr(Title, tva.a_type, mpeg7.DEFAULT_TITLE_TYPE);
	...
	private ValidateReleaseInformation(BasicDescription : XmlElement, errs : ErrorList, errCode : string) {
		...
		const locations = [];
		forEachNamedChildElement(BasicDescription, tva.e_ReleaseInformation, (release) => {
			if (!hasChild(release, tva.e_ReleaseDate) && !hasChild(release, tva.e_ReleaseLocation)) {
				...
				const location = getAnyNs(release, tva.e_ReleaseLocation);
	...
	private ValidateBasicDescription(parentElement : XmlElement, requestType : string, categoryGroup : XmlElement | null, errs : ErrorList) {
		...
		const BasicDescription = getAnyNs(parentElement, tva.e_BasicDescription);
		if (!BasicDescription) {
	 ...
	 */
	private ValidateProgramInformation(ProgramInformation : XmlElement, programCRIDs : string[], groupCRIDs : string[], requestType : string, indexes : string[], errs : ErrorList) : string | null {
		...
		const programCRID = attrAnyNsValueOrNull(ProgramInformation, tva.a_programId);
		if (programCRID) {
		...
		forEachChildElement(ProgramInformation, (child) => {
			switch (child.name) {
				...
				case tva.e_EpisodeOf: // <ProgramInformation><EpisodeOf>
					...
					foundCRID = attrAnyNsValueOrNull(child, tva.a_crid);
					if (foundCRID) {
				...
				case tva.e_MemberOf: // <ProgramInformation><MemberOf>
					// eslint-disable-next-line no-case-declarations
					const child_type = attrAnyNsValueOrNull(child, tva.a_type),
						child_crid = attrAnyNsValueOrNull(child, tva.a_crid);
					if ([CG_REQUEST_SCHEDULE_NOWNEXT, CG_REQUEST_SCHEDULE_WINDOW].includes(requestType)) {
						...
						if (child_crid && child_crid == dvbi.CRID_NOW) isCurrentProgram = true;
					...
					if (child_type && child_type != tva.t_MemberOfType)
						errs.addError({
							code: "PI043",
					...
					// eslint-disable-next-line no-case-declarations
					const MO_index = attrAnyNsValueOrNull(child, tva.a_index);
					if (MO_index) {
	 ...
	 */
	private CheckProgramInformation(ProgramDescription : XmlElement, programCRIDs : string[], groupCRIDs : string[], requestType : string, errs : ErrorList, o : { childCount : number } | null = null) {
		if (!parameterCheck("CheckProgramInformation", ProgramDescription, tva.e_ProgramDescription, errs, "CPI000")) return null;

		const ProgramInformationTable = getAnyNs(ProgramDescription, tva.e_ProgramInformationTable);
		...
		const indexes = [];
		forEachNamedChildElement(ProgramInformationTable, tva.e_ProgramInformation, (ProgramInformation) => {
	 ...
	 * @param {Set}        indexes            an accumulation of the @index values found
	 * @param {Array}      groupsFound        groupId values found (null if not needed)
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	private ValidateGroupInformationBoxSets(GroupInformation : XmlElement, requestType : string, categoryGroup : XmlElement | null, indexes : Set<number>, groupsFound : string[] | null, errs : ErrorList) {
		...
		const groupId = attrAnyNsValueOrNull(GroupInformation, tva.a_groupId);
		if (groupId && isCRIDURI(groupId) && groupsFound) groupsFound.push(groupId);

		const categoryCRID = categoryGroup ? attrAnyNsValueOr(categoryGroup, tva.a_groupId, "") : "";

		if (!isParentGroup) {
			const MemberOf = getAnyNs(GroupInformation, tva.e_MemberOf);
			if (MemberOf) {
				checkAttributes(MemberOf, [tva.a_type, tva.a_index, tva.a_crid], [], tvaEA.MemberOf, errs, "GIB041");
				const member = attrAnyNsValueOrNull(MemberOf, tva.a_type);
				if (member && member != tva.t_MemberOfType) 
					errs.addError({
						code: "GIB042",
						message: `${GroupInformation.name}.${tva.e_MemberOf}@xsi:${tva.a_type} is invalid (${attrAnyNsValueOr(MemberOf, tva.a_type, "!!oops!!").quote()})`,
						fragment: MemberOf,
					});
				const MemberOf_index = attrAnyNsValueOrNull(MemberOf, tva.a_index);
				if (MemberOf_index) {
				...
				const crid = attrAnyNsValueOrNull(MemberOf, tva.a_crid);
	...
	private ValidateGroupInformationSchedules(GroupInformation : XmlElement, requestType : string, categoryGroup : XmlElement | null, errs : ErrorList) {
		...
		if ([CG_REQUEST_SCHEDULE_NOWNEXT, CG_REQUEST_SCHEDULE_WINDOW].includes(requestType)) {
			...
			if (!attrAnyNs(GroupInformation, tva.a_numOfItems))
				errs.addError({
					code: "GIS015",
	...
	private ValidateGroupInformationMoreEpisodes(GroupInformation : XmlElement, requestType : string, categoryGroup : XmlElement | null, groupsFound : string[] | null, errs : ErrorList) {
		...
		const GroupInformation_groupId = attrAnyNsValueOrNull(GroupInformation, tva.a_groupId);
		if (GroupInformation_groupId) {
			...
			else groupsFound.push(GroupInformation_groupId);
		}

		TrueValue(GroupInformation, tva.a_ordered, errs, "GIM004", false);

		const GroupType = getAnyNs(GroupInformation, tva.e_GroupType);
		if (GroupType) {
			checkAttributes(GroupType, [tva.a_type, tva.a_value], [], tvaEA.GroupType, errs, "GIM011");

			const GroupType_type = attrAnyNsValueOrNull(GroupType, tva.a_type);
			...
			const GroupTYpe_value = attrAnyNsValueOrNull(GroupType, tva.a_value);
	 ...
	 * @param {ErrorList}  errs               errors found in validaton
	 */
	private ValidateGroupInformation(GroupInformation : XmlElement, requestType : string, categoryGroup : XmlElement | null, indexes : Set<number>, groupsFound : string[] | null, errs : ErrorList) {
	...
	private CheckGroupInformation(ProgramDescription : XmlElement, requestType : string, groupIds : string[] | null, errs : ErrorList, o : { childCount : number } | null = null) {
		...
		const GroupInformationTable = getAnyNs(ProgramDescription, tva.e_GroupInformationTable);
		...
		let categoryGroup: XmlElement = null;
		if ([CG_REQUEST_BS_LISTS, CG_REQUEST_BS_CATEGORIES].includes(requestType)) {
			forEachNamedChildElement(GroupInformationTable, tva.e_GroupInformation, (GroupInformation) => {
		...
		if (categoryGroup) {
			const numOfItems = attrAnyNs(categoryGroup, tva.a_numOfItems) ? valUnsignedInt(attrAnyNs(categoryGroup, tva.a_numOfItems).value) : 0;
	...
	private CheckGroupInformationBoxsetContents(ProgramDescription : XmlElement, requestType : string, groupIds : string[] | null, errs : ErrorList, _o : { childCount : number } | null = null) {
		...
		// find which GroupInformation element is the "category group"
		let contentsGroup : XmlElement = null;
		forEachNamedChildElement(GroupInformationTable, tva.e_GroupInformation, (GroupInformation) => {
			...
			if (attrAnyNs(GroupInformation, tva.a_groupId)) groupIds.push(attrAnyNs(GroupInformation, tva.a_groupId).value);
		});
		if (!contentsGroup)
			errs.addError({
				code: "GIC115",
		...
		if (contentsGroup) {
			const cgCRID = attrAnyNsValueOr(contentsGroup, tva.a_groupId, "none");
			forEachNamedChildElement(GroupInformationTable, tva.e_GroupInformation, (GroupInformation) => {
				if (GroupInformation.line != contentsGroup.line) {
					const MemberOf = getAnyNs(GroupInformation, tva.e_MemberOf);
					if (MemberOf) {
						const MemberOf_crid = attrAnyNsValueOrNull(MemberOf, tva.a_crid);
						...
						const MemberOf_type = attrAnyNsValueOrNull(MemberOf, tva.a_type);
	...
	private ValidateGroupInformationNowNext(GroupInformation : XmlElement, requestType : string, numEarlier : number, numNow : number, numLater : number, groupCRIDsFound : string[], errs : ErrorList) {
		...
		if (GroupInformation_groupId) {
			if (
				...
				const numOfItems = attrAnyNs(GroupInformation, tva.a_numOfItems) ? valUnsignedInt(attrAnyNs(GroupInformation, tva.a_numOfItems).value) : -1;
	...
	private ValidateAVAttributes(AVAttributes : XmlElement, AudioCodecCS : ClassificationScheme, errs : ErrorList) {
		...
		const isValidAudioMixType = (mixType : string) : boolean => [mpeg7.AUDIO_MIX_MONO, mpeg7.AUDIO_MIX_STEREO, mpeg7.AUDIO_MIX_5_1].includes(mixType);
		...
		if (CG_SchemaVersion(documentNamespace(AVAttributes)) < cgVersions.r2)
		...
		const isValidAudioLanguagePurpose = (purpose : string) : boolean=> profiledAudioPurposeCS.includes(purpose);

		checkTopElementsAndCardinality(
			AVAttributes,
			CG_SchemaVersion(documentNamespace(AVAttributes)) >= cgVersions.r2
				? [
		...
		const foundAttributes = [],
			audioCounts = [];
		forEachNamedChildElement(AVAttributes, tva.e_AudioAttributes, (AudioAttributes) => {
			checkTopElementsAndCardinality(
				AudioAttributes,
				[
			...
			const Coding = getAnyNs(AudioAttributes, tva.e_Coding);
			let Coding_val = undefined;
			if (Coding) {
				checkAttributes(Coding, [tva.a_href], [], tvaEA.Coding, errs, "AV006");

				if (attrAnyNs(Coding, tva.a_href)) {
					Coding_val = attrAnyNs(Coding, tva.a_href).value;
					if (AudioCodecCS.count() && !AudioCodecCS.isLeaf(Coding_val))
						errs.addError({
							code: "AV007",
			...
			const MixType = getAnyNs(AudioAttributes, tva.e_MixType);
			let MixType_href = undefined;
			if (MixType) {
				checkAttributes(MixType, [tva.a_href], [], tvaEA.MixType, errs, "AV011");
				MixType_href = attrAnyNsValueOrNull(MixType, tva.a_href);
				if (MixType_href) {
					if (!isValidAudioMixType(MixType_href))
						errs.addError({
							code: "AV012",
			...
			const AudioLanguage = getAnyNs(AudioAttributes, tva.e_AudioLanguage);
			if (AudioLanguage) {
				checkAttributes(AudioLanguage, [tva.a_purpose], [], tvaEA.AudioLanguage, errs, "AV013");
				let validPurpose = false;
				const audioLang = AudioLanguage.content;
				const AudioLanguage_purpose = attrAnyNsValueOrNull(AudioLanguage, tva.a_purpose);
				if (AudioLanguage_purpose) {
					if (!(validPurpose = isValidAudioLanguagePurpose(AudioLanguage_purpose)))
						errs.addError({
							code: "AV014",
				...
				const validLanguage = checkLanguage(audioLang, AudioLanguage, errs, "AV015");
				if (validLanguage && validPurpose) {
					...
					const combo = `${Coding_val ? Coding_val : "dflt"}!--!${MixType_href ? MixType_href : "dflt"}!--!${audioLang}!--!${attrAnyNs(AudioLanguage, tva.a_purpose).value}`;
					if (isIn(foundAttributes, combo))
						errs.addError({
							code: "AV016",
							message: `audio ${tva.a_purpose.attribute()} ${attrAnyNs(AudioLanguage, tva.a_purpose).value.quote()} already specified for language ${audioLang.quote()}`,
							fragment: AudioLanguage,
						});
		...
		// <VideoAttributes>
		forEachNamedChildElement(AVAttributes, tva.e_VideoAttributes, (VideoAttributes) => {
			checkTopElementsAndCardinality(
				VideoAttributes,
				[
		...
		const CaptioningAttributes = getAnyNs(AVAttributes, tva.e_CaptioningAttributes);
		if (CaptioningAttributes) {
			...
			const Coding = getAnyNs(CaptioningAttributes, tva.e_Coding);
			if (Coding) {
				checkAttributes(Coding, [tva.a_href], [], tvaEA.Coding, errs, "AV041");
				const codingHref = attrAnyNsValueOrNull(Coding, tva.a_href);
		...
		const AccessibilityAttributes = getAnyNs(AVAttributes, tva.e_AccessibilityAttributes);
	...
	private ValidateRestartRelatedMaterial(RelatedMaterial : XmlElement, errs : ErrorList) : boolean {
		if (!parameterCheck("ValidateRestartRelatedMaterial", RelatedMaterial, tva.e_RelatedMaterial, errs, "RR000")) return false;

		const isRestartLink = (str : string) => str == dvbi.RESTART_LINK;
	...
	private ValidateInstanceDescription(VerifyType : string, InstanceDescription : XmlElement, isCurrentProgram : boolean, errs : ErrorList) {
		...
		function checkGenre(genre : XmlElement, errs : ErrorList, errcode : string) {
			...
			return attrAnyNsValueOr(genre, tva.a_href, null);
		}

		const isMediaAvailability = (str : string) => [dvbi.MEDIA_AVAILABLE, dvbi.MEDIA_UNAVAILABLE].includes(str);
		const isEPGAvailability = (str : string) => [dvbi.FORWARD_EPG_AVAILABLE, dvbi.FORWARD_EPG_UNAVAILABLE].includes(str);
		const isAvailability = (str : string) => isMediaAvailability(str) || isEPGAvailability(str);

		switch (VerifyType) {
			case tva.e_OnDemandProgram:
				// eslint-disable-next-line no-case-declarations
				const allowedODChildren =
					CG_SchemaVersion(documentNamespace(InstanceDescription)) < cgVersions.r2
						? [
		...
		// @serviceInstanceId
		const InstanceDescription_serviceInstanceID = attrAnyNsValueOrNull(InstanceDescription, tva.a_serviceInstanceID);
		if (InstanceDescription_serviceInstanceID && InstanceDescription_serviceInstanceID.length == 0)
			errs.addError({
				code: "ID009",
		...
		switch (VerifyType) {
			case tva.e_OnDemandProgram:
				...
				const Genre1 = getAnyNs(InstanceDescription, tva.e_Genre, 1),
					Genre2 = getAnyNs(InstanceDescription, tva.e_Genre, 2);
				const g1href = checkGenre(Genre1, errs, "ID011");
				/* eslint-enable */
				if (g1href && !isAvailability(g1href))
					errs.addError({
						code: "ID012",
						message: `first ${`${InstanceDescription.name}.+${tva.e_Genre}`.elementize()} must contain a media or fepg availability indicator`,
						fragment: Genre1,
					});
				...
				if (g2href && !isAvailability(g2href))
					errs.addError({
						code: "ID014",
						message: `second ${`${InstanceDescription.name}.+${tva.e_Genre}`.elementize()} must contain a media or fepg availability indicator`,
						fragment: Genre2,
					});
				...
				if (Genre1 && Genre2) {
					if ((isMediaAvailability(g1href) && isMediaAvailability(g2href)) || (isEPGAvailability(g1href) && isEPGAvailability(g2href))) {
						errs.addError({
							code: "ID015-1",
							message: `${`${InstanceDescription.name}.+${tva.e_Genre}`.elementize()} elements must indicate different availabilities`,
			...
			case tva.e_ScheduleEvent:
				// eslint-disable-next-line no-case-declarations
				const Genre = getAnyNs(InstanceDescription, tva.e_Genre);
				if (Genre) {
					checkAttributes(Genre, [tva.a_href], [tva.a_type], tvaEA.Genre, errs, "ID016");
					const Genre_href = attrAnyNsValueOrNull(Genre, tva.a_href);
					const Genre_type = attrAnyNsValueOrNull(Genre, tva.a_type);
					if (Genre_href) {
						if (isRestartAvailability(Genre_href)) {
							...
							errs.addError({
								code: "ID017",
								message: `${`${InstanceDescription.name}.${tva.e_Genre}`.elementize()} must contain a restart link indicator`,
								line: InstanceDescription.line,
							});
					}
				}
		...
		// <CaptionLanguage>
		const CaptionLanguage = getAnyNs(InstanceDescription, tva.e_CaptionLanguage);
		if (CaptionLanguage) {
		...
		// <SignLanguage>
		const SignLanguage = getAnyNs(InstanceDescription, tva.e_SignLanguage);
		if (SignLanguage) {
		...
		// <AVAttributes>
		const AVAttributes = getAnyNs(InstanceDescription, tva.e_AVAttributes);
		if (AVAttributes) this.ValidateAVAttributes(AVAttributes, this.allowedAudioSchemes, errs);

		// <OtherIdentifier>
		forEachNamedChildElement(InstanceDescription, tva.e_OtherIdentifier, (OtherIdentifier) => {
			checkAttributes(OtherIdentifier, [tva.a_type], [], tvaEA.OtherIdentifier, errs, "VID052");
			const OtherIdentifier_type = attrAnyNsValueOrNull(OtherIdentifier, tva.a_type);
			if (OtherIdentifier_type) {
				if (
					...
					if (!attrAnyNs(OtherIdentifier, mpeg7.a_organization))
		...
		// <RelatedMaterial>
		const RelatedMaterial = getAnyNs(InstanceDescription, tva.e_RelatedMaterial);
		if (RelatedMaterial) {
	...
	private CheckPlayerApplication(node : XmlElement, allowedContentTypes: Array<string>, errs: ErrorList, errcode: string) {
		...
		if (!attrAnyNs(node, tva.a_contentType)) {
			errs.addError({
				code: `${errcode}-1`,
		...
		const node_contentType = attrAnyNsValueOrNull(node, tva.a_contentType);
		if (allowedTypes.includes( node_contentType)) {
	...
	private ValidateOnDemandProgram(OnDemandProgram : XmlElement, programCRIDs: Array<string>, plCRIDs: Array<string>, requestType: string, errs: ErrorList) {
		...
		// <Program>
		const Program = getAnyNs(OnDemandProgram, tva.e_Program);
		if (Program) {
			checkAttributes(Program, [tva.a_crid], [], tvaEA.Program, errs, "OD012");
			const programCRID = attrAnyNsValueOrNull(Program, tva.a_crid);
			if (programCRID) {
				if (!isCRIDURI(programCRID))
					errs.addError({
						code: "OD010",
		...
		// <ProgramURL>
		const ProgramURL = getAnyNs(OnDemandProgram, tva.e_ProgramURL);
		if (ProgramURL) this.CheckPlayerApplication(ProgramURL, [dvbi.XML_AIT_CONTENT_TYPE], errs, "OD020");

		// <AuxiliaryURL>
		const AuxiliaryURL = getAnyNs(OnDemandProgram, tva.e_AuxiliaryURL);
		...
		if (validRequest && [CG_REQUEST_BS_CONTENTS, CG_REQUEST_SCHEDULE_NOWNEXT, CG_REQUEST_SCHEDULE_TIME, CG_REQUEST_SCHEDULE_WINDOW, CG_REQUEST_PROGRAM].includes(requestType)) {
			const InstanceDescription = getAnyNs(OnDemandProgram, tva.e_InstanceDescription);
		...
		const soa = getAnyNs(OnDemandProgram, tva.e_StartOfAvailability),
			eoa = getAnyNs(OnDemandProgram, tva.e_EndOfAvailability);

		if (soa && eoa) {
			try {
				const fr = Temporal.Instant.from(soa.content),
					to = Temporal.Instant.from(eoa.content);
		...
		if ([CG_REQUEST_SCHEDULE_NOWNEXT, CG_REQUEST_SCHEDULE_TIME, CG_REQUEST_SCHEDULE_WINDOW, CG_REQUEST_PROGRAM].includes(requestType)) {
			const DeliveryMode = getAnyNs(OnDemandProgram, tva.e_DeliveryMode);
		...
		// <Free>
		const Free = getAnyNs(OnDemandProgram, tva.e_Free);
	 ...
	 * @param {ErrorList}  errs                errors found in validaton
	 */
	private ValidateEvent(Event : XmlElement, programCRIDs: Array<string>, plCRIDs: Array<string>, currentProgramCRID: string, Schedule: XmlElement | null, errs: ErrorList) {
		...
		const startSchedule = Schedule ? attrAnyNs(Schedule, tva.a_start) : null;
		const endSchedule = Schedule ? attrAnyNs(Schedule, tva.a_end) : null;
		let start_schedule_period = null, end_schedule_period = null;
		try {
			if (startSchedule) 
		...
		// <Program>
		const Program = getAnyNs(Event, tva.e_Program);
		if (Program) {
		...
		// <ProgramURL>
		const ProgramURL = getAnyNs(Event, tva.e_ProgramURL);
		if (ProgramURL)
		...
		// <InstanceDescription>
		const serviceIDs = [];
		forEachNamedChildElement(Event, tva.e_InstanceDescription, (thisInstanceDescription) => {
			this.ValidateInstanceDescription(Event.name, thisInstanceDescription, isCurrentProgram, errs);
			const instanceServiceID = attrAnyNsValueOr(thisInstanceDescription, tva.a_serviceInstanceID, "dflt");
		...
		const pstElem = getAnyNs(Event, tva.e_PublishedStartTime);
		if (pstElem) {
			if (isUTCDateTime(pstElem.content)) {
				let PublishedStartTime = null;
				try {
				...
				if (start_schedule_period && (Temporal.Instant.compare(PublishedStartTime, start_schedule_period) == -1))
					errs.addError({
						code: `${prefix}041`,
				...
				if (end_schedule_period && (Temporal.Instant.compare(PublishedStartTime, end_schedule_period) == 1))
					errs.addError({
						code: `${prefix}042`,
				...
				const pdElem = getAnyNs(Event, tva.e_PublishedDuration);
				if (endSchedule && pdElem) {
					let parsedPublishedDuration = null;
					try {
					...
					if (parsedPublishedDuration && (Temporal.Instant.compare(PublishedStartTime.add(parsedPublishedDuration), end_schedule_period) == 1))	
						errs.addError({
							code: `${prefix}043`,
		...
		// <ActualStartTime>
		const astElem = getAnyNs(Event, tva.e_ActualStartTime);
		...
		// <FirstShowing>
		const FirstShowing = getAnyNs(Event, tva.e_FirstShowing);
		if (FirstShowing) BooleanValue(FirstShowing, tva.a_value, errs, `${prefix}061`);

		// <Free>
		const Free = getAnyNs(Event, tva.e_Free);
	 ...
	 * @param {ErrorList}  errs                errors found in validaton
	 */
	private ValidateScheduleEvents(Schedule : XmlElement, programCRIDs : string[], plCRIDs : string[], currentProgramCRID : string, errs : ErrorList) {
		if (!parameterCheck("ValidateScheduleEvents", Schedule, tva.e_Schedule, errs, "SE000")) return;

		forEachNamedChildElement(Schedule, tva.e_ScheduleEvent, (ScheduleEvent) =>
	 ...
	 */
	private ValidateSchedule(Schedule : XmlElement, programCRIDS : Array<string>, plCRIDs : Array<string>, currentProgramCRID : string, requestType : string, errs : ErrorList) : string{
		...
		const startSchedule = attrAnyNs(Schedule, (tva.a_start)),
			endSchedule = attrAnyNs(Schedule, tva.a_end);
		let fr=null, to = null;
		try {
			if (startSchedule) fr = Temporal.Instant.from(startSchedule.value);
	 ...
	 * @param {integer}    o.childCount        the number of child elements to be present (to match GroupInformation@numOfItems)
	*/
	private CheckProgramLocation(ProgramDescription : XmlElement, programCRIDs : Array<string>, currentProgramCRID : string, requestType : string, errs : ErrorList, o : { childCount: number } = null) {
		if (!parameterCheck("CheckProgramLocation", ProgramDescription, tva.e_ProgramDescription, errs, "PL000")) return;

		const ProgramLocationTable = getAnyNs(ProgramDescription, tva.e_ProgramLocationTable);
		...
		const allowedElements = [{ name: tva.e_OnDemandProgram, minOccurs: 0, maxOccurs: Infinity }];
		switch (requestType) {
			case CG_REQUEST_SCHEDULE_NOWNEXT:
			case CG_REQUEST_SCHEDULE_TIME:
			case CG_REQUEST_SCHEDULE_WINDOW:
			...
			case CG_REQUEST_PROGRAM:
				allowedElements.push({ 
					name: (CG_SchemaVersion(documentNamespace(ProgramDescription)) >= cgVersions.r3) ? tva.e_BroadcastEvent : tva.e_Schedule, 
		...
		if (requestType == CG_REQUEST_PROGRAM) {
			if (hasChild(ProgramLocationTable, tva.e_Schedule) && hasChild(ProgramLocationTable, tva.e_BroadcastEvent))
				errs.addError({
					code: "PL015",
					message: `The ${tva.e_ProgramLocationTable.elementize()} may not contain both ${tva.e_Schedule.elementize()} and ${tva.e_BroadcastEvent.elementize()} elements`,
					line: ProgramLocationTable.line,
					key: "mixed schedule and broadcast events",
				});
		}

		GetNodeLanguage(ProgramLocationTable, false, errs, "PL018");
		...
		forEachChildElement(ProgramLocationTable, (child) => {
			switch (child.name) {
				case tva.e_OnDemandProgram:
	 ...
	 */
	doValidateContentGuide(CGtext : string, requestType : string, errs : ErrorList, options : { log_prefix? : string, report_schema_version? : boolean } = {}) {
		this.numRequests++;
		...
		const CG = SchemaLoad(CGtext, errs, "CG001");
		if (!CG) return;
		...
		const TVAMain : XmlElement = CG.root;
		GetNodeLanguage(TVAMain, true, errs, "CG005");
		const ProgramDescription = getAnyNs(TVAMain, tva.e_ProgramDescription);
		...
		switch (requestType) {
			...
			case CG_REQUEST_SCHEDULE_NOWNEXT:
				...
				if (hasChild(ProgramDescription, tva.e_GroupInformationTable)) this.CheckGroupInformationNowNext(ProgramDescription, groupIds, requestType, errs);
				// eslint-disable-next-line no-case-declarations
