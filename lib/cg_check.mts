...
const SchemaVersions : Array<SchemaReleaseVersion> = [
	{
 ...
 */
let SchemaVersion = (namespace : string) : SchemaReleases=> {
 ...
 */
let valUnsignedInt = (str : string) : number => {
	const intRegex = /[\d]+/;
 ...
 *
 * @param {XmlElement | null} elem  the XML element to be checked
 * @param {string} attrName         the name of the attribute carrying the boolean value
 * @param {Array<string>} allowed   the set or permitted values
 ...
 */
function AllowedValue(elem : XmlElement | null, attrName : string, allowed : Array<string>, errs : ErrorList, errCode : string, isRequired : boolean = true) {
	if (!elem) {
		errs.addError({ type: APPLICATION, code: `${errCode}-0`, message: "AllowedValue() called with elem==null" });
		return;
	}
	const elem_attr = elem.attrAnyNsValueOr(attrName, null);
	const elem_loc = `${elem.parent?.name ? `${elem.parent.name}.` : ""}${elem.name}`;
	if (elem_attr) {
		if (!isIn(allowed, elem_attr))
			errs.addError({
				code: `${errCode}-1`,
				message: `${attrName.attribute(elem_loc)} must be ${allowed.join(" or ")}`,
				fragment: elem,
			});
		}
	else if (isRequired)
		errs.addError({
			code: `${errCode}-2`,
			message: `${attrName.attribute()} must be specified for ${elem_loc}`,
			fragment: elem,
		});
}
 ...
 * @param {boolean} isRequired  true if the specified attribute is required to be specified for the element
 */
let BooleanValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean= true) => 
 ...
 * @param {boolean} isRequired  true if the specified attribute is required to be specified for the element
 */
let TrueValue =  (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean= true) => 
 ...
 * @param {boolean} isRequired  true if the specified attribute is required to be specified for the element
 */
let FalseValue = (elem : XmlElement, attrName : string, errs : ErrorList, errCode : string, isRequired : boolean= true) => 
...
export default class ContentGuideCheck {
	...
	private allowedCreditItemRoles : Role;
	...
	private doSchemaVerification(TVAdoc : XmlDocument, props : DocumentProperties, errs : ErrorList, errCode : string, report_schema_version : boolean = true) {
		let _rc = true;
		...
		if (x && x.schema) {
			...
			if (report_schema_version) SchemaVersionCheck(props, TVAdoc, x.status, errs, `${errCode}a`);
		} else _rc = false;
	 ...
	 *
	 * @param {XmlElement | null} node  the node to check
	 ...
	 */
	private hasElement(node : XmlElement | null, elementName : string) : boolean {
		if (!node) return false;
		return node.childNodes().find((c) => c instanceof XmlElement && c.name == elementName) != undefined;
	}

	/**
	 * check that the serviceIdRef attribute is a TAG URI and report warnings
	 *
	 * @param {XmlElement | null} elem  the node containing the element being checked
	 * @param {ErrorList} errs          errors found in validaton
	 * @param {string} errCode          error code prefix to be used in reports
	 * @returns {string} the serviceIdRef, whether it is valid of not
	 */
	private checkTAGUri(elem : XmlElement | null, errs : ErrorList, errCode : string) {
		if (elem) {
			const elem_serviceIDRef = elem.attrAnyNsValueOr(tva.a_serviceIDRef, null);
			if (elem_serviceIDRef) {
				if (!isTAGURI(elem_serviceIDRef)) 
					errs.addError({
						type: WARNING,
						code: errCode,
	 ...
	 * @param {XmlElement | null} BasicDescription  the element whose children should be checked
	 * @param {Array<string>} requiredLengths	      @length attributes that are required to be present
	 * @param {Array<string>} optionalLengths	      @length attributes that can optionally be present
	 * @param {ErrorList} errs                      errors found in validaton
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	private ValidateSynopsis(props : DocumentProperties, BasicDescription : XmlElement | null, requiredLengths : Array<string>, optionalLengths : Array<string>, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "SY000",
		...
		let synopsisLengthError = (label : string, length : number, actual : number) => 
		...
		while ((Synopsis = BasicDescription.get(xPath(props.prefix, tva.e_Synopsis, ++s), props.schema)) != null) {
			...
			const synopsisLength = Synopsis.attrAnyNsValueOr(tva.a_length, null);
	 ...
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	 private ValidateKeyword(props : DocumentProperties, BasicDescription : XmlElement | null, /* eslint-disable no-unused-vars*/minKeywords : number,/* eslint-enable*/ maxKeywords : number, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "KW000",
	 ...
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	 private ValidateGenre(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
	 ...
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	private ValidateParentalGuidance(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "PG000",
		...
		type CountryInfo = {
			country : string,
			pgCount : number,
			MinimumAge : XmlElement | null,
			ParentalRating : XmlElement | null,
		};
	 ...
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	private ValidateCreditsList(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "CL000",
		...
		function ValidateName(elem : XmlElement, errs : ErrorList, errCode : string) {
			function checkNamePart(elem : XmlElement, errs : ErrorList, errCode : string) {
				const parent_element = elem.parent?.name ? ` in ${elem.parent.name.elementize()}` : "";
				if (unEntity(elem.content).length > dvbi.MAX_NAME_PART_LENGTH)
					errs.addError({
						code: errCode,
						fragment: elem,
						message: `${elem.name.elementize()}${parent_element} is longer than ${dvbi.MAX_NAME_PART_LENGTH} characters`,
						key: keys.k_LengthError,
					});
			}
		...
		if (CreditsList) {
			...
			while ((CreditsItem = CreditsList.get(xPath(props.prefix, tva.e_CreditsItem, ++ci), props.schema)) != null) {
				...
				if (CreditsItemRole &&!this.allowedCreditItemRoles.isIn(CreditsItemRole))
					errs.addError({
						code: `${errCode}-2`,
	 ...
	 * @param {ErrorList} errs                      errors found in validaton
	 */
	private ValidateRelatedMaterial_PromotionalStillImage(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "RMPSI000",
	 ...
	 * @param {ErrorList} errs                      errors found in validaton
	 */
	 private ValidateRelatedMaterial_Pagination(props : DocumentProperties, BasicDescription : XmlElement | null, Location : string, errs : ErrorList) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "VP000",
		...
		function checkLinkCounts(elements : Array<XmlElement>, label : string, errs : ErrorList, errCode : string) {
	 ...
	 * @param {ErrorList} errs                      errors found in validaton
	 */
	private ValidateRelatedMaterial_MoreEpisodes(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "RMME000",
		...
		switch (BasicDescription.parent?.name) {
			...
			case tva.e_GroupInformation:
				this.ValidateRelatedMaterial_Pagination(props, BasicDescription, "More Episodes", errs);
				break;
		}
	}
	 ...
	 */
	private ValidateTemplateAIT(RelatedMaterial : XmlElement | null, Location : string, errs : ErrorList) {
		if (!RelatedMaterial) {
			errs.addError({
				type: APPLICATION,
				code: "TA000",
			...
			RelatedMaterial.childNodes().forEachSubElement((child) => {
				switch (child.name) {
					case tva.e_HowRelated:
						HowRelated = child as XmlElement;
						break;
		...
		const HowRelated_href = (HowRelated as XmlElement).attrAnyNsValueOr(tva.a_href, null);
		if (HowRelated_href) {
			if (HowRelated_href != dvbi.TEMPLATE_AIT_URI)
				...
				if (MediaLocator.length != 0)
					MediaLocator.forEach((locator) => {
						...
						(locator as XmlElement).childNodes()?.forEachNamedSubElement(tva.e_AuxiliaryURI, (AuxiliaryURI) => {
							hasAuxiliaryURI = true;
							checkAttributes(AuxiliaryURI, [tva.a_contentType], [], tvaEA.AuxiliaryURI, errs, "TA010");
							let contentType = AuxiliaryURI.attrAnyNsValueOr(tva.a_contentType, null);
	 ...
	 * @param {ErrorList} errs                      errors found in validaton
	 */
	private ValidateRelatedMaterial_BoxSetList(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "MB000",
		...
		let countImage : Array<XmlElement> = [],
			countTemplateAIT : Array<XmlElement> = [],
			hasPagination = false;
		let rm = 0,
			RelatedMaterial;
		...
		if (hasPagination) this.ValidateRelatedMaterial_Pagination(props, BasicDescription, "Box Set List", errs);
	}
	 ...
	 *
	 * @param {DocumentProperties} props          Metadata of the XML document
	 * @param {XmlElement | null} containingNode  the element whose children should be checked
	 * @param {boolean} allowSecondary            indicates if Title with @type="secondary" is permitted
	 * @param {boolean} TypeIsRequired            true if the @type is a required attribute in this use of <Title>
	 * @param {ErrorList} errs                    errors found in validaton
	 * @param {string} errCode                    error code prefix to be used in reports
	 */
	private ValidateTitle(props : DocumentProperties, containingNode : XmlElement | null, allowSecondary : boolean, TypeIsRequired : boolean, errs : ErrorList, errCode : string) {
		if (!containingNode) {
		...
		let mainTitles : Array<TitleType> = [],
			secondaryTitles : Array<TitleType> = [];
		let t = 0,
			Title,
			requiredAttributes : Array<string> = [],
			optionalAttributes : Array<string> = [tva.a_lang];
		...
		secondaryTitles.forEach((title) => {
			if (!mainTitles.find((e) => e.lang == title.lang)) {
				let tLoc = title.lang != DEFAULT_LANGUAGE ? ` for @xml:${tva.a_lang}=${title.lang.quote()}` : "";
				errs.addError({
					code: `${errCode}-16`,
	 ...
	 * @param {string} errCode                      error code prefix to be used in reports
	 */
	private ValidateReleaseInformation(props : DocumentProperties, BasicDescription : XmlElement | null, errs : ErrorList, errCode : string) {
		if (!BasicDescription) {
			errs.addError({
				type: APPLICATION,
				code: "RI000",
			...
			case tva.e_ProgramInformation:
				switch (requestType) {
					...
					case CGrequests.SCHEDULE_TIME:
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, true, true, errs, "BD011");
						this.ValidateSynopsis(props, BasicDescription as XmlElement, [tva.SYNOPSIS_MEDIUM_LABEL], [tva.SYNOPSIS_SHORT_LABEL], errs, "BD012");
						this.ValidateGenre(props, BasicDescription as XmlElement, errs, "BD013");
					...
					case CGrequests.PROGRAM: // 6.10.5.3
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, true, true, errs, "BD021");
						this.ValidateSynopsis(props, BasicDescription as XmlElement, [tva.SYNOPSIS_MEDIUM_LABEL], [tva.SYNOPSIS_SHORT_LABEL, tva.SYNOPSIS_LONG_LABEL], errs, "BD022");
						this.ValidateKeyword(props, BasicDescription as XmlElement, 0, 20, errs, "BD023");
						this.ValidateGenre(props, BasicDescription as XmlElement, errs, "BD024");
						this.ValidateParentalGuidance(props, BasicDescription as XmlElement, errs, "BD025");
						this.ValidateCreditsList(props, BasicDescription as XmlElement, errs, "BD026");
						this.ValidateRelatedMaterial_PromotionalStillImage(props, BasicDescription as XmlElement, errs);
					...
					case CGrequests.BS_CONTENTS: // 6.10.5.4
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, true, true, errs, "BD031");
						this.ValidateSynopsis(props, BasicDescription as XmlElement, [], [tva.SYNOPSIS_MEDIUM_LABEL], errs, "BD032");
						this.ValidateParentalGuidance(props, BasicDescription as XmlElement, errs, "BD033");
						this.ValidateRelatedMaterial_PromotionalStillImage(props, BasicDescription as XmlElement, errs);
					...
					case CGrequests.MORE_EPISODES:
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, true, true, errs, "BD041");
			...
			case tva.e_GroupInformation:
				switch (requestType) {
					...
					case CGrequests.BS_LISTS: // 6.10.5.5
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, false, false, errs, "BD063");
						if (!isParentGroup) {
							this.ValidateSynopsis(props, BasicDescription as XmlElement, [tva.SYNOPSIS_MEDIUM_LABEL], [], errs, "BD064");
							this.ValidateKeyword(props, BasicDescription as XmlElement, 0, 20, errs, "BD065");
					...
					case CGrequests.BS_CATEGORIES:
						...
						this.ValidateTitle(props, BasicDescription as XmlElement, false, false, errs, "BD082");
						if (!isParentGroup) this.ValidateSynopsis(props, BasicDescription as XmlElement, [tva.SYNOPSIS_SHORT_LABEL], [], errs, "BD083");
						this.ValidateGenre(props, BasicDescription as XmlElement, errs, "BD084");
						this.ValidateRelatedMaterial_PromotionalStillImage(props, BasicDescription as XmlElement, errs);
						this.ValidateRelatedMaterial_Pagination(props, BasicDescription as XmlElement, "Box Set Categories", errs);
						break;
					default:
						errs.addError({
							type: APPLICATION,
	...
	private CheckProgramInformation(props : DocumentProperties, ProgramDescription : XmlElement | null, programCRIDs : Array<string>, groupCRIDs : Array<string> | null, requestType : CGrequests, errs : ErrorList, o : ProgramInformationStats | null= null) : string | null {
		...
		checkAttributes(ProgramInformationTable as XmlElement, [], [tva.a_lang], tvaEA.ProgramInformationTable, errs, "PI102");
		GetNodeLanguage(ProgramInformationTable as XmlElement, false, errs, "PI103");
	...
	private ValidateGroupInformationBoxSets(props : DocumentProperties, GroupInformation : XmlElement | null, requestType : CGrequests, categoryGroup : XmlElement | null, indexes : Array<string> | null, groupsFound : Array<string> | null, errs : ErrorList) {
		...
		let groupId = GroupInformation.attrAnyNsValueOr(tva.a_groupId, null);
		...
		if (!isParentGroup) {
			...
			if (MemberOf) {
				checkAttributes(MemberOf as XmlElement , [tva.a_type, tva.a_index, tva.a_crid], [], tvaEA.MemberOf, errs, "GIB041");
				const member = (MemberOf as XmlElement).attrAnyNsValueOr(tva.a_type, null);
				if (member && member != tva.t_MemberOfType)
					errs.addError({
						code: "GIB042",
						message: `${GroupInformation.name}.${tva.e_MemberOf}@xsi:${tva.a_type} is invalid (${member.quote()})`,
						fragment: MemberOf as XmlElement,
					});
				const MemberOf_index = (MemberOf as XmlElement).attrAnyNsValueOr(tva.a_index, null);
				if (MemberOf_index) {
					const MemberOf_indexN : number = Number(MemberOf_index);
					if (MemberOf_indexN >= 1) {
						if (indexes) {
							if (DuplicatedValue(indexes, MemberOf_index))
								errs.addError({
									code: "GI043",
									message: `duplicated ${tva.a_index.attribute(`${GroupInformation.name}.${tva.e_MemberOf}`)} value (${MemberOf_indexN})`,
									fragment: MemberOf as XmlElement,
								});
						}
					} else
						errs.addError({
							code: "GIB44",
							message: `${tva.a_index.attribute(`${GroupInformation.name}.${tva.e_MemberOf}`)} must be an integer >= 1 (parsed ${MemberOf_indexN})`,
							fragment: MemberOf as XmlElement,
						});
				}
				const crid = (MemberOf as XmlElement).attrAnyNsValueOr(tva.a_crid, null);
		...
		this.ValidateBasicDescription(props, GroupInformation, requestType, categoryGroup, errs);
	}
	...
	private ValidateGroupInformationMoreEpisodes(props : DocumentProperties, GroupInformation : XmlElement | null, requestType : CGrequests, categoryGroup : XmlElement | null, groupsFound : Array<string> | null, errs : ErrorList) {
		...
		else if (groupsFound) groupsFound.push(groupId);
		...
		if (GroupType) {
			checkAttributes(GroupType as XmlElement, [tva.a_type, tva.a_value], [], tvaEA.GroupType, errs, "GIM011");

			const GroupType_type = (GroupType as XmlElement).attrAnyNsValueOr(tva.a_type, null);
			if (GroupType_type && GroupType_type != tva.t_ProgramGroupTypeType)
				errs.addError({
					...
					fragment: GroupType as XmlElement,
				});
			const GroupTYpe_value = (GroupType as XmlElement).attrAnyNsValueOr(tva.a_value, null);
	...
	private ValidateGroupInformation(props : DocumentProperties, GroupInformation : XmlElement | null, requestType : CGrequests, categoryGroup : XmlElement | null, indexes : Array<string> | null, groupsFound : Array<string> | null, errs : ErrorList) {
		...
		const GroupType = GroupInformation.get(xPath(props.prefix, tva.e_GroupType), props.schema);
		if (GroupType) {
			const _type = (GroupType as XmlElement).attrAnyNsValueOr(tva.a_type, null);
			if (!(_type && _type == tva.t_ProgramGroupTypeType))
				errs.addError({
					code: "GI011",
		...
		if (categoryGroup) {
			...
			if (notBSContents(requestType) && numOfItems != giCount)
				errs.addError({
					...
					line: (categoryGroup as XmlElement).line,
					key: "mismatch count",
				});
	...
	private ValidateGroupInformationNowNext(props : DocumentProperties, GroupInformation : XmlElement | null, requestType : CGrequests, numEarlier : number, numNow : number, numLater : number, groupCRIDsFound : Array<string>, errs : ErrorList) {
		...
		this.ValidateGroupInformation(props, GroupInformation, requestType, null, null, null, errs);
		...
		if (GroupInformation_groupId) {
			if (
				...
				let numOfItems = Number(GroupInformation.attrAnyNsValueOr(tva.a_numOfItems, "-1"));
	...
	private CheckGroupInformationNowNext(props : DocumentProperties, ProgramDescription : XmlElement | null, groupIds :Array<string>, requestType : CGrequests, errs : ErrorList) {
		...
		GetNodeLanguage(GroupInformationTable as XmlElement, false, errs, "NM002");
	 ...
	 *
	 * @param {DocumentProperties} props        Metadata of the XML document
	 * @param {XmlElement | null} AVAttributes  the <AVAttributes> node to be checked
	 * @param {ErrorList} errs                  errors found in validaton
	 */
	private *ValidateAVAttributes(props : DocumentProperties, AVAttributes : XmlElement | null, errs : ErrorList) {
		if (!AVAttributes) {
			errs.addError({
				type: APPLICATION,
				code: "AV000",
		...
		checkTopElementsAndCardinality(
			AVAttributes,
			SchemaVersion(props.namespace) >= SchemaReleases.SCHEMA_r2
				? [
		...
		while ((AudioAttributes = AVAttributes.get(xPath(props.prefix, tva.e_AudioAttributes, ++aa), props.schema)) != null) {
			...
			const AudioLanguage = AudioAttributes.get(xPath(props.prefix, tva.e_AudioLanguage), props.schema) as XmlElement;
			if (AudioLanguage) {
				...
				if (validLanguage && validPurpose) {
					...
					let found = audioCounts.find((a) => a.language == audioLang);
					if (found) 
						found.elements.push(AudioLanguage)
					else audioCounts.push({language: audioLang, elements: [AudioLanguage]});

					let combo = `${audioLang}!--!${AudioLanguage_purpose}`;
		...
		audioCounts.forEach((lang) => {
			if (lang.elements.length > 2)
				errs.addError({
					code: "AV020",
					message: `more than 2 ${tva.e_AudioAttributes.elementize()} elements for language ${lang.language.quote()}`,
					multiElementError: lang.elements,
				});
		});
		...
		if (AccessibilityAttributes) {
			CheckAccessibilityAttributes(
				AccessibilityAttributes as XmlElement,
				{
	 ...
	 *
	 * @param {DocumentProperties} props           Metadata of the XML document
	 * @param {XmlElement | null} RelatedMaterial  the <RelatedMaterial> node to be checked
	 * @param {ErrorList} errs                     errors found in validaton
	 ...
	 */
	private ValidateRestartRelatedMaterial(props : DocumentProperties, RelatedMaterial : XmlElement | null, errs : ErrorList) {
		if (!RelatedMaterial) {
			errs.addError({
				type: APPLICATION,
				code: "RR000",
		...
		let HowRelated = RelatedMaterial.get(xPath(props.prefix, tva.e_HowRelated), props.schema) as XmlElement;
		if (HowRelated) {
		...
		let MediaLocator = RelatedMaterial.get(xPath(props.prefix, tva.e_MediaLocator), props.schema) as XmlElement;
		if (MediaLocator)
	 ...
	 * @param {XmlElement | null} InstanceDescription  the <InstanceDescription> node to be checked
	 * @param {boolean} isCurrentProgram               indicates if this <InstanceDescription> element is for the currently airing program
	 * @param {ErrorList} errs                         errors found in validaton
	 */
	private ValidateInstanceDescription(props : DocumentProperties, VerifyType : string, InstanceDescription : XmlElement | null, isCurrentProgram : boolean, errs : ErrorList) {
		...
		function checkGenre(genre : XmlElement | null, errs : ErrorList, errcode : string) {
			...
			const GenreType = genre.attrAnyNsValueOr(tva.a_type, tva.DEFAULT_GENRE_TYPE);
			const Genre_parent = `${genre.parent ? `${genre.parent.name}.` : ""}${genre.name}`;
			if (GenreType != tva.GENRE_TYPE_OTHER)
				errs.addError({
					code: `${errcode}-2`,
					message: `${tva.a_type.attribute(Genre_parent)} must contain ${tva.GENRE_TYPE_OTHER.quote()}`,
					fragment: genre,
				});
		...
		// @serviceInstanceId
		const InstanceDescription_serviceInstanceID = InstanceDescription.attrAnyNsValueOr(tva.a_serviceInstanceID, null)
		if (InstanceDescription_serviceInstanceID && InstanceDescription_serviceInstanceID.length == 0)
			errs.addError({
				code: "ID009",
				message: `${tva.a_serviceInstanceID.attribute()} should not be empty if specified`,
				line: InstanceDescription.line,
				key: "empty ID",
			});

		let restartGenre : XmlElement | undefined = undefined,
			restartRelatedMaterial : XmlElement | undefined = undefined;
		// <Genre>
		switch (VerifyType) {
			case tva.e_OnDemandProgram:
				...
				/* eslint-disable no-case-declarations */
				let Genre1 = InstanceDescription.get(xPath(props.prefix, tva.e_Genre, 1), props.schema) as XmlElement,
					Genre2 = InstanceDescription.get(xPath(props.prefix, tva.e_Genre, 2), props.schema) as XmlElement;
			...
			case tva.e_ScheduleEvent:
				/* eslint-disable no-case-declarations */
				let Genre = InstanceDescription.get(xPath(props.prefix, tva.e_Genre), props.schema) as XmlElement;
				/* eslint-enable */
				if (Genre) {
					checkAttributes(Genre, [tva.a_href], [tva.a_type], tvaEA.Genre, errs, "ID016");
					const Genre_href = Genre.attrAnyNsValueOr(tva.a_href, null);
					if (Genre_href) {
						if (isRestartAvailability(Genre_href)) {
							const Genre_type = Genre.attrAnyNsValueOr(tva.a_type, "");
							restartGenre = Genre;
							if (Genre.attrAnyNs(tva.a_type) && Genre_type != tva.GENRE_TYPE_OTHER)
								errs.addError({
									code: "ID018",
		...
		// <CaptionLanguage>
		let CaptionLanguage = InstanceDescription.get(xPath(props.prefix, tva.e_CaptionLanguage), props.schema) as XmlElement;
		if (CaptionLanguage) {
		...
		// <SignLanguage>
		let SignLanguage = InstanceDescription.get(xPath(props.prefix, tva.e_SignLanguage), props.schema) as XmlElement;
		if (SignLanguage) {
		...
		// <AVAttributes>
		let AVAttributes = InstanceDescription.get(xPath(props.prefix, tva.e_AVAttributes), props.schema) as XmlElement;
		if (AVAttributes) 
			this.ValidateAVAttributes(props, AVAttributes, errs);
		...
		// <RelatedMaterial>
		let RelatedMaterial = InstanceDescription.get(xPath(props.prefix, tva.e_RelatedMaterial), props.schema) as XmlElement;
		if (RelatedMaterial) 
			if (this.ValidateRestartRelatedMaterial(props, RelatedMaterial, errs)) 
				restartRelatedMaterial = RelatedMaterial; //THIS
		...
		if ((restartGenre && !restartRelatedMaterial) || (restartRelatedMaterial && !restartGenre))
			errs.addError({
				...
				multiElementError: [restartGenre as XmlElement, restartRelatedMaterial as XmlElement],
			});
	}
	 ...
	 *
	 * @param {XmlElement | null} node                      the element node containing the an XML AIT reference
	 * @param {Array<string> | string} allowedContentTypes  the contentTypes that can be signalled in the node@contentType attribute
	 * @param {ErrorList} errs                              errors found in validaton
	 * @param {string} errcode                              error code to be used with any errors found
	 */
	private CheckPlayerApplication(node : XmlElement | null, allowedContentTypes : Array<string> | string, errs : ErrorList, errcode : string) {
		if (!node) {
			errs.addError({ type: APPLICATION, code: "PA000", message: "CheckPlayerApplication() called with node==null" });
			return;
		}
		const allowedTypes = Array.isArray(allowedContentTypes) ? allowedContentTypes : [allowedContentTypes];
		const node_contentType = node.attrAnyNsValueOr(tva.a_contentType, null);
		if (!node_contentType) {
			errs.addError({
				code: `${errcode}-1`,
	 ...
	 * @param {ErrorList} errs                    errors found in validaton
	 */
	private ValidateEvent(props : DocumentProperties, Event : XmlElement, programCRIDs : Array<string>, plCRIDs : Array<string>, currentProgramCRID : string | null, Schedule : XmlElement | null, errs : ErrorList) {
		let prefix = "";
		...
		let startSchedule = Schedule ? Schedule.attrAnyNs(tva.a_start) : null,
			start_schedule_period : Date | undefined = undefined,
			endSchedule = Schedule ? Schedule.attrAnyNs(tva.a_end) : null,
			end_schedule_period : Date | undefined = undefined;
		...
		// <Program>
		let Program = Event.get(xPath(props.prefix, tva.e_Program), props.schema) as XmlElement;
		if (Program) {
		...
		// <ProgramURL>
		let ProgramURL = Event.get(xPath(props.prefix, tva.e_ProgramURL), props.schema) as XmlElement;
		if (ProgramURL)
		...
		let pstElem = Event.get(xPath(props.prefix, tva.e_PublishedStartTime), props.schema) as XmlElement;
		if (pstElem) {
			if (isUTCDateTime(pstElem.content)) {
				let PublishedStartTime = new Date(pstElem.content);
				let elems : Array<XmlElement> = [pstElem];
				if (Schedule) elems.push(Schedule);
				...
				let pdElem = Event.get(xPath(props.prefix, tva.e_PublishedDuration), props.schema) as XmlElement;
				if (endSchedule && pdElem) {
					let parsedPublishedDuration = new parseISOduration2(pdElem.content);
					if (end_schedule_period && (parsedPublishedDuration.add(PublishedStartTime) > end_schedule_period)) {
						let elems : Array<XmlElement> = [pdElem];
						if (Schedule) elems.push(Schedule);
						errs.addError({
							code: `${prefix}043`,
		...
		// <ActualStartTime>
		let astElem = Event.get(xPath(props.prefix, tva.e_ActualStartTime), props.schema) as XmlElement;
		...
		// <FirstShowing>
		let FirstShowing = Event.get(xPath(props.prefix, tva.e_FirstShowing), props.schema) as XmlElement;
		if (FirstShowing) BooleanValue(FirstShowing, tva.a_value, errs, `${prefix}061`);

		// <Free>
		let Free = Event.get(xPath(props.prefix, tva.e_Free), props.schema) as XmlElement;
	 ...
	 *
	 * @param {DocumentProperties} props          Metadata of the XML document
	 * @param {XmlElement | null} BroadcastEvent  the node containing the <BroadcastEvent> being checked
	 * @param {Array<string>} programCRIDs        array of program crids defined in <ProgramInformationTable>
	 * @param {Array<string>} plCRIDs             array of program crids defined in <ProgramLocationTable>
	 * @param {string | null} currentProgramCRID  CRID of the currently airing program
	 * @param {CGrequests} requestType            the type of content guide request being checked
	...
	private ValidateBroadcastEvent(props : DocumentProperties, BroadcastEvent : XmlElement | null, programCRIDs : Array<string>, plCRIDs : Array<string>, currentProgramCRID : string | null, requestType : CGrequests, errs : ErrorList) {
		...
		this.ValidateEvent(props, BroadcastEvent, programCRIDs, plCRIDs, currentProgramCRID, null, errs);
	}
	 ...
	 *
	 * @param {DocumentProperties} props          Metadata of the XML document
	 * @param {XmlElement | null} Schedule        the <Schedule> node containing the <ScheduleEvent> element to be checked
	 * @param {Array<string>} programCRIDs        array of program crids defined in <ProgramInformationTable>
	 * @param {Array<string>} plCRIDs             array of program crids defined in <ProgramLocationTable>
	 * @param {string | null} currentProgramCRID  CRID of the currently airing program
	 * @param {ErrorList} errs                    errors found in validaton
	 */
	private ValidateScheduleEvents(props : DocumentProperties, Schedule : XmlElement | null, programCRIDs : Array<string>, plCRIDs : Array<string>, currentProgramCRID : string | null, errs : ErrorList) {
		if (!Schedule) {
			errs.addError({
				type: APPLICATION,
				code: "SE000",
	 ...
	 *
	 * @param {DocumentProperties} props          Metadata of the XML document
	 * @param {XmlElement | null} Schedule        the node containing the <Schedule> being checked
	 * @param {Array<string>} programCRIDs        array of program crids defined in <ProgramInformationTable>
	 * @param {Array<string>} plCRIDs             array of program crids defined in <ProgramLocationTable>
	 * @param {string | null} currentProgramCRID  CRID of the currently airing program
	 * @param {CGrequests} requestType            the type of content guide request being checked
	...
	private CheckProgramLocation(props :DocumentProperties, ProgramDescription : XmlElement | null, programCRIDs : Array<string>, currentProgramCRID : string | null, requestType : CGrequests, errs : ErrorList, o : ProgramInformationStats | null = null) {
		...
		let ProgramLocationTable = ProgramDescription.get(xPath(props.prefix, tva.e_ProgramLocationTable), props.schema) as XmlElement;
	...
	doValidateContentGuide(CGtext : string | null, requestType : CGrequests, errs : ErrorList, options : any) {
		...
		this.doSchemaVerification(CG, props, errs, "CG003", local_options.report_schema_version);

		let TVAMain = CG.root;
		GetNodeLanguage(TVAMain, true, errs, "CG005");
		let ProgramDescription = TVAMain.get(xPath(props.prefix, tva.e_ProgramDescription), props.schema) as XmlElement;
		if (!ProgramDescription) {
		...
		switch (requestType) {
			...
			case CGrequests.SCHEDULE_NOWNEXT:
				...
				if (this.hasElement(ProgramDescription, tva.e_GroupInformationTable)) 
					this.CheckGroupInformationNowNext(props, ProgramDescription, groupIds, requestType, errs);
				/* eslint-disable no-case-declarations */
