...
import { slVersions, dvbi, dvbisld, dvbiEC, dvbiEA, XMLdocumentType, validApplicationTypes, DvbIApplicationTypes } from "./DVB-I_definitions.mts";

import ErrorList, { WARNING, APPLICATION } from "./error_list.mts";
import { isIn, unEntity, DuplicatedValue, parameterCheck } from "./utils.mts";
import { isMIME } from "./MIME_checks.mts";
import { isPostcode, isASCII, isHTTPURL, isHTTPPathURL, isDomainName, isRTSPURL, isTAGURI, hasNonPrintableChars, isUUIDformat } from "./pattern_checks.mts";
import { checkValidLogos } from "./related_material_checks.mts";
import { sl_InvalidHrefValue, InvalidURL, DeprecatedAttribute, DeprecatedElement, keys, InvalidCountryCode } from "./common_errors.mts";
...
import { GetSchema, SL_SchemaVersion, SchemaSpecVersion, isA177specification_URN, isContentFinishedBanner, isOutScheduleHours } from "./sl_data_versions.mts";
...
import { CMCD_custom_stats, LoadKnownCustomKeysRegistry } from "./CMCD_custom_keys.mts";
...
import { getAnyNs, attrAnyNs, attrAnyNsValueOr, attrAnyNsValueOrNull, documentNamespace, hasChild, forEachChildElement, forEachNamedChildElement } from "../libxml2-wasm-extensions.mts";
import type { ValidatorInitializationOptions } from "./validator.mts";
 ...
 */
const validServiceIdentifier = (identifier : string) : boolean => isTAGURI(identifier);
const validServiceListIdentifier = (identifier : string) : boolean => isTAGURI(identifier);
 ...
 */
const uniqueServiceIdentifier = (identifier : string, identifiers : string[]) : boolean => !isIn(identifiers, identifier);
 ...
 * @returns {String}
 */
const localizedSubscriptionPackage = (pkg : XmlElement, lang? : string) : string=> `${pkg.content}/lang=${lang ? lang : mlLanguage(pkg)}`;
 ...
 */
const UnspecifiedTargetRegion = (region : string, loc : string, errCode : string, element : XmlElement) => ({
	code: errCode,
 ...
 * @param {String}     errCode    The error code to be reported
 */
const NoDeliveryParams = (source, serviceId, element, errCode) => ({
	code: errCode,
...
type AnnouncedAudioLanguage_type = { 
	language: string; 
	used: boolean; 
	fragment: XmlElement 
};
...
export default class ServiceListCheck {
	...
	constructor(useURLs : boolean, opts : ValidatorInitializationOptions, async : boolean = true) {
		this.numRequests = 0;
		...
		LoadKnownCustomKeysRegistry(useURLs, async)
	}
	 ...
	 */
	private addRegion(Region : XmlElement, depth : number, knownRegionIDs : Array<StoredRegion_type>, countries : Array<string>, errs : ErrorList) {
		if (!parameterCheck("addRegion", Region, dvbi.e_Region, errs, "AR000")) return;

		const schemaVersion = SL_SchemaVersion(documentNamespace(Region));
		const regionID = attrAnyNsValueOrNull(Region, dvbi.a_regionID);
		const displayRegionID = regionID ? regionID.quote() : '"noID"';
		let countriesSpecified = [];
		const countryCodesSpecified = attrAnyNs(Region, dvbi.a_countryCodes);
		...
		if (schemaVersion >= slVersions.r4) {
			const selectable = attrAnyNs(Region, dvbi.a_selectable) ? attrAnyNs(Region, dvbi.a_selectable).value == "true" : true;
		...
		// <Region><Postcode>
		forEachNamedChildElement(Region, dvbi.e_Postcode, (Postcode) => {
		...
		forEachNamedChildElement(Region, dvbi.e_Region, (RegionChild) => this.addRegion(RegionChild, depth + 1, knownRegionIDs, countriesSpecified, errs));
	}
	...
	private checkSignalledApplication(MediaLocator : XmlElement, Location : string, AppType : string, errs : ErrorList) {
		const isValidApplicationType = (type : string) => validApplicationTypes.includes(type);
		...
		if (!MediaLocator)
			...
			forEachNamedChildElement(MediaLocator, tva.e_MediaUri, (MediaUri) => {
				hasMediaURI = true;
				if (attrAnyNs(MediaUri, tva.a_contentType) && !isValidApplicationType(attrAnyNs(MediaUri, tva.a_contentType).value))
					errs.addError({
						code: "SA003",
						message: `${tva.a_contentType.attribute()} ${attrAnyNs(MediaUri, tva.a_contentType).value.quote()} is not supported application type for ${tva.e_RelatedMaterial.elementize()}${tva.e_MediaLocator.elementize()} in ${Location}`,
						fragment: MediaUri,
				...
				const MediaUri_contentType = attrAnyNsValueOrNull(MediaUri, tva.a_contentType);
	 ...
	 */
	private validServiceApplication(HowRelated : XmlElement, schemaVersion : number) {
		...
		const val = attrAnyNsValueOrNull(HowRelated, dvbi.a_href);
	 ...
	 * @returns {boolean} true if this is a valid application launching method else false
	 */
	private validServiceInstanceApplication(HowRelated : XmlElement, schemaVersion : number) {
		// return true if the HowRelated element has a valid CS value for Service Related Applications (A177 5.2.3)
		// urn:dvb:metadata:cs:LinkedApplicationCS:2019
		if (!HowRelated) return false;
		const val = attrAnyNsValueOr(HowRelated, dvbi.a_href, null);
	 ...
	 */
	private validateRelatedMaterial(RelatedMaterial : XmlElement, Location : string, LocationType : string, errs : ErrorList, errCode : string) : string{
		if (!parameterCheck("validateRelatedMaterial", RelatedMaterial, dvbi.e_RelatedMaterial, errs, `${errCode}-a`)) return "";

		let rc = "";
		const documentNS = documentNamespace(RelatedMaterial);
		...
		forEachChildElement(RelatedMaterial, (child) => {
			switch (child.name) {
				...
				case tva.e_AccessibilityAttributes:
					AccessibilityAttributes.push(child);
					break;
			}
		});
		...
		if (attrAnyNs(HowRelated, dvbi.a_href)) {
			const RMErrorDescription = (_code : string, _elem : string, _table : number) => ({
				code: _code,
			...
			switch (LocationType) {
				case SERVICE_LIST_RM:
					if (validServiceListLogo(HowRelated)) {
						rc = attrAnyNs(HowRelated, dvbi.a_href).value;
						...
						errs.addError(sl_InvalidHrefValue(attrAnyNs(HowRelated, dvbi.a_href).value, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-11`));
				...
				case SERVICE_RM:
					...
					const HowRelated1_href = attrAnyNsValueOrNull(HowRelated, dvbi.a_href);
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNS) == slVersions.r0)
						errs.addError({
							code: `${errCode}-21`,
							message: `${HowRelated1_href.quote()} not permitted for ${documentNS.quote()} in ${Location}`,
					...
					} else if (this.validServiceApplication(HowRelated, SL_SchemaVersion(documentNS))) {
						rc = HowRelated1_href;
						MediaLocator.forEach((locator) => this.checkSignalledApplication(locator, Location, rc, errs));
					} else {
						errs.addError(sl_InvalidHrefValue(HowRelated1_href, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-24`));
				...
				case SERVICE_INSTANCE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated2_href = attrAnyNsValueOrNull(HowRelated, dvbi.a_href);
					if (isContentFinishedBanner(HowRelated) && SL_SchemaVersion(documentNS) == slVersions.r0)
						errs.addError({
							code: `${errCode}-31`,
							message: `${HowRelated2_href.quote()} not permitted for ${documentNS.quote()} in ${Location}`,
							key: "invalid CS value",
							fragment: HowRelated,
						});
					...
					} else if (isOutScheduleHours(HowRelated) && SL_SchemaVersion(documentNS) >= slVersions.r6)
						errs.addError({
							code: `${errCode}-35`,
					...
					else if (this.validServiceInstanceApplication(HowRelated, SL_SchemaVersion(documentNS))) {
						rc = HowRelated2_href;
						MediaLocator.forEach((locator) => this.checkSignalledApplication(locator, Location, rc, errs));
					} else {
						errs.addError(sl_InvalidHrefValue(HowRelated2_href, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-34`));
				...
				case CONTENT_GUIDE_RM:
					// eslint-disable-next-line no-case-declarations
					const HowRelated3_href = attrAnyNsValueOrNull(HowRelated, dvbi.a_href);
					if (validContentGuideSourceLogo(HowRelated)) {
						rc = HowRelated3_href;
						checkValidLogos(RelatedMaterial, Location, errs, `${errCode}-41`);
					} else {
						errs.addError(sl_InvalidHrefValue(HowRelated3_href, HowRelated, tva.e_RelatedMaterial.elementize(), Location, `${errCode}-42`));
		...
		AccessibilityAttributes.forEach((aa) => {
	 ...
	 */
	private hasSignalledApplication(node : XmlElement) : boolean {
		if (node) {
			forEachNamedChildElement(node, tva.e_RelatedMaterial, (elem) => {
				const hr = getAnyNs(elem, tva.e_HowRelated);
				if (hr && this.validServiceApplication(hr, SL_SchemaVersion(documentNamespace(node)))) return true;
			});
		}
		return false;
	}
	 ...
	 */
	private validateAContentGuideSource(source : XmlElement, loc : string, errs : ErrorList, errCode : string) {
		...
		const CheckEndpoint = (elementName : string, suffix : number, MustEndWithSlash : boolean = false) => {
			const ep = getAnyNs(source, elementName);
			if (ep) {
				const epURL = getAnyNs(ep, dvbi.e_URI);
				if (epURL) {
				...
				const ep_contentType = attrAnyNsValueOrNull(ep, dvbi.a_contentType);
		...
		forEachNamedChildElement(source, tva.e_RelatedMaterial, (RelatedMaterial) => 
			this.validateRelatedMaterial(RelatedMaterial, loc, CONTENT_GUIDE_RM, errs, `${errCode}-3`)
		);
		...
		CheckEndpoint(dvbi.e_GroupInfoEndpoint, 18, SL_SchemaVersion(documentNamespace(source)) >= slVersions.r5);
	 ...
	 */
	private GetLanguage(node : XmlElement, parentLang : string, isRequired : boolean, errs : ErrorList, errCode : string) {
		if (!node) return parentLang;
		if (!attrAnyNs(node, tva.a_lang) && isRequired) {
			errs.addError({
				code: errCode,
		...
		if (!attrAnyNs(node, tva.a_lang)) return parentLang;

		const localLang = attrAnyNsValueOrNull(node, tva.a_lang);
	...
	private ValidateSynopsisType(Element : XmlElement, ElementName : string, requiredLengths : string[], optionalLengths : string[], parentLanguage : string, errs : ErrorList, errCode : string) {
		...
		const synopsisLengthError = (label : string, length : number) => `length of ${`${tva.a_length.attribute(ElementName)}=${label.quote()}`.elementize()} exceeds ${length} characters`;
		const synopsisToShortError = (label : string, length : number) => `length of ${`${tva.a_length.attribute(ElementName)}=${label.quote()}`.elementize()} is less than ${length} characters`;
		const singleLengthLangError = (length : string, lang : string) => `only a single ${ElementName.elementize()} is permitted per length (${length}) and language (${lang})`;
		const requiredSynopsisError = (length : string) => `a ${ElementName.elementize()} element with ${tva.a_length.attribute()}=${length.quote()} is required`;
		...
		forEachNamedChildElement(Element, ElementName, (ste) => {
			const synopsisLang = this.GetLanguage(ste, parentLanguage, false, errs, `${errCode}-2`);
			const synopsisLength = attrAnyNsValueOrNull(ste, tva.a_length);
	...
	private hasServiceApplication(node : XmlElement, href : string, contentType : string) : boolean {
		let rc = false;
		let rm = 0,
			RelatedMaterial : XmlElement;
		while (!rc && (RelatedMaterial = getAnyNs(node, tva.e_RelatedMaterial, ++rm)) != null) {
			const HowRelated = getAnyNs(RelatedMaterial, tva.e_HowRelated);
			if (HowRelated && attrAnyNsValueOrNull(HowRelated, tva.a_href) == href) {
				let ml = 0,
					MediaLocator: XmlElement; 
				while (!rc && (MediaLocator = getAnyNs(RelatedMaterial, tva.e_MediaLocator, ++ml)) != null) {
					const MediaUri = getAnyNs(MediaLocator, tva.e_MediaUri);
					if (MediaUri && attrAnyNsValueOrNull(MediaUri,tva.a_contentType) == contentType) rc = true;
				}
			}
		}
		return rc;
	 ...
	 * @param {ErrorList}  errs                          errors found in validaton
	 */
	private validateServiceInstance(ServiceInstance : XmlElement, thisServiceId : string, declaredSubscriptionPackages : Array<string>, declaredAudioLanguages : Array<AnnouncedAudioLanguage_type>, errs : ErrorList) {
		...
		function checkMulticastDeliveryParams(params : XmlElement, errs : ErrorList, errCode : string) {
			const IPMulticastAddress = getAnyNs(params, dvbi.e_IPMulticastAddress);
			if (IPMulticastAddress) {
				const CNAME = getAnyNs(IPMulticastAddress, dvbi.e_CNAME);
		...
		const instanceNS = documentNamespace(ServiceInstance);
		const deliveryParameters = (instance : XmlElement) =>
			getAnyNs(instance, dvbi.e_DVBTDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_DVBSDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_DVBCDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_DASHDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_SATIPDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_MulticastTSDeliveryParameters) ||
			getAnyNs(instance, dvbi.e_RTSPDeliveryParameters);

		checkAttributes(ServiceInstance, [], [dvbi.a_priority, dvbi.a_id, tva.a_lang], dvbiEA.ServiceInstanceType, errs, "SI005");
		const ServiceInstance_elements = [
		...
		if (SL_SchemaVersion(instanceNS) >= slVersions.r3) ServiceInstance_elements.push({ name: dvbi.e_AltServiceName, minOccurs: 0, maxOccurs: Infinity });
		if (SL_SchemaVersion(instanceNS) >= slVersions.r6) ServiceInstance_elements.push({ name: dvbi.e_IdentifierBasedDeliveryParameters, minOccurs: 0 });
		checkTopElementsAndCardinality(
			ServiceInstance,
			ServiceInstance_elements,
		...
		if (SL_SchemaVersion(instanceNS) <= slVersions.r4) {
			const ServiceInstance_prior = attrAnyNs(ServiceInstance, dvbi.a_priority);
			if (ServiceInstance_prior && parseInt(ServiceInstance_prior.value) < 0)
				errs.addError({
					code: "SI011",
					message: `${dvbi.a_priority.attribute(dvbi.e_ServiceInstance)} should not be negative`,
					line: ServiceInstance_prior.line,
					key: `negative ${dvbi.a_priority.attribute()}`,
				});
		}

		//<ServiceInstance@id>
		const ServiceInstance_id = attrAnyNsValueOrNull(ServiceInstance, dvbi.a_id);
		...
		const controlApps : Array<XmlElement>= [];
		forEachNamedChildElement(ServiceInstance, dvbi.e_RelatedMaterial, (RelatedMaterial) => {
			...
			if (foundHref != "" && validServiceInstanceControlApplication(foundHref, SL_SchemaVersion(instanceNS))) controlApps.push(RelatedMaterial);
			if (foundHref == dvbi.APP_IN_CONTROL) {
				...
				if (SL_SchemaVersion(instanceNS) >= slVersions.r5 && foundDeliveryParams != null)
					errs.addError({
						type: WARNING,
						code: "SI022",
			...
			if ([dvbi.APP_IN_PARALLEL, dvbi.APP_IN_CONTROL, dvbi.APP_IN_SERIES].includes(foundHref)) {
				...
				forEachNamedChildElement(RelatedMaterial, tva.e_MediaLocator, (MediaLocator) => {
					const MediaUri = getAnyNs(MediaLocator, tva.e_MediaUri);
					if (MediaUri) {
						const contentType = attrAnyNsValueOrNull(MediaUri, tva.a_contentType);
		...
		forEachNamedChildElement(ServiceInstance, dvbi.e_ContentProtection, (ContentProtection) => {
			...
			forEachNamedChildElement(ContentProtection, dvbi.e_CASystemId, (CASystemID) => {
				checkAttributes(CASystemID, [], [dvbi.a_cpsIndex], [dvbi.a_cpsIndex], errs, "SI031");
				let CASystemID_value = null;
				if (SL_SchemaVersion(instanceNS) <= slVersions.r1) {
					// first two versions of the schema were 'incorrect' - has nested <CASystemId> elements.
					const nestedCAsystemid = getAnyNs(CASystemID, dvbi.e_CASystemId);
					if (nestedCAsystemid) {
				...
				if (CASystemID_value) {
					...
					if (isNaN(CASid_value)) {
						...
						if (!KnownCASystemID(CASid_value)) {
							errs.addError({
								code: "SI033",
			...
			forEachNamedChildElement(ContentProtection, dvbi.e_DRMSystemId, (DRMSystemID) => {
				...
				if (SL_SchemaVersion(instanceNS) <= slVersions.r1) {
					...
					const nestedDRMsystemid = getAnyNs(DRMSystemID, dvbi.e_DRMSystemId);
				...
				if (DRMSystemID_value && DRMSystemID_value.startsWith(UUID_type) && !isUUIDformat(DRMSystemID_value.substring(UUID_type.length))) {
					errs.addError({
						code: "SI043",
						message: "specified UUID is not in the correct format",
						fragment: DRMSystemID,
						key: keys.k_InvalidIdentifier,
					})
				}
				if (DRMSystemID_value && !KnownDRMScheme(DRMSystemID_value)) 
					errs.addError({
						code: "SI042",
						message: `${dvbi.e_DRMSystemId.elementize()} value (${DRMSystemID_value}) is not found in ${DASH_IF_Content_Protection_List}`,
						fragment: DRMSystemID,
						key: keys.k_InvalidIdentifier,
						description: "The value shall consist of DRM System ID values as described in clause 8.2 of ETSI TS 103 285",
						clause: "A177 Table 35",
					});
			});
/*			forEachNamedChildElement(ContentProtection, [dashif.e_Authzurl, dashif.e_Laurl], (DRMurl) => {
				if (!isHTTPURL(DRMurl.content))
					errs.addError({
						code: "SI045",
						fragment: DRMurl,
						key: keys.k_InvalidURL,
						message: `${DRMurl.name.elementize()} does not contain a valid URL`,
					});
			}); */
		});

		// <ServiceInstance><ContentAttributes>
		const ContentAttributes = getAnyNs(ServiceInstance, dvbi.e_ContentAttributes);
		if (ContentAttributes) {
			// Check ContentAttributes/AudioAttributes - other subelements are checked with schema based validation
			forEachNamedChildElement(ContentAttributes, tva.e_AudioAttributes, (conf) => {
				forEachChildElement(conf, (child) => {
					const child_href = attrAnyNsValueOrNull(child, dvbi.a_href);
					switch (child.name) {
						case tva.e_Coding:
							if (child_href && !this.allowedAudioSchemes.isIn(child_href))
								errs.addError({
									code: "SI052",
						...
						case tva.e_MixType:
							...
							if (child_href && !this.audioPresentations.isIn(child_href))
								errs.addError({
									code: "SI055",
									message: `invalid ${dvbi.a_href.attribute(child.name)} value (${child_href}) all nodes in the CS are used`,
									fragment: child,
									key: "audio codec",
								});
							break;
						case tva.e_AudioLanguage:
							...
							// eslint-disable-next-line no-case-declarations
							const AudioLanguage_purpose = attrAnyNsValueOrNull(child, tva.a_purpose);
							if (AudioLanguage_purpose) {
								...
								const profiledAudioPurposeCS = [dvbi.AUDIO_PURPOSE_MAIN];
								if (SL_SchemaVersion(documentNamespace(child)) < slVersions.r6)
								...
								if (!profiledAudioPurposeCS.includes(AudioLanguage_purpose)) {
									errs.addError({
										code: "SI054",
										message: `the value ${AudioLanguage_purpose.quote()} is not permitted for ${tva.a_purpose.attribute(tva.e_AudioLanguage)}.`,
										fragment: child,
										key: "audio purpose",
			...
			forEachNamedChildElement(ContentAttributes, dvbi.e_AudioConformancePoint, (conf) => {
				if (SL_SchemaVersion(instanceNS) > slVersions.r4) 
					errs.addError(DeprecatedElement(conf, SchemaSpecVersion(slVersions.r4), "SI062"));
				const conf_href = attrAnyNsValueOrNull(conf, dvbi.a_href);
				if (conf_href && !this.allowedAudioConformancePoints.isLeaf(conf_href))
					errs.addError({
						code: "SI061",
						message: `invalid ${dvbi.a_href.attribute(dvbi.e_AudioConformancePoint)} (${conf_href}) all nodes in the CS are used`,
						fragment: conf,
			...
			forEachNamedChildElement(ContentAttributes, tva.e_VideoAttributes, (conf) => {
				forEachChildElement(conf, (child) => {
					const child_href = attrAnyNsValueOrNull(child, dvbi.a_href);
					switch (child.name) {
						case tva.e_Coding:
							if (child_href && !this.allowedVideoSchemes.isIn(child_href))
								errs.addError({
									code: "SI072",
									message: `invalid ${dvbi.a_href.attribute(tva.e_Coding)} (${child_href}) -only leaf nodes are used from the CS`,
									fragment: child,
									key: "video codec",
								});
							break;
						case tva.e_PictureFormat:
							if (child_href && !this.allowedPictureFormats.isIn(child_href))
								errs.addError({
									code: "SI082",
									message: `invalid ${dvbi.a_href.attribute(tva.e_PictureFormat)} value (${child_href}) all nodes in the CS are used`,
									fragment: child,
									key: tva.e_PictureFormat,
								});
							break;
						case dvbi.e_Colorimetry:
							if (child_href && !this.allowedColorimetry.isLeaf(child_href))
								errs.addError({
									code: "SI084",
									message: `invalid ${dvbi.a_href.attribute(dvbi.e_Colorimetry)} value (${child_href}) only leaf nodes in the CS are used`,
									fragment: child,
			...
			const isHDRDMISystem = (CSterm : string) : boolean => CSterm.substring(CSterm.lastIndexOf(":") + 1).startsWith("2.3.");

			// Check @href of ContentAttributes/VideoConformancePoints
			let codec_count = 0;
			const conf_points = [];
			forEachNamedChildElement(ContentAttributes, dvbi.e_VideoConformancePoint, (conf) => {
				const conformanceVal = attrAnyNsValueOrNull(conf, dvbi.a_href);
				if (conformanceVal) {
					if (!this.allowedVideoConformancePoints.isLeaf(conformanceVal))
						errs.addError({
							code: "SI091",
							message: `invalid ${dvbi.a_href.attribute(dvbi.e_VideoConformancePoint)} value (${conformanceVal}) -only leaf nodes are used from the CS`,
							fragment: conf,
			...
			forEachNamedChildElement(ContentAttributes, tva.e_CaptionLanguage, (conf) => {
				checkLanguage(conf.content, conf, errs, "SI101");
			});
			// Check ContentAttributes/SignLanguage
			forEachNamedChildElement(ContentAttributes, tva.e_SignLanguage, (conf) => {
				checkLanguage(conf.content, conf, errs, "SI111");
			});

			// Check ContentAttributes/AccessibilityAttributes
			const aa = getAnyNs(ContentAttributes, tva.e_AccessibilityAttributes);
			if (aa) {
				CheckAccessibilityAttributes(
					aa,
					{
		...
		const Availability = getAnyNs(ServiceInstance, dvbi.e_Availability);
		if (Availability) {
			forEachNamedChildElement(Availability, dvbi.e_Period, (Period) => {
				if (attrAnyNs(Period, dvbi.a_validFrom) && attrAnyNs(Period, dvbi.a_validTo)) {
					// validTo should be >= validFrom
					const ZuluIfNeeded = (dt : string) : string => (dt.endsWith("Z") || dt.lastIndexOf("+") > 12 || dt.lastIndexOf("-") > 12) ? dt : `${dt}Z`; 
					try {
						const fr = Temporal.Instant.from(ZuluIfNeeded(attrAnyNs(Period, dvbi.a_validFrom).value)),
							to = Temporal.Instant.from(ZuluIfNeeded(attrAnyNs(Period, dvbi.a_validTo).value));
				...
				forEachNamedChildElement(Period, dvbi.e_Interval, (Interval) => {
					const unzone = (time : string) : string => time.includes("Z") ? time.substring(0, time.indexOf("Z")) : time;
					const startTime = unzone(attrAnyNsValueOr(Interval, dvbi.a_startTime, "00:00:00Z")),
						endTime = unzone(attrAnyNsValueOr(Interval, dvbi.a_endTime, "23:59:59.999Z"));
					try {
						if (Temporal.PlainTime.compare(Temporal.PlainTime.from(startTime), Temporal.PlainTime.from(endTime)) == 1) 
							errs.addError({
								code: "SI125",
								message: `invalid availability interval for service ${thisServiceId.quote()}. ${startTime} > ${endTime}`,
								fragment: Interval,
		...
		const packages_found : Array<string> = [];
		forEachNamedChildElement(ServiceInstance, dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
			const pkg_lang = mlLanguage(SubscriptionPackage);
			const pkg = localizedSubscriptionPackage(SubscriptionPackage, pkg_lang);
			if (SL_SchemaVersion(instanceNS) >= slVersions.r3) {
				if (!declaredSubscriptionPackages.includes(pkg))
					errs.addError({
						code: "SI130",
						message: `${dvbi.e_SubscriptionPackage.elementize()} ${SubscriptionPackage.content.quote()} with language ${pkg_lang.quote()} is not declared in ${dvbi.e_SubscriptionPackageList.elementize()}`,
			...
			if (isIn(packages_found, pkg)) {
				errs.addError({
					type: WARNING,
					code: "SI131",
					message: `${dvbi.e_SubscriptionPackage.elementize()} ${SubscriptionPackage.content.quote()}" with language ${pkg_lang.quote()} is already defined in this service instance`,
					fragment: SubscriptionPackage,
		...
		const SourceType = getAnyNs(ServiceInstance, dvbi.e_SourceType);
		if (SourceType) {
			let v1Params = false;
			switch (SourceType.content) {
				case dvbi.DVBT_SOURCE_TYPE:
					if (!hasChild(ServiceInstance, dvbi.e_DVBTDeliveryParameters)) errs.addError(NoDeliveryParams("DVB-T", thisServiceId, SourceType, "SI151"));
					v1Params = true;
					break;
				case dvbi.DVBS_SOURCE_TYPE:
					if (!hasChild(ServiceInstance, dvbi.e_DVBSDeliveryParameters)) errs.addError(NoDeliveryParams("DVB-S", thisServiceId, SourceType, "SI152"));
					v1Params = true;
					break;
				case dvbi.DVBC_SOURCE_TYPE:
					if (!hasChild(ServiceInstance, dvbi.e_DVBCDeliveryParameters)) errs.addError(NoDeliveryParams("DVB-C", thisServiceId, SourceType, "SI153"));
					v1Params = true;
					break;
				case dvbi.DVBDASH_SOURCE_TYPE:
					if (!hasChild(ServiceInstance, dvbi.e_DASHDeliveryParameters)) errs.addError(NoDeliveryParams("DVB-DASH", thisServiceId, SourceType, "SI154"));
					v1Params = true;
					break;
				case dvbi.DVBIPTV_SOURCE_TYPE:
					if (!hasChild(ServiceInstance, dvbi.e_MulticastTSDeliveryParameters) && !hasChild(ServiceInstance, dvbi.e_RTSPDeliveryParameters))
				...
				case dvbi.DVBAPPLICATION_SOURCE_TYPE:
					...
					break;
				default:
					switch (SL_SchemaVersion(instanceNS)) {
						case slVersions.r0:
							errs.addError({
								code: "SI158",
						...
						case slVersions.r4:
						case slVersions.r5:
							if (!hasChild(ServiceInstance, dvbi.e_OtherDeliveryParameters))
								errs.addError({
									code: "SI159",
			...
			if (v1Params && SL_SchemaVersion(instanceNS) >= slVersions.r1) 
				errs.addError(DeprecatedElement(SourceType, SchemaSpecVersion(slVersions.r1), "SI160"));
		} else {
			if (SL_SchemaVersion(instanceNS) == slVersions.r0)
				errs.addError({
					code: "SI161",
		...
		forEachNamedChildElement(ServiceInstance, dvbi.e_AltServiceName, (altSN) => {
		...
		const DASHDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_DASHDeliveryParameters);
		if (DASHDeliveryParameters) {
			const URIBasedLocation = getAnyNs(DASHDeliveryParameters, dvbi.e_UriBasedLocation);
			if (URIBasedLocation) {
				const uriContentType = attrAnyNsValueOrNull(URIBasedLocation, dvbi.a_contentType);
				if (uriContentType && !validDASHcontentType(uriContentType, SL_SchemaVersion(instanceNS)))
					errs.addError({
						code: "SI173",
						...
						key: `not ${dvbi.a_contentType.attribute()} for DASH`,
						clause: "A177 clause 5.2.7.2",
						description: `${dvbi.a_contentType.attribute()} indicates if the URL is for a DASH MPD or a Playlist`,
					});

				const uri = getAnyNs(URIBasedLocation, dvbi.e_URI);
			...
			forEachNamedChildElement(DASHDeliveryParameters, dvbi.e_Extension, (Extension) => {
				CheckExtension(Extension, EXTENSION_LOCATION_DASH_INSTANCE, errs, "SI179");
			});
		}

		// <ServiceInstance><DVBTDeliveryParameters>
		const DVBTDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_DVBTDeliveryParameters);
		if (DVBTDeliveryParameters) {
			const DVBTtargetCountry = getAnyNs(DVBTDeliveryParameters, dvbi.e_TargetCountry);
			if (DVBTtargetCountry) {
				...
				if (SL_SchemaVersion(instanceNS) >= slVersions.r6) 
		...
		const DVBCDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_DVBCDeliveryParameters);
		if (DVBCDeliveryParameters) {
			const DVBCtargetCountry = getAnyNs(DVBCDeliveryParameters, dvbi.e_TargetCountry);
			...
			if (SL_SchemaVersion(instanceNS) >= slVersions.r7) {
				// prior to A177r7 only a single value for <DVBCDeliveryParameters><NetworkID> was specified
				const knownIDs = [];
				forEachNamedChildElement(DVBCDeliveryParameters, dvbi.e_NetworkID, (NetworkID) => {
		...
		const DVBSDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_DVBSDeliveryParameters);
		if (DVBSDeliveryParameters) {
			const ERROR_KEY = "satellite tuning";
			const ModulationSystem = getAnyNs(DVBSDeliveryParameters, dvbi.e_ModulationSystem);
			const RollOff = getAnyNs(DVBSDeliveryParameters, dvbi.e_RollOff);
			const ModulationType = getAnyNs(DVBSDeliveryParameters, dvbi.e_ModulationType);
			const FEC = getAnyNs(DVBSDeliveryParameters, dvbi.e_FEC);

			if (ModulationSystem) {
				const checkElement = (element : XmlElement, elementName : string, allowed : string[], modulation : string, errCode : string) => {
				...
				const DisallowedElement = (element : XmlElement, childElementName : string, modulation : string, suffix : string = "-0") => {
					if (hasChild(element, childElementName))
						errs.addError({
							...
							fragment: getAnyNs(element, childElementName),
						});
				};
				...
				switch (ModulationSystem.content) {
					...
					case sats.MODULATION_S2X:
						...
						// eslint-disable-next-line no-case-declarations
						const ChannelBonding = getAnyNs(DVBSDeliveryParameters, dvbi.e_ChannelBonding);
						if (ChannelBonding) {
							const freqs = [];
							let primarySpecified = false;
							forEachNamedChildElement(ChannelBonding, dvbi.e_Frequency, (Frequency) => {
								...
								const Frequency_primary = attrAnyNsValueOr(Frequency, dvbi.a_primary, null);
		...
		const RTSPDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_RTSPDeliveryParameters);
		if (RTSPDeliveryParameters) {
			const RTSPURL = getAnyNs(RTSPDeliveryParameters, dvbi.e_RTSPURL);
		...
		const MulticastTSDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_MulticastTSDeliveryParameters);
		...
		const OtherDeliveryParameters = getAnyNs(ServiceInstance, dvbi.e_OtherDeliveryParameters);
	 ...
	 * @param {ErrorList}  errs                          errors found in validaton
	 */
	private validateService(service : XmlElement, thisServiceId : string, knownServices : string[], knownRegionIDs : StoredRegion_type[], declaredSubscriptionPackages : string[], declaredAudioLanguages : AnnouncedAudioLanguage_type[], ContentGuideSourceIDs : string[], errs : ErrorList) {
		...
		const uID = getAnyNs(service, dvbi.e_UniqueIdentifier);
		if (uID) {
		...
		forEachNamedChildElement(service, dvbi.e_ServiceInstance, (ServiceInstance) => 
			this.validateServiceInstance(ServiceInstance, thisServiceId, declaredSubscriptionPackages, declaredAudioLanguages, errs)
		);

		//check <TargetRegion>
		const rBuf = new Set<string>();
		forEachNamedChildElement(service, dvbi.e_TargetRegion, (TargetRegion) => {
		...
		forEachNamedChildElement(service, tva.e_RelatedMaterial, (RelatedMaterial) => 
			this.validateRelatedMaterial(RelatedMaterial, `service ${thisServiceId.quote()}`, SERVICE_RM, errs, "SL150")
		);
		//check <ServiceGenre>
		forEachNamedChildElement(service, dvbi.e_ServiceGenre, (ServiceGenre) => {
			checkAttributes(ServiceGenre, [tva.a_href], [tva.a_type], tvaEA.Genre, errs, "SL160");
			const Genre_type = attrAnyNsValueOrNull(ServiceGenre, tva.a_type);
			...
			const Genre_value = attrAnyNsValueOrNull(ServiceGenre, dvbi.a_href);
			if (Genre_value && !this.allowedGenres.isIn(Genre_value))
				errs.addError({
					code: "SL162",
					message: `service ${thisServiceId.quote()} has an invalid ${dvbi.a_href.attribute(dvbi.e_ServiceGenre)} value ${Genre_value.quote()} (must be content genre)`,
					fragment: ServiceGenre,
					key: `invalid ${dvbi.a_href.attribute(dvbi.e_ServiceGenre)}`,
					clause: "A177 Table 15",
					description: "A genre that characterizes the programming on the service.",
				});
		});

		//check <ServiceType>
		const ServiceType = getAnyNs(service, dvbi.e_ServiceType);
		if (ServiceType) {
			const ServiceType_href = attrAnyNsValueOrNull(ServiceType, dvbi.a_href);
		...
		// check <RecordingInfo>
		const RecordingInfo = getAnyNs(service, dvbi.e_RecordingInfo);
		if (RecordingInfo) {
			const RI_href = attrAnyNsValueOrNull(RecordingInfo, dvbi.a_href);
			if (RI_href && !this.RecordingInfoCSvalues.isIn(RI_href))
			errs.addError({
				code: "SL180",
				message: `invalid ${dvbi.e_RecordingInfo.elementize()} value ${RI_href.quote()} for service ${thisServiceId} all nodes in the CS are used`,
		...
		const sCG = getAnyNs(service, dvbi.e_ContentGuideSource);
		...
		const sCGref = getAnyNs(service, dvbi.e_ContentGuideSourceRef);
		...
		forEachNamedChildElement(service, dvbi.e_AdditionalServiceParameters, (AdditionalParams) =>
			CheckExtension(AdditionalParams, EXTENSION_LOCATION_SERVICE_ELEMENT, errs, "SL211")
		);

		// check <NVOD>
		const NVOD = getAnyNs(service, dvbi.e_NVOD);
		if (NVOD) {
			const NVOD_mode = attrAnyNsValueOrNull(NVOD, dvbi.a_mode);
			if (NVOD_mode && NVOD_mode == dvbi.NVOD_MODE_REFERENCE) {
				if (attrAnyNs(NVOD, dvbi.a_reference))
					errs.addError({
						code: "SL221",
				...
				if (attrAnyNs(NVOD, dvbi.a_offset))
					errs.addError({
						code: "SL222",
			...
			if (NVOD_mode && NVOD_mode == dvbi.NVOD_MODE_TIMESHIFTED) {
				...
				const NVOD_reference = attrAnyNsValueOrNull(NVOD, dvbi.a_reference);
				if (NVOD_reference) {
					...
					const referenced_service = attrAnyNsValueOrNull(NVOD, dvbi.a_reference);

					while ((service2 = getAnyNs(ServiceList, dvbi.e_Service, ++s2)) != null && !referredService) {
						const ui = getAnyNs(service2, dvbi.e_UniqueIdentifier);
					...
					if (!referredService)
						...
						const refNVOD = getAnyNs(referredService, dvbi.e_NVOD);
						if (!refNVOD)
							errs.addError({
								code: "SL225b",
							...
							const refNVOD_mode = attrAnyNsValueOrNull(refNVOD, dvbi.a_mode);
			...
			const svcType = getAnyNs(service, dvbi.e_ServiceType);
			if (svcType && attrAnyNs(svcType, dvbi.a_href) && !attrAnyNs(svcType, dvbi.a_href).value.endsWith("linear"))
		...
		// check <Prominence>
		const ProminenceList = getAnyNs(service, dvbi.e_ProminenceList);
		if (ProminenceList) {
			const	known = [];
			forEachNamedChildElement(ProminenceList, dvbi.e_Prominence, (PE) => {
				// if @region is used, it must be in the RegionList
				const PE_region = attrAnyNsValueOrNull(PE, dvbi.a_region);
				if (PE_region) {
				...
				const PE_country = attrAnyNsValueOrNull(PE, dvbi.a_country);
				...
				// for exact match
				const PE_ranking = attrAnyNsValueOrNull(PE, dvbi.a_ranking);
		...
		const ParentalRating = getAnyNs(service, dvbi.e_ParentalRating);
		if (ParentalRating) {
			const foundCountries = [];
			let noCountrySpecified = false;
			forEachNamedChildElement(ParentalRating, dvbi.e_MinimumAge, (MinimumAge) => {
				const MinimumAge_countryCodes = attrAnyNsValueOrNull(MinimumAge, dvbi.a_countryCodes);
						...
						.forEach((country) => {
							...
							if (isIn(foundCountries, country))
								errs.addError({
									...
									description: `A maximum of one ${dvbi.e_MinimumAge} shall be defined per country`,
								});
	...
	private ValidateRequirements(Requires : XmlElement, errs : ErrorList, errCode : string) {
		...
		const Delivery = getAnyNs(Requires, dvbisld.e_Delivery);
		const documentNS = documentNamespace(Requires);
		if (Delivery) {
			forEachNamedChildElement(Delivery, dvbisld.e_DVBTDelivery, (DVBTDelivery) => {
				if (DVBTDelivery.attr(dvbisld.a_originalNetworkID) && SL_SchemaVersion(documentNS) >= slVersions.r6) {
					errs.addError(DeprecatedAttribute(DVBTDelivery.attr(dvbisld.a_originalNetworkID), SchemaSpecVersion(slVersions.r6), `${errCode}-10`));
				}
			});

			forEachNamedChildElement(Delivery, dvbisld.e_DVBSDelivery, (DVBSDelivery) => {
				if (DVBSDelivery.attr(dvbisld.a_originalNetworkID) && SL_SchemaVersion(documentNS) >= slVersions.r6) {
					errs.addError(DeprecatedAttribute(DVBSDelivery.attr(dvbisld.a_originalNetworkID), SchemaSpecVersion(slVersions.r6), `${errCode}-20`));
				}
			});
			const ApplicationDelivery = getAnyNs(Delivery, dvbisld.e_ApplicationDelivery);
			if (ApplicationDelivery) {
				forEachNamedChildElement(ApplicationDelivery, dvbisld.e_ApplicationType, (ApplicationType) => {
					const Application_contentType = attrAnyNsValueOrNull(ApplicationType, dvbisld.a_contentType);
					if (Application_contentType) {
						...
						if (Application_contentType == dvbi.XML_AIT_CONTENT_TYPE) {
							const xmlAitApplicationType = attrAnyNsValueOrNull(ApplicationType, dvbisld.a_xmlAitApplicationType);
							if (xmlAitApplicationType) {
								if (!isMIME(xmlAitApplicationType))
									errs.addError({
										code: `${errCode}-47`,
										message: `${dvbisld.a_xmlAitApplicationType.attribute()} must contain the MIME type of the signalled application`,
										fragment: ApplicationType,
										key: "invalid format",
										clause: "A177 clause 5.2.4.1",
										description: "Other MIME type values may also be signalled for applications specific to the DVB-I client or the underlying platform.",
									});
								else if (!DvbIApplicationTypes.includes(xmlAitApplicationType))
									errs.addError({
										code: `${errCode}-49`,
										type: WARNING,
										message: `non-standard application type (${xmlAitApplicationType}) specified`,
										fragment: ApplicationType,
										key: "non-standard",
									});
							} else
								errs.addError({
									code: `${errCode}-48`,
									message: `${dvbisld.a_xmlAitApplicationType.attribute()} must be specified when ${dvbisld.a_contentType.attribute()}=${dvbi.XML_AIT_CONTENT_TYPE.quote()}`,
									fragment: ApplicationType,
									key: `missing ${dvbisld.a_xmlAitApplicationType.attribute(dvbisld.e_ApplicationType)}`,
									clause: "A177 Table 12h",
									description: `If @contentType=${dvbi.XML_AIT_CONTENT_TYPE.quote()}, the ${dvbisld.a_xmlAitApplicationType} attribute shall be included ...`,
								});
						}
					}
				});
		...
		const extraCaps = attrAnyNsValueOrNull(Requires, dvbi.a_extraCapabilities);
		if (extraCaps)
			extraCaps.split(" ").forEach((capability) => {
				if (!this.allowedAudioSchemes.isIn(capability) && !this.allowedVideoSchemes.isIn(capability) && !KnownDRMScheme(capability)) {
					errs.addError({
						type: WARNING,
						code: `${errCode}-30`,
						line: Requires.line,
						message: `the value ${capability.quote()} is not a term from the audio codecs scheme or the video codecs scheme or a known DRM System identifier`,
	...
	private doSchemaVerification(ServiceList : XmlDocument, errs : ErrorList, errCode : string, report_schema_version : boolean = true) {
	 ...
	 */
	/*public*/ doValidateServiceList(SLtext : string, errs : ErrorList, options : { log_prefix?: string | null, report_schema_version?: boolean } = {}) {
		this.numRequests++;
		if (!SLtext) {
			errs.addError({
				type: APPLICATION,
		...
		const ServiceList_lang = attrAnyNsValueOrNull(ServiceList, tva.a_lang);
		if (ServiceList_lang) {
		...
		const thisServiceListId = attrAnyNsValueOrNull(ServiceList, dvbi.a_id);
		...
		forEachNamedChildElement(ServiceList, dvbi.e_StandardVersion, (StandardVersion) => {
		...
		//check <ServiceList><Requires>
		forEachNamedChildElement(ServiceList, dvbi.e_Requires, (Requires) => {
			this.ValidateRequirements(Requires, errs, "SL025");
		});

		//check <ServiceList><LanguageList>
		const announcedAudioLanguages : AnnouncedAudioLanguage_type[] = [];
		const LanguageList = getAnyNs(ServiceList, dvbi.e_LanguageList);
		if (LanguageList) {
			forEachNamedChildElement(LanguageList, tva.e_Language, (Language) => {
		...
		let countControlApps = 0;
		forEachNamedChildElement(ServiceList, tva.e_RelatedMaterial, (RelatedMaterial) => {
		...
		const RegionList = getAnyNs(ServiceList, dvbi.e_RegionList);
		if (RegionList) {
			// recurse the regionlist - Regions can be nested in Regions
			forEachNamedChildElement(RegionList, dvbi.e_Region, (Region) => this.addRegion(Region, 0, knownRegionIDs, null, errs));
		}

		//check <ServiceList><TargetRegion>
		const rBuf = new Set<string>();
		forEachNamedChildElement(ServiceList, dvbi.e_TargetRegion, (TargetRegion) => {
			const found = knownRegionIDs.find((r) => r.region == TargetRegion.content);
		...
		const declaredSubscriptionPackages = [];
		const SubscriptionPackageList = getAnyNs(ServiceList, dvbi.e_SubscriptionPackageList);
		if (SubscriptionPackageList) {
			forEachNamedChildElement(SubscriptionPackageList, dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
		...
		const ContentGuideSourceIDs = [];
		const CGSourceList = getAnyNs(ServiceList, dvbi.e_ContentGuideSourceList);
		if (CGSourceList) {
			let cgs = 0;
			forEachNamedChildElement(CGSourceList, dvbi.e_ContentGuideSource, (CGSource) => {
				...
				const CGSource_CGSID = attrAnyNsValueOr(CGSource, dvbi.a_CGSID, null);
				if (CGSource_CGSID) {
		...
		const slGCS = getAnyNs(ServiceList, dvbi.e_ContentGuideSource);
		...
		let s = 0;
		const knownServices = [];
		forEachNamedChildElement(ServiceList, dvbi.e_Service, (service) => {
			s++;
		...
		if (SL_SchemaVersion(ns) >= slVersions.r5) {
			...
			// check <TestService>
			let ts = 0;
			forEachNamedChildElement(ServiceList, dvbi.e_TestService, (testService) => {
				ts++;
		...
		forEachNamedChildElement(ServiceList, dvbi.e_Service, (service) => {
			const CGSR = getAnyNs(service, dvbi.e_ContentGuideServiceRef);
			if (CGSR) {
				const uniqueID = getAnyNs(service, dvbi.e_UniqueIdentifier);
		...
		if (SL_SchemaVersion(ns) >= slVersions.r5) {
			...
			// issues a warning if this is a reference to self
			forEachNamedChildElement(ServiceList, dvbi.e_TestService, (testService) => {
				const CGSR = getAnyNs(testService, dvbi.e_ContentGuideServiceRef);
				if (CGSR) {
					const uniqueID = getAnyNs(testService, dvbi.e_UniqueIdentifier);
					if (uniqueID && CGSR.content == uniqueID.content)
						errs.addError({
							type: WARNING,
							code: "SL231",
		...
		const LCNtableList = getAnyNs(ServiceList, dvbi.e_LCNTableList);
		if (LCNtableList) {
			const tableQualifiers = new Set<string>();
			forEachNamedChildElement(LCNtableList, dvbi.e_LCNTable, (LCNTable) => {
				// <LCNTable><TargetRegion>
				const TargetRegions = new Set<string>();
				forEachNamedChildElement(LCNTable, dvbi.e_TargetRegion, (TargetRegion) => {
				...
				forEachNamedChildElement(LCNTable, dvbi.e_SubscriptionPackage, (SubscriptionPackage) => {
					let packageLanguage = null;
					if (SL_SchemaVersion(ns) >= slVersions.r5) {
						errs.addError(DeprecatedElement(SubscriptionPackage, SchemaSpecVersion(slVersions.r5), "SL244"));
					}
					const SubscriptionPackage_lang = attrAnyNsValueOrNull(SubscriptionPackage, tva.a_lang);
					if (SubscriptionPackage_lang) {
						packageLanguage = SubscriptionPackage_lang;
				...
				if (TargetRegions.size == 0) TargetRegions.add(LCN_TABLE_NO_TARGETREGION);
				if (SubscriptionPackages.size == 0) SubscriptionPackages.add(LCN_TABLE_NO_SUBSCRIPTION);
				...
				// <LCNTable><LCN>
				const LCNNumbers = [];
				forEachNamedChildElement(LCNTable, dvbi.e_LCN, (LCN) => {
					// LCN@channelNumber
					const LCN_channelNumber = attrAnyNsValueOrNull(LCN, dvbi.a_channelNumber);
					if (LCN_channelNumber) {
						if (isIn(LCNNumbers, LCN_channelNumber))
							errs.addError({
								code: "SL262",
								message: `duplicated channel number ${LCN_channelNumber} for ${dvbi.e_LCNTable.elementize()}`,
								key: "duplicate channel number",
								fragment: LCN,
							});
						else LCNNumbers.push(LCN_channelNumber);
						const chanNumV = parseInt(LCN_channelNumber, 10);
					...
					// LCN@serviceRef
					const LCN_serviceRef = attrAnyNsValueOrNull(LCN, dvbi.a_serviceRef);
				...
				// <LCNTable><LCNRange>
				forEachNamedChildElement(LCNTable, dvbi.e_LCNRange, (LCNRange) => {
					...
					const serviceType = attrAnyNsValueOrNull(LCNRange, dvbi.a_serviceType);
					if (serviceType) {
					...
					const serviceGenre = attrAnyNsValueOrNull(LCNRange, dvbi.a_serviceGenre);
		...
		announcedAudioLanguages.forEach((language) => {
			if (!language.used)
				errs.addError({
					code: "SL282",
					type: WARNING,
					message: `audio language ${language.language.quote()} is defined in ${dvbi.e_LanguageList.elementize()} but not used`,
