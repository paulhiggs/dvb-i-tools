...
import { documentNamespace, attrAnyNsValueOr, attrAnyNsValueOrNull } from "../libxml2-wasm-extensions.mts";
...
type VersionInfo_type = {
	ver: number;
	val: string;
};
const OutOfScheduledHoursBanners : VersionInfo_type[] = [
...
const ContentFinishedBanners : VersionInfo_type[] = [
...
const ServiceListLogos : VersionInfo_type[] = [
...
const ServiceLogos : VersionInfo_type[] = [
...
const ServiceBanners : VersionInfo_type[] = [
...
const ContentGuideSourceLogos : VersionInfo_type[] = [
 ...
 */
export const SchemaSpecVersion = (version : number) : string => {
 ...
 */
export const GetSchema = (namespace : string) : SchemaVersion_type | undefined => SchemaVersions.find((s) => s.namespace == namespace);
 ...
 */
export const validServiceUnavailableApplication = (hrefType: string) : boolean => (hrefType == dvbi.APP_OUTSIDE_AVAILABILITY);
 ...
 */
function match(permittedValues : Array<VersionInfo_type>, value: string, namespace: string) : boolean {
...
export const validOutScheduleHours = (HowRelated : XmlElement) : boolean => 
	match(OutOfScheduledHoursBanners, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));

export const isOutScheduleHours = (HowRelated : XmlElement) => attrAnyNsValueOr(HowRelated, dvbi.a_href, "none").endsWith(OUTSIDE_AVAILABILITY_TERM);
 ...
 */
export const validContentFinishedBanner = (HowRelated : XmlElement) => match(ContentFinishedBanners, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));

export const isContentFinishedBanner = (HowRelated : XmlElement) => attrAnyNsValueOr(HowRelated, dvbi.a_href, "none").endsWith(CONTENT_FINISHED_TERM);
 ...
 */
export const validServiceListLogo = (HowRelated : XmlElement) : boolean => match(ServiceListLogos, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));
 ...
 */
export const validServiceLogo = (HowRelated : XmlElement) : boolean => match(ServiceLogos, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));
 ...
 */
export const validServiceBanner = (HowRelated : XmlElement) : boolean => match(ServiceBanners, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));
 ...
 */
export const validContentGuideSourceLogo = (HowRelated : XmlElement) : boolean => match(ContentGuideSourceLogos, attrAnyNsValueOrNull(HowRelated, dvbi.a_href), documentNamespace(HowRelated));
...
const validAgreementApplication = (hrefType: string, schemaVersion: number) : boolean => schemaVersion >= slVersions.r7 && appTypes.includes(hrefType);
...
export function validServiceAgreementApp(HowRelated: XmlElement): boolean {
	const HRhref = attrAnyNsValueOrNull(HowRelated, dvbi.a_href);
	return HRhref ? validAgreementApplication(HRhref, SL_SchemaVersion(documentNamespace(HowRelated))) : false;
}
 ...
 */
export const isA177specification_URN = (urn : string) : boolean => SchemaVersions.find((v) => v?.URN == urn) != undefined;

/**
 * determines if the A177 specification version from the  URN
 *
 * @param {String} urn   The URN to check
 * @returns {slVersions} A177 version number or "undefined"
 */
