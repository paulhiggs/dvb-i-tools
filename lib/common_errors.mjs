...
export let DeprecatedElement = (what, when, errCode) => ({
	...
	fragment: what,
	line: what.line,
	message: `Element ${what.name.elementize()} in ${what.parent.name.elementize()} is deprecated in ${when}`,
...
export let DeprecatedAttribute = (what, when, errCode) => ({
	...
	message: `Attribute ${what.name.elementize(what.parent.name)} is deprecated in ${when}`,
