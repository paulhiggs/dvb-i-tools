...
export let DeprecatedElement = (what, since, errCode) => ({
	type: WARNING,
	code: errCode,
	fragment: what,
...
export let DeprecatedAttribute = (what, since, errCode) => ({
	type: WARNING,
	code: errCode,
	fragment: what.parent,
	message: `Attribute ${what.name.elementize(what.parent.name)} is deprecated since ${since}`,
 ...
 * @param {String} loc      The location of the element
 */
export let InvalidCountryCode = (value, src, loc) => `invalid country code ${value.quote()} ${src ? `for ${src} parameters ` : ""}in ${loc}`;
