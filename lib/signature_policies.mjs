...
export function  ValidateSignaturePolicies(element, errs, errCode) {
	const policies_found = [];
		...
		const TrustAnchor = element.getAnyNs(dvbi.e_TrustAnchor);
		if (TrustAnchor)
			TrustAnchor.forEachNamedChildElement(dvbi.e_CAFingerprint, (CAFingerprint) => {
				...
				if (Fingerprint_algorithm && !dvbi.ALLOWED_FINGERPRINT_ALGOS.includes(Fingerprint_algorithm))
					errs.addError({
						code: `${errCode}-02`,
						message: `${Fingerprint_algorithm} is not a valif algorithm for ${dvbi.e_CAFingerprint.elementize()}`,
						key: keys.k_SignaturePolicies,
