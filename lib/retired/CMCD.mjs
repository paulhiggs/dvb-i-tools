...
import { isObjectEmpty, parameterCheck } from "./utils.mjs";
...
function checkCMCDkeys(CMCDreport, errs, errCode) {
	if (!parameterCheck("checkCMCDkeys", CMCDreport, dvbi.e_Report, errs, "CK000")) return;

	const keys_to_use = CMCDv1_keys;
	const enabled_keys = CMCDreport.attrAnyNs(dvbi.a_enabledKeys)?.value.split(" ");
...
function check_CMCD(CMCDelem, counts, errs, errCode) {
	...
	CMCDelem.forEachNamedChildElement(dvbi.e_Report, (CMCDreport) => {
