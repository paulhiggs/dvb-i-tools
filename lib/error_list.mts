...
const nthIndexOf = (str: string, pat: string, n: number) => {
	let i = -1;
...
export type ReportedErrorType = {
	...
	description?: string, 
	clause?: string, 
	multiElementError?: XmlElement[] | string[], 
	fragments?: (string | XmlElement)[],
	reportInTable?: boolean, // whether to include the error in the error table (default true). This is set to false for errors that are only used for debugging and should not be reported to the user
}
...
export default class ErrorList {
	...
	fatals: LoggedErrorType[];
	errors: LoggedErrorType[];
	warnings: LoggedErrorType[];
	debugs: LoggedErrorType[];
	informationals: LoggedErrorType[];
	markupXML: MarkupLine[];
	...
	private debugMessage(err : LoggedErrorType) {
	...
	addError(e : ReportedErrorType) {
		...
		if (e.type && ![FATAL, ERROR, WARNING, INFORMATION, APPLICATION, DEBUG].includes(e.type)) {
		...
		if (e.type == DEBUG) {
			...
			e.fragments.forEach((fragment) => {
				const newError = { code: e.code, message: e.message };
				if (fragment) {
