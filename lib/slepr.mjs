...
import { LoadLanguages, LoadCountries, LoadGenres } from "./classification_scheme_loaders.mjs";
...
export default class SLEPR {
	...
	/* public */ loadDataFiles(useURLs, preloadedLanguageValidator = null, preloadedCountries = null, preloadedGenres = null) {
		this.#knownLanguages = preloadedLanguageValidator || LoadLanguages(useURLs);
		this.#knownCountries = preloadedCountries || LoadCountries(useURLs);
		this.#knownGenres = preloadedGenres || LoadGenres(useURLs);
	}
	...
	/* private */ #checkQuery(req, params) {
		...
		if (req.query) {
			...
			//Language(s)
			const checkLanguage = (language) => isTVAAudioLanguageType(language, false);
			...
			// Genre(s)
			const checkGenre = (genre) => this.#knownGenres.isIn(genre);
