...
export default class SLEPR {
	...
	/* public */ processServiceListRequest(req, res) {
		...
		if (ix != -1) requestedVersion = Number.parseInt(uas.substring(ix + DVBIuasPrefix.length));
		...
		if (queryParams.regulatorListFlag || queryParams.Language || queryParams.TargetCountry || queryParams.Genre || requestedVersion != -1) {
			...
			while ((prov = slepr.get("//" + xPath(props.prefix, dvbisld.e_ProviderOffering, ++p), props.schema)) != null) {
				...
				while ((serv = prov.get(xPath(props.prefix, dvbisld.e_ServiceListOffering, ++s), props.schema)) != null) {
					...
					if (!removeService && requestedVersion != -1) {
						...
						while (
							!keepService &&
							(ServiceListURI = serv.get(xPath(props.datatypes_prefix ? props.datatypes_prefix : props.prefix, dvbisld.e_ServiceListURI, ++u), props.schema))
						) {
