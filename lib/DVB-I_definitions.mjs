...
import { tva, tvaEA, tvaEC, TVA_CSmetadata } from "./TVA_definitions.mjs";
import { HBBTV_APP_TYPE } from "./HbbTV_definitions.mjs";
...
export const CONTENT_FINISHED_TERM = "1000.2",
	OUTSIDE_AVAILABILITY_TERM = "1000.1";
...
const CMCDdelivery = `${CMCDterm}:delivery`;
export const CMCD_MODE_REQUEST = `${CMCDdelivery}:request`;
export const CMCD_MODE_EVENT = `${CMCDdelivery}:event`;

export const CMCD_METHOD_HTTP_HEADER = `${CMCDdelivery}:customHTTPHeader`,
	CMCD_METHOD_QUERY_ARGUMENT = `${CMCDdelivery}:queryArguments`,
	CMCD_METHOD_BODY = `${CMCDdelivery}:body`;
...
import { HbbTV_Standard_versions, HbbTV_Features } from './HbbTV_definitions.mjs'
import { CTA5000_Standard_versions} from './CTA_definitions.mjs'

HbbTV_Standard_versions.forEach((v) => dvbi.ApplicationStandards.push(v))
CTA5000_Standard_versions.forEach((v) => dvbi.ApplicationStandards.push(v))

HbbTV_Features.forEach((f) => dvbi.ApplicationOptions.push(f)) 
