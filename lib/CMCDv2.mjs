...
import { CMCD_METHOD_HTTP_HEADER, CMCD_METHOD_QUERY_ARGUMENT } from "./DVB-I_definitions.mjs";
...
function checkCMCDkeys(Report, version, errs, errCode) {
	...
	const obfuscate_url = Report.attrAnyNsValueOr(dvbi.a_obfuscateURL, null);
	...
	if (reporting_mode == CMCD_MODE_EVENT) {
		if (!isIn(keys, CMCD_keys.timestamp)) 
			errs.addError({
				code: `${errCode}d`,
	...
	if (version >= 2) {
		let missingMandatoryKeyError = (key) => ({
			code: `${errCode}f`,
		...
		if (!isIn(keys, CMCD_keys.buffer_starvation)) errs.addError(missingMandatoryKeyError(CMCD_keys.buffer_starvation));
		if (!isIn(keys, CMCD_keys.startup)) errs.addError(missingMandatoryKeyError(CMCD_keys.startup));
		if (!isIn(keys, CMCD_keys.CMCD_version)) errs.addError(missingMandatoryKeyError(CMCD_keys.CMCD_version));
	}
}
...
function check_CMCD(CMCDelem, counts, errs, errCode) {
	...
	if (isObjectEmpty(counts)) {
		counts[reportingMode(CMCD_MODE_REQUEST)] = counts[reportingMode(CMCD_MODE_EVENT)] = counts.numV1 = 0;
	}
	...
	CMCDversion = parseInt(CMCDversion); // should result in a number or NaN, which is handled in the checks below
	if (CMCDversion == 1) {
		if (counts.numV1 != 0) 
			errs.addError({
				code: `${errCode}-6`,
	...
	CMCDelem.forEachNamedChildElement(dvbi.e_Report, (Report) => {
		...
		switch (reporting_mode) {
			...
			case CMCD_MODE_EVENT:
				...
				const Report_eventURL = Report.attrAnyNsValueOr(dvbi.a_eventURL, null);
				if (Report_eventURL) {
